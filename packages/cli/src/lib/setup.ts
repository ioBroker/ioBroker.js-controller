    ...
    switch (command) {
        ...
        case 'passwd': {
            ...
            dbConnect(params, async ({ objects }) => {
                ...
                void users.setUserPassword(user, password, (err: Error | null | undefined) => {
                    if (err) {
                        console.error(err);
                        return void callback(EXIT_CODES.CANNOT_CREATE_USER_OR_GROUP);
                    }
        ...
        case 'mh': {
            ...
            dbConnect(params, async ({ objects, states }) => {
                ...
                if (cmd === 's' || cmd === 'status') {
                    ...
                    void mh.connect(parseInt(args[1]), args[2], (err: Error | null | undefined) => {
                        if (err) {
                            console.error(err);
                        }
...
export function execute(): void {
    ...
    void processCommand(command, args, _yargs.argv as Record<string, string | boolean | number>, exitApplicationSave);
}

process.on('unhandledRejection', (e: any) => {
    console.error(`Uncaught Rejection: ${e.stack || e}`);
    exitApplicationSave(EXIT_CODES.UNCAUGHT_EXCEPTION).catch(e => console.error(`Cannot exit: ${e.toString()}`));
});
