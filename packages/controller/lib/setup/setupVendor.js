...
function Vendor(options) {
    ...
    this.checkVendor = (file, password, logger) => {
        ...
        if (data.uuid) {
            ...
            // check UUID
            promises.push(
                objects.getObject('system.meta.uuid').then(obj => {
                    if (obj && obj.native) {
                        ...
                        return objects
                            .setObjectAsync('system.meta.uuid', {
                                type: 'meta',
                                common: {
                                    name: 'uuid',
                                    type: 'uuid'
        ...
        if (data.vendor) {
            ...
            // store vendor
            promises.push(
                objects.getObject('system.config').then(obj => {
                    if (obj && obj.native) {
            ...
            Promise.all(promises)
                    ...
                    if (data.objects) {
                        for (let id of Object.keys(data.objects)) {
                            if (!id.includes('*')) {
                                ((_id, _newObj) => {
                                    _promises.push(
                                        objects.getObject(_id).then(obj => {
                                            if (obj) {
                                                obj.nonEdit = obj.nonEdit || {};
                                                ...
                                                if (!isDeepStrictEqual(originalObj, obj)) {
                                                    ...
                                                    return objects
                                                        ...
                                                        .catch(err =>
                                                            logger.error(`Cannot update "${obj._id}": ${err.message}`)
                                                        );
                                                }
                                            } else {
                                                return objects
                                                    .setObjectAsync(_id, _newObj)
                                                    .then(() => logger.info(`object "${_newObj._id}" updated`))
                                                    .catch(err =>
                                                        logger.error(
                                                            `Cannot update ${_id} "${JSON.stringify(_newObj)}": ${
                                                                err.message
                                                            }`
                                                        )
                                                    );
                                                            ...
                                                            if (!isDeepStrictEqual(originalObj, obj)) {
                                                                ...
                                                                tasks.push(
                                                                    objects
                                                                        ...
                                                                        .catch(err =>
                                                                            logger.error(
                                                                                `Cannot update "${obj._id}": ${err.message}`
                                                                            )
                                                                        )
                                                                );
                                                            }
                ...
                .then(() => {
                    if (data.model) {
                        ...
                        return objects.getObject('system.host.' + hostname).then(obj => {
                            if (obj && obj.common) {
                                if (
                                    (model.name &&
                                        model.name !== 'JS controller' &&
                                        obj.common.title === 'JS controller') ||
                                    ...
                                    obj.common.title &&
                                        logger.info(
                                            `Update "system.host.${hostname}:common.title" = "${obj.common.title}"`
                                        );
                                    obj.common.icon &&
                                        logger.info(
                                            `Update "system.host.${hostname}:common.icon"  = "${!!obj.common.icon}"`
                                        );
                                    obj.common.color &&
                                        logger.info(
                                            `Update "system.host.${hostname}:common.color" = "${obj.common.color}"`
                                        );
