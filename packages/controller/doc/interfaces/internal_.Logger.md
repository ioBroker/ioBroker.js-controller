...
When using an older Node.js library that emits `'data'` events and has a [pause](internal_.Logger.md#pause) method that is advisory only, the`readable.wrap()` method can be used to create a `Readable`
stream that uses
