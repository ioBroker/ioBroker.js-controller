...
The buffered data will be flushed when either the [uncork](internal_.Duplex.md#uncork) or [end](internal_.Duplex.md#end) methods are called.
...
Calling the [write](internal_.Duplex.md#write) method after calling [end](internal_.Duplex.md#end) will raise an error.
...
The `writable.uncork()` method flushes all data buffered since [cork](internal_.Duplex.md#cork) was called.
...
recommended to encapsulate the logic into a `Readable` and use [pipe](internal_.Duplex.md#pipe). However, if calling `write()` is preferred, it is
