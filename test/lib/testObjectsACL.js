...
function register(it, expect, context) {
    ...
    it(textName + 'invalid user name must be checked', () => {
        const objects = context.objects;
        return objects.getObject(secretId, {user: 'admin'}).then(obj => {
    ...
    it(textName + 'invalid user name must be checked', () => {
        const objects = context.objects;
        return objects.getObject(secretId, {user: 'system.user.admin1'}).then(obj => {
            expect(1).to.be.equal('Never happens');
        }).catch(err => {
            console.error(err);
            expect(err).to.be.equal('permissionError');
    ...
    it(textName + 'user may not read secret object', () => {
        const objects = context.objects;
        return objects.getObject(secretId, {user: 'system.user.user'}).then(obj => {
            expect(1).to.be.equal('Never happens');
        }).catch(err => {
            expect(err).to.be.equal('permissionError');
        })
