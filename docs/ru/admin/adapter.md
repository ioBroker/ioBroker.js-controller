...
Когда выбран этот значок, отображаются только адаптеры, для которых имеется обновление (функция переключения). Плитки обновляемых адаптеров имеют зеленый заголовок. Если обновление для адаптера отсутствует, появится соответствующее сообщение.
