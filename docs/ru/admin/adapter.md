...
** ВНИМАНИЕ: **** Использование этой опции может привести к проблемам с установкой ioBroker.** Адаптеры GitHub могут работать неправильно (так как они все еще находятся в стадии разработки). Рекомендуется дождаться стабильной версии вместо использования этой опции. Если вы используете этот значок для установки адаптера, вы загружаете версию адаптера для разработки, которую не следует использовать в производственной системе.
...
Если вы выберете вкладку ***ЛЮБОЙ*** , вы можете ввести в поле любой путь к файлу или URL (например, URL-адрес для разработчика внешнего адаптера) и установить соответствующий адаптер.
