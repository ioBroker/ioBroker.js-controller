...
Имейте в виду, что этот пароль хранится в ioBroker незащищенным образом - поэтому не используйте пользователя «admin»!
...
- `<state>` - состояние внутри элемента управления (дополнительную информацию см. в разделе [Поддерживаемые типы элементов управления] (# supported-control-types)).
...
Для получения дополнительных сведений об элементах управления и их состояниях ознакомьтесь также с API Loxone (особенно с файлом структуры): https://www.loxone.com/enen/kb/api/
...
- логическое состояние (истина / ложь) тревоги (rw); запись в это значение true немедленно включит тревогу (без заранее заданной задержки)
...
- `confirmNeeded` (ro) boolean (true / false), нужно ли пользователю подтверждать тревогу
...
- состояние музыкального сервера `serverState` (ro):
    ...
    * -1 = неизвестно
    * 0 = не в сети
    * 1 = инициализация (загрузка, попытки до него добраться)
    * 2 = онлайн
...
- `down` (rw) спускаются ли жалюзи вниз
...
- отсчет `deactivationDelay` (ro), пока выход не будет деактивирован
    ...
    * в противном случае отсчет будет отсчитываться от deactivationDelayTotal
...
- один канал для каждого часа прогноза под названием `HourXX`, где` XX` - количество часов с этого момента
...
- `detectdTemperature-formatted`: отформатированное значение воспринимаемой температуры с единицей измерения
...
Если вам требуется отсутствующий тип элемента управления, укажите имя, как оно указано в журнале ошибок ioBroker, а также все необработанное содержимое устройства в дереве объектов ioBroker:
