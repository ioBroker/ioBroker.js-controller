...
# IoBroker socket.io
Этот адаптер используется некоторыми веб-приложениями и адаптерами для связи с ioBroker с использованием протокола socket.io.
...
- **callback** - функция `` `(error, objs)` `` - objs выглядит следующим образом: `` `{'id1': 'object1', 'id2': 'object2', ...}` ` `
...
- **callback** - функция `` `(ошибка, данные, шаг, sessionId) {}` ``
