...
- int16le - 16-битная подпись (Little Endian): AABB => BBAA
...
- uint64be - 64-битная беззнаковая (Big Endian): AABBCCDDEEFFGGHH => AABBCCDDEEFFGGHH
...
Кроме того, и «замена байтов», и «замена слов» по существу полностью изменили бы последовательность байтов в целом, чтобы получить еще один результат:
...
Учитывая, что протокол сообщения Modbus RTU имеет формат с прямым порядком байтов, для успешного обмена 32-битным типом данных через сообщение Modbus RTU необходимо учитывать порядок байтов как главного, так и подчиненного устройств. Многие ведущие и ведомые устройства RTU допускают конкретный выбор порядка байтов, особенно в случае программно-имитируемых модулей. Нужно просто убедиться, что оба устройства имеют одинаковый порядок байтов.
...
Обратите внимание, что различные порядки байтов и слов требуют использования соответствующей функции перемещения FieldServer. После выбора правильного перемещения данных данные можно преобразовать в обоих направлениях.
...
- Ananas32 / 64 - ведомый симулятор (только регистры и входы, без катушек и цифровых входов)
...
# 0.3.1 (2015-10-26)
* (bluefox) исправить ошибку с длиной блока чтения (мастер)
