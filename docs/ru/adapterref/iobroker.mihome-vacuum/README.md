...
Если робот выбирает только одну область и всегда строит карту одинаково, вы можете надежно отправить ее в разные места или очистить эту область пылесосом.
...
Под объектом «mihome -uum.X.control.X_send_command» вы можете отправлять свои собственные команды роботу.
...
| app_rc_move | [{"seqnum": '0-1000', "скорость": VALUE1, "омега": VALUE2, "duration": VALUE3}] | Шаг. Порядковый номер должен быть непрерывным, VALUE1 (скорость) = -0,3-0,3, VALUE2 (вращение) = -3.1-3.1, VALUE3 (продолжительность)
...
| чистая комната / комнаты | `cleanRooms` | `rooms` | `rooms` - это разделенная запятыми строка с enum.rooms.XXX |
...
## Ошибки
- случайные разъединения, однако, это не из-за адаптера, а главным образом в его собственных сетях
