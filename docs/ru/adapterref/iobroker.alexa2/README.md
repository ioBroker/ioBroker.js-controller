...
Большое спасибо soef за хорошую версию 1 адаптера, а также Hauke и ruhr70 за идеи в их сценариях от ioBroker-Forum (особенно обновления медиа-прогресса)! Также большое спасибо meicker за поддержку в документировании всего этого и многочисленным пользователям ioBroker Forum за поддержку тестирования!
...
### Alexa2.0. Разговор. *
Bespoken обычно является поставщиком услуг, который помогает автоматически тестировать навыки. Но на самом деле вы можете использовать его для отправки команд на «ваш» аккаунт Alexa / Amazon. При этом вы можете запускать действия умений, которые обычно не доступны через приложение Alexa. По характеру его работы возможны только команды, которые не взаимодействуют напрямую с «устройством, с которым вы разговариваете», как обычные команды, которые выполняют определенное действие и дают ответ. Воспроизведение аудио или видео или что-либо подобное, обычно выполняемое устройством, которому вы дали команду, не будет работать!
...
Все Alexa-контакты, которые можно использовать для отправки текстовых сообщений, включая его самого. Собственный контакт получает специальное «(Я)» после его имени.

| Государственное название | смысл |
| - | - |
| #clearOwnMessages | Существует только в собственном контакте, а триггер удаляет все сообщения, которые отправляются самому себе (также включает сообщения себе через приложение или устройства!) |
...
Как правило, вы можете отправить только 250 символов на команду выступления. Используя точку с запятой, можно написать столько, сколько вы хотите, при условии, что вы разделяете 250 символов точкой с запятой.
...
Обзор процедур, установленных в Alexa App. Самостоятельно созданные подпрограммы имеют серийный номер, который Amazon показывает как «предварительно сконфигурированный: ...». Каждая подпрограмма может быть запущена с помощью кнопки для запуска один раз.
...
Вы можете использовать один или несколько таймеров на каждом устройстве Alexa. Из-за очень динамической природы таймеров больше не будет создаваться никаких объектов, таких как «Тревога» или «Напоминания», но существует способ получения информации о срабатывании.
...
| colorName | Название цвета, как определено Alexa - фиксированные значения | изменяемый для установки цвета, 0-144 |
...
С помощью #brightness вы можете регулировать яркость вашего света, #colorName - выбрать один предопределенный цвет (0-144). Для HUE Ambient light вы можете выбрать между 19 значениями от 0 до 18 в #colorTemperaName. Весь свет может включаться и выключаться с помощью #powerState.
...
* Как обновить исходное состояние для тома, перемешать или повторить и сделать не нарушать ?! Или ненужный?
