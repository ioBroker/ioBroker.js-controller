...
Если вы не уверены, использовался ли Redis и есть ли доступ к старой системе, используйте команду `iobroker status` для получения необходимой информации. «При использовании Redis результат будет выглядеть так:
