...
Если нет уверенности в том, использовался ли Redis, а доступ к старой системе все еще существует, используйте команду `iobroker status` для получения необходимой информации. «Вывод выглядит так, когда используется Redis:
...
Если для типа объектов и / или типа состояний указано «redis», необходимо установить сервер Redis в новой системе.
...
С помощью Backitup теперь есть возможность восстановить дополнительные данные, если они были зарезервированы заранее на старой системе.
...
Прежде всего, несколько команд должны быть введены через Putty или аналогичные.
...
Как только это будет сделано, вы можете остановить свой ioBroker следующим образом:
...
Теперь это сделано, и система установлена заново, а все настройки, сценарии, визуализации и т. Д. Восстановлены.
