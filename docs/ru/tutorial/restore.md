...
Если вы не уверены, использовался ли Redis, и есть ли доступ к старой системе, то используйте команду `iobroker status` для вызова необходимой информации. «При использовании Redis результат будет выглядеть следующим образом:
...
С помощью небольшого инструмента «htop» вы можете очень хорошо видеть все запущенные процессы, что не только интересно для восстановления, но и в целом очень полезно.
...
Поскольку здесь не требуется никаких знаний о Linux и все это делается через веб-интерфейс Iobroker, вариант автоматического восстановления с использованием [BackItUp](https://github.com/simatec/ioBroker.backitup/blob/master/README.md) имеет место первым.
...
Как только это произойдет, вы остановите свой ioBroker следующим образом:
...
Для этого требуется резервное копирование, поскольку оно было специально создано с помощью Backitup.
