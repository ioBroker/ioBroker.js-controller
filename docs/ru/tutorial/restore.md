...
Там установите "источник резервной копии" на локальный, а затем сохраните.
...
С помощью Backitup теперь есть возможность восстановить дополнительные данные, если они были предварительно зарезервированы в старой системе.
...
В этом случае список извлеченных резервных копий будет выглядеть, как в приведенном здесь примере.
...
Во-первых, нужно отправить несколько команд через Putty или что-то подобное.
...
Если бы только состояния были под Redis, это не обязательно должно быть заранее.
