...
Если нет уверенности в том, что Redis использовался, а доступ к старой системе все еще существует, используйте команду `iobroker status` для получения необходимой информации. «Вывод выглядит так, когда используется Redis:
...
Если ВСЕ экземпляры должны запускаться автоматически после восстановления, необходимо активировать, а затем сохранить параметр «Запускать все адаптеры после восстановления».
