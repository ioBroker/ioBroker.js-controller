...
Если нет уверенности в том, что Redis использовался и доступ к старой системе все еще существует, используйте команду `iobroker status` для получения необходимой информации. «Вывод выглядит так, когда используется Redis:
...
Если вам больше не нужно иметь доступ к старой системе и вы не знаете, что там было предварительно настроено, то заранее установите сервер Redis.
...
Затем вы должны позволить установщику выполнить исправление, если все права в системе не должны соответствовать где-либо.
...
После завершения установки вы можете сохранить «Резервное копирование ioBroker», созданное старой системой, в новой системе, используя программу sftp, например FileZilla или WinSCP, в пути / opt / iobroker / backups.
...
Все адаптеры, которые все еще установлены или находятся в режиме ожидания, еще не имеют значок в экземплярах.
