...
### Предисловие:
Поскольку некоторым пользователям очень трудно восстанавливать, должно помочь пошаговое руководство по восстановлению после сбоя или после замены оборудования, изменения системы или чего-либо еще.
...
Если нет уверенности в том, что Redis использовался, а доступ к старой системе все еще существует, используйте команду `iobroker status` для получения необходимой информации. «Вывод выглядит так при использовании Redis:
...
Если «redis» относится к типу Objects и / или типу States, вы должны установить сервер Redis в новой системе.
...
Если это так, вы можете запустить iobroker вручную с помощью следующей команды через консоль.
...
В случаях вы можете видеть, какие адаптеры постепенно устанавливаются.
