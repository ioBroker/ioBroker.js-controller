...
В принципе, вы можете сказать одно заранее: восстановление выполняется за несколько минут, если все сделано правильно, и никто не должен бояться.

В конце все данные снова доступны, и была установлена новая система.
...
Если объекты типа и / или состояния «redis», вы должны установить сервер Redis в новой системе.
...
Далее вы должны запустить установщик Fix time, если где-то не все права должны соответствовать системе.
...
С помощью небольшого инструмента «htop» вы можете очень хорошо видеть все запущенные процессы, которые не только для восстановления интересны, но и вообще очень полезны.
...
Если резервное копирование ioBroker было успешно сохранено, откроется Backitup и откроется вкладка «Восстановление».
...
Обычно восстановление должно быть выполнено примерно через 10-15 минут, и ioBroker перезапускается.
...
В процессах, которые могут быть отображены с помощью htop **ни один из** процессов, отмеченных на изображении, больше не доступен.
Если это так, остановите htop с помощью «F10» и выполните следующую команду через терминал.

```
iobroker start
```

Теперь ioBroker должен запуститься снова, и на вкладке «Журнал» вы можете увидеть, что все адаптеры, которые были установлены в старой системе, переустанавливаются npm.

Здесь нужно немного терпения, и пусть просто заработает iobroker.
В случаях вы можете увидеть, какие адаптеры установлены так постепенно.
...
Если все проблемы исправлены и iobroker был остановлен, для восстановления консоли можно использовать следующие команды:
...
Если у вас мало опыта работы с терминальными командами и вы чувствуете себя неуверенно, то Backitup находится на безопасной стороне.

Однако, если вы хотите точно увидеть, что происходит в вашей системе, вам следует выбрать версию вручную через консоль. Здесь вы можете увидеть каждый отдельный процесс в терминале.