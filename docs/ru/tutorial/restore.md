...
В принципе, можно заранее сказать одно: восстановление может быть выполнено за несколько минут, если оно выполнено правильно, и никого не нужно бояться.
...
Если вы не уверены, использовался ли Redis, а доступ к старой системе все еще есть, используйте команду `iobroker status`, чтобы получить необходимую информацию. «При использовании Redis вывод выглядит следующим образом:
...
Поскольку здесь не требуется никаких знаний о Linux, а все это делается через веб-интерфейс Iobroker, сначала применяется вариант автоматического восстановления с использованием [Резервное копирование](https://github.com/simatec/ioBroker.backitup/blob/master/README.md).
...
Резервное копирование также может выполнять восстановление с NAS, Dropbox или Google Диска, но локальный вариант имеет меньше всего проблем.
