...
hash: hx3wXtdD+XiC1OM1qwPEfGVcJ34OcPiljh0YKH36UzE=
---
# Основная концепция
...
## ID
ID - это строка максимальной длиной 240 байт, иерархически структурированная, уровни разделены точками.
...
- `system` - пространство имён для системных объектов
...
для метода `setState` все, кроме `val`, является необязательным, `from` устанавливается автоматически методом `setState`. `ack` по умолчанию имеет значение false, `ts` и `lc` установлены должным образом
...
Древовидная структура автоматически собирается по именам. Например. ```system.adapter.0.admin``` является родительским для `system.adapter.0.admin.uptime`. Используйте это соглашение об именах с точкой «.» Как разделителем уровней.
...
список транспорта см. в адаптере истории README
...
* `remote` - ТВ или другие пульты с состоянием - строка с нажатыми значениями, например «ИГРАТЬ», «СТОП», «ПАУЗА»
...
#### Описание каналов
~~ Имена атрибутов могут быть определены адаптером произвольно, кроме тех, которые написаны **жирным** шрифтом. ~~
...
| ммм | индикатор. обслуживание.ммм | | | | mmm = lowbat или unreach или что-то еще |
...
* `common.adminColumns` - настраиваемые атрибуты, которые должны отображаться в админке в обозревателе объектов. Например: `[{" name ": {" en ":" KNX-адрес "}," path ":" native.address "," width ": 100}, {" name ":" DPT "," path ": "native.dpt", "width": 100, "edit": true}] `
* `common.adminTab.fa-icon` - Font-Awesome имя значка для TAB.
* `common.adminTab.ignoreConfigUpdate` - не обновлять ВКЛАДКУ конфигурации, если конфигурация изменена (чтобы включить параметры настройки во ВКЛАДКЕ)
* `common.adminTab.link` - ссылка на iframe во ВКЛАДКЕ. Вы можете использовать замену параметров так: «http://% ip%:% port%». IP будет заменен на IP хоста. "порт" будет извлечен из native.port.
* `common.adminTab.name` - имя TAB в админке
* `common.adminTab.singleton` - [true / false], если в адаптере есть TAB для администратора. Будет показана только одна вкладка для всех экземпляров.
...
* `common.dataSource` - Как данные будут получены от устройства:` poll / push / предположение`. Это важно вместе с `connectionType`.
* `common.dependencies` - массив типа` [{"js-controller": "> = 2.0.0"}] `, который описывает, какие модули ioBroker требуются для этого адаптера.
...
* `common.enabled` - **обязательный** [true / false] значение должно быть false, поэтому новые экземпляры по умолчанию отключены
...
* `common.localLinks` - ссылка на веб-сервис этого адаптера. Например, по адресу http:// localhost: 5984 / _utils для футона от администратора
...
* `common.logTransporter` - если этот адаптер получает журналы с других хостов и адаптеров (например, чтобы сохранить их где-нибудь)
* `common.main` - Стартовый файл адаптера. То же, что и в package.json.
...
* `common.messagebox` - истина, если поддерживается окно сообщения. Если да, то будет создан объект system.adapter. &lt; adapter.name & gt &lt; adapter.instance & gt.messagebox для отправки сообщений адаптеру (используется для электронной почты, pushover, ...;
...
* `common.osDependencies.win32` - не используется, поскольку win32 не имеет диспетчера пакетов
...
* `common.readme` - устарело. Используйте `docs`.
...
* `common.titleLang` - **обязательно** более длинное название адаптера на всех поддерживаемых языках, например {en: 'Adapter', de: 'adapter', ru: 'Драйвер'}
...
* `common.unchanged` - (система), пожалуйста, не используйте этот флаг. Это флаг, информирующий систему о том, что диалог конфигурации должен отображаться в админке.
...
* `common.webExtension` - относительное имя файла для подключения веб-расширения. Например. в simple-api "lib / simpleapi.js" относительно корневого каталога адаптера. Кроме того, требуется native.webInstance, чтобы указать, куда будет включено это расширение. Пустой означает, что он должен работать как собственный веб-сервис. «*» означает, что каждый веб-сервер должен включать его.
...
* `common.wwwDontUpload` - Не загружать в БД каталог www. Используется только для администратора. Вы можете просто назвать свой каталог как-нибудь еще и ОК.
...
* `subscribe` - запускается, когда состояние *system.adapter. &lt; имя-адаптера &gt;. & lt; instance-number & gt; .alive* изменяется на *true* Убивается, когда *.alive* изменяется на *false* и устанавливает *.alive* на *false* если процесс завершается (не будет **перезапущен** при выходе из процесса)
