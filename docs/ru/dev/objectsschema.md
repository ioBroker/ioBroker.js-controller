...
### Пространство имен system.host. & Lt; hostname &gt;
```
{
    _id:   id,
    type: 'host',
...
* `common.messagebox` - true, если поддерживается окно сообщения. Если да, будет создан объект system.adapter. & Lt; имя-адаптера & gt &lt; adapter.instance & gt.messagebox для отправки сообщений адаптеру (используется для электронной почты, отправки, ...;
