...
для метода `setState` все, кроме `val` является необязательным, `from` устанавливается автоматически методом `setState`. `ack` по умолчанию имеет значение false, `ts` и `lc` настроены так, как ожидается
...
* `ts` - отметка времени Unix, указывающая последнее обновление состояния (в миллисекундах)
...
* `common` - объект, содержащий свойства абстракции, характерные для ioBroker
...
Древовидная структура собирается автоматически по именам. Например. ```system.adapter.0.admin``` является родительским для `system.adapter.0.admin.uptime`. Используйте это соглашение об имени с точкой ".", Как разделитель уровней.
...
* `common.workingID` (строка, необязательно) - если это состояние имеет вспомогательное состояние WORKING. Здесь должно быть написано полное имя или только последняя часть, если первые части совпадают с фактическими. Используется для HM.LEVEL и обычно имеет значение "РАБОЧАЯ"
...
* `light.color.hsl` - Установить цвет в оттенках / насыщенности / яркости (цветовой тон оттенка - LivingColors ...)
...
* `common.allowInit` - [true / false] позволяет« запланированному »адаптеру называться« не в расписании », если изменились настройки или запущен адаптер. Или разрешите запуск запланированного адаптера один раз после изменения конфигурации, а затем по расписанию.
...
* `common.connection-type` - тип соединения с устройством. Смотрите [Типы соединений] (adapterpublish.md)
* `common.compact` - сообщает контроллеру, что при желании этот адаптер может быть запущен в том же процессе
...
* `common.docs` - структура типа` {"en": "docs / en / README.md", "de": ["docs / de / README.md", "docs / de / README1.md" ]} `которая описывает документацию, если не в README.md
...
* `common.keywords` - аналогичен ключевым словам в package.json, но может быть определен на многих языках. Просто массив.
...
* `common.messagebox` - true, если поддерживается окно сообщения. Если да, будет создан объект system.adapter. &lt; adapter.name & gt &lt; adapter.instance & gt.messagebox для отправки сообщений адаптеру (используется для электронной почты, pushover, ...;
...
* `common.onlyWWW` - [true / false] сообщает контроллеру, что в адаптере есть только html-файлы и нет main.js, например, рикша
...
* `common.welcomeScreen` - массив страниц, который должен отображаться на странице« web »index.html. ["vis / edit.html", "vis / index.html"] или [{"link": "vis / edit.html", "name": "Vis editor", "img": "vis / img / edit.png "," color ":" blue "}," vis / index.html "]
...
* `common.host` - (обязательный) хост, на котором должен запускаться адаптер - объект *system.host. &lt; host &gt;* должен существовать
