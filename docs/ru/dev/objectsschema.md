...
* `enum` - объекты, содержащие в общем массиве элементы, указывающие на состояния, каналы, устройства или файлы. перечисления могут иметь родительское перечисление (возможна древовидная структура)
...
* `common.type` (необязательный - (по умолчанию смешанный == любой тип) (возможные значения: число, строка, логическое значение, массив, объект, смешанный, файл). В качестве исключения объекты с типом` meta` могут иметь `common .type = meta.user` или `meta.folder`
...
##### Штат `common.history`
Для функции истории требуется адаптер истории или любой другой адаптер хранилища типа история.
...
* `common.main` - **Не рекомендуется** Использовать main в package.json.
