...
hash: w9muL5MeLsTIpkxeaMm57s3izdYinIaUHAcjJkfOKLc=
---
# Основная концепция
...
* `instance` - экземпляр адаптера. Родитель должен иметь тип адаптера
...
* `common.custom` (необязательно) - структура с пользовательскими настройками для конкретных адаптеров. Как `{" Influxdb.0 ": {" enabled ": true," alias ":" name "}}`. Атрибут enabled является обязательным, и если он неверен, весь атрибут будет удален.
...
список транспортов смотрите в адаптере истории README
...
* `button` - как настенный переключатель или пульт от телевизора, где каждая кнопка находится в состоянии, например .play, .stop, .pause.
...
* `common.adminTab.link` - ссылка на iframe во ВКЛАДКЕ. Вы можете использовать замену параметров так: "http://% ip%:% port%". IP будет заменен IP-адресом хоста. "порт" будет извлечен из native.port.
...
* `common.config.minHeight` - минимальная высота для диалога конфигурации (устарело - действительно только для admin2)
...
#### Пользователь
* `common.name` - (обязательный) Имя пользователя (с учетом регистра)
