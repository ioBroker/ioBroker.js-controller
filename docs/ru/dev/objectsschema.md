...
* & lt; имя-адаптера & gt; - объект, содержащий вложения, доступные через http:// & lt; couch & gt;: 5984 / iobroker / & lt; имя-адаптера & gt; / path
...
* scripts.py. - Скрипты Python Engine Engine (будущее)
...
* `meta` - редко меняющая мета-информацию, которая нужна адаптеру или его экземплярам
* `config` - настройки
* `script`
* `пользователь`
* `группа`
...
* `common.history.HISTORY-INSTANCE.changesOnly` (необязательно, логическое значение, если записано только значение true, в котором регистрируются изменения)
...
* `common.messagebox` - true, если поддерживается окно сообщения. Если да, будет создан объект system.adapter. & Lt; имя-адаптера & gt & lt; adapter.instance & gt.messagebox для отправки сообщений адаптеру (используется для электронной почты, отправки, ...;
...
* `common.adminTab.link` - ссылка для iframe в TAB. Вы можете использовать замену параметров следующим образом: «http://% ip%:% port%». IP будет заменен на IP-адрес хоста. "порт" будет извлечен из native.port.
...
* ** подписка ** - запускается, когда состояние *system.adapter. & lt; имя-адаптера & gt;. & lt; номер-экземпляра & gt; .alive* меняется на *true* Уничтожается при изменении *.alive* на *false* и установке *.alive* на *false* при выходе из процесса (** не ** будет перезапущен при выходе из процесса)
...
#### Пользователь
* `common.name` - (обязательно) Имя пользователя (@HQ: регистр не учитывается? @Bluefox ваш выбор, я думаю, что регистр тоже подходит)
