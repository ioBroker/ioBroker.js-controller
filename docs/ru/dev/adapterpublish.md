...
При желании несколько авторов также можно сохранить для npm, используя атрибут `contributors` в package.json.
...
Первая часть (слева направо) представляет собой `Major Part`, вторая часть - `minor` Part, а последняя часть - `micro` Part.
