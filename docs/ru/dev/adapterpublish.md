...
10. Адаптер должен использовать оба теста, указанные в шаблоне. Для этого учетную запись Github можно связать с Appveyor (тесты Windows) и Travis CI (тесты Linux и Mac OS), а соответствующий репозиторий можно зарегистрировать для тестирования.
...
2. Для адаптера имеется [Тестовая ветка форума] (https://forum.iobroker.net/viewforum.php?f=36), в которой пользовательский отзыв уже получен.
...
4. Адаптер должен определить атрибут списка `docs` в io-package.json, указав, где искать инструкции на соответствующем языке.
...
Определите §SSSSS_0§§ в §SSSSSSS_1§§ как:
