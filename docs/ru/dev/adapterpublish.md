...
8. В io-package.json атрибуты `connectionType` и` dataSource` должны быть созданы в общем. Для этой цели в этом [списке] (# тип подключения адаптера) должна быть указана наилучшая категория подключения.
...
Требуются инструкции на английском языке (в экстренных случаях можно сослаться на стандарт README). Немецкое руководство также желательно, так как большинство пользователей говорят по-немецки, но это необязательно.
...
При этом первая часть (слева направо) представляет собой `Major Part`, вторая часть - часть `minor`, а последняя часть - часть `micro`.
