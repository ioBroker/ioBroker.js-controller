...
If you are not sure whether Redis was used and there is still access to the old system, use the command `iobroker status` to call up the required information.
...
Anyone who has little experience with terminal commands and feels unsafe is on the safe side with Backitup.
