...
If you are not sure whether Redis was used and whether there is still access to the old system, then use the command `iobroker status` to get the required information. "When using Redis, the output looks like this:
...
Backitup can also do a restore from the NAS, Dropbox or Google Drive, but the local variant has the fewest possibilities for problems.
...
Congratulations, the newly installed system is now ready with all settings, scripts, visualizations, etc.
