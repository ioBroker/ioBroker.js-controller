...
Since no Linux knowledge is required here, and the whole thing is done via the Iobroker web interface, the variant of automatic restore using [Backitup](https://github.com/simatec/ioBroker.backitup/blob/master/README.md) takes place first.
...
If ALL instances should start automatically after the restore, the option “Start all adapters after the restore” must be activated and then saved.
...
Please do not restart ioBroker, at most update the view with F5 every now and then until all instances have an icon.
...
If all outputs are correct and iobroker has been stopped, the restore can now be performed via the console with the following commands:
