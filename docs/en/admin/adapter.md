...
** ATTENTION: **** Using this option can lead to problems with your ioBroker installation.** Adapters from GitHub may not work properly (as they are still under development). It is recommended to wait for a stable release instead of using this option. If you install an adapter using this icon, you are downloading a development version of an adapter that should not be used in a productive system.
