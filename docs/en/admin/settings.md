...
If only one history adapter (SQL / History / InfluxDB) is installed, this is used; if there are several, one can be selected.
...
A random key as a password for the account is generated.
...
On the right-hand side, the exact form in which this data is sent is then displayed.
