...
从ioBroker设置上述状态中的一个或多个只会在大约100毫秒后向Miniserver发送命令。
这是为了防止单个用户输入的颜色多次更改。
...
-`off`（wo）将任何值写入此状态将禁用调光器，将位置设置为0，但记住最后一个位置
...
-`version`（ro）仅限Loxone对讲机-包含当前已安装固件的文本
...
###灯光控制器
由（酒店）照明控制器提供。
只能在Loxone应用程序中修改场景，但是可以在ioBroker中选择场景。
...
-`active`（rw）按钮的当前状态
-`pulse`（wo）向此状态写入任何值将模拟仅在很短时间内按下按钮
