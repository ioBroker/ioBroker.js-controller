...
2.打开实例配置对话框
3.按下“搜索设备”按钮以发现所有已连接的I2C设备-这将需要一些时间，请耐心等待！
4.在各自的选项卡中配置所有找到的设备。
5.保存配置（这将重新启动适配器）
...
如果已连接设备，但未在配置屏幕中显示该设备，请确保将用户添加到`i2c`组中：
...
### ADS1015 16位4通道ADC（48-4B）
德州仪器（TI）具有内部基准的4个3.3kSPS，12位ADC。

### ADS1115 16位4通道ADC（48-4B）
德州仪器（TI）具有内部基准的4x 860-SPS，16位ADC。
...
### MCP3422 18位2通道ADC（68）
具有板载基准的Microchip 18位2通道模数转换器。

### MCP3423 18位2通道ADC（68-6F）
具有板载基准的Microchip 18位2通道模数转换器。

### MCP3424 18位4通道ADC（68-6F）
具有板载基准的Microchip 18位，4通道模数转换器。

### MCP3426 16位2通道ADC（68）
具有板载基准的Microchip 16位2通道模数转换器。

### MCP3427 16位2通道ADC（68-6F）
具有板载基准的Microchip 16位2通道模数转换器。

### MCP3428 16位4通道ADC（68-6F）
具有板载基准的Microchip 16位，4通道模数转换器。
...
Adafruit PCA9685转接板，用于16通道12位PWM。适配器专注于将0..4095中的16个通道用作LED调光器。
...
##脚本中的用法
脚本中`sendTo`支持的命令为`search`，`read`和`write`。

`search`将总线编号作为消息，并返回总线上找到的地址数组的JSON字符串。

`read`将包含地址和可选寄存器以及要读取的字节数的对象作为消息。它返回包含读取数据的缓冲区。

`write`将包含地址，数据作为缓冲区和可选写入寄存器的对象作为消息。成功时将返回已写入的缓冲区。
...
###远程I2C
如果您是在台式机上进行开发并希望在SBC（例如Raspberry Pi）上测试I2C，则可以执行以下操作：
...
-在台式机上手动配置适配器实例，使其在“本机”中包含“ clientAddress”设置：
...
-重新启动台式机上的适配器实例
-适配器现在将在已配置的SBC上执行所有I2C命令，而不是在本地执行
