...
如果不确定是否使用Redis并且仍然可以访问旧系统，请使用`iobroker status`命令获取所需的信息。“使用Redis时，输出如下所示：
...
现在，ioBroker应该再次启动，并且“日志”选项卡显示npm正在重新安装旧系统上安装的所有适配器。
