...
如果您再也无法访问旧系统并且不知道之前的配置是什么，那么您绝对应该提前安装Redis服务器。
...
安装完成后，您可以使用sftp程序（例如FileZilla或WinSCP）将以前由旧系统创建的“ ioBroker备份”保存在新系统中的路径/ opt / iobroker / backups中。
...
现在将在浏览器中打开另一个选项卡，您可以在其中像在控制台上一样在Backitup WebUI中遵循恢复过程。
...
请不要重新启动ioBroker，最多时不时使用F5更新视图，直到为所有实例提供图标为止。
