...
如果您无法再访问旧系统，并且不知道之前进行了确切的配置，则一定要提前安装Redis服务器。
...
使用小型工具“ htop”，您可以很好地看到所有正在运行的进程，这不仅对还原很有趣，而且通常非常有用。
安装如下：
...
安装完成后，可以使用sftp程序（例如FileZilla或WinSCP）将先前由旧系统创建的“ ioBroker备份”保存在新系统中，该文件位于/ opt / iobroker / backups路径中。
...
现在，将在浏览器中打开另一个选项卡，您可以在其中跟随Backitup WebUI中的恢复过程，就像在控制台上一样。
...
使用Backitup，现在有可能恢复更多数据，前提是这些数据也已事先备份在旧系统上。
...
如果只有州在Redis下运行，则不必一定要提前。

一旦发生这种情况，您可以按照以下步骤停止ioBroker：
...
为此需要Backitup，因为它们是使用Backitup专门创建的。
...
现在已完成，并重新安装了系统，并还原了所有设置，脚本，可视化文件等。

###结论：
基本上，两个变体都会导致相同的结果。
如果您几乎没有使用终端命令的经验并且感到不安全，那么Backitup会让您感到安全。

但是，如果要确切查看系统上发生的情况，则应通过控制台选择手动变量。在这里，您可以在终端中详细了解每个流程。