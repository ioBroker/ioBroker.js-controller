...
#### ** 1。通过备份自动还原**
由于此处不需要Linux知识，并且不需要通过Iobroker Web界面进行整个操作，因此自动还原的变体首先使用[BackItUp中](https://github.com/simatec/ioBroker.backitup/blob/master/README.md)进行。
...
结论：
基本上，两个变体都会导致相同的结果。
如果您几乎没有使用终端命令的经验，并且在那儿感到不安全，那么Backitup就是安全的一面。
