...
对象代表很少更改的较大数据，例如系统设备的元数据，配置和其他文件。每个对象都必须具有属性“类型”。有关更多信息，请参见下文，特定对象类型需要哪些可用的对象类型以及哪些必需属性。适配器模块将为您提供诸如setObject，getObject等功能。
...
*`common.type`（可选-（默认为blend ==任何类型）（可能的值：数字，字符串，布尔值，数组，对象，混合，文件）。作为例外，类型为meta的对象可以具有common .type = meta.user`或`meta.folder`
*`common.min`（可选）
*`common.max`（可选）
...
*`common.logTransporter`-如果此适配器从其他主机和适配器接收日志（例如，将日志散布在某处）
