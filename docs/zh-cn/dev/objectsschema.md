...
*`user`-用户名，用于设置值
*`expire`-一个整数值，可用于设置在给定秒数后到期的状态。可以和setValue一起使用。该值到期后，它将从redisDB中消失。
*`c`-此状态更改的注释。
*`q`-质量。具有以下状态的编号：
...
*`common.role`（字符串，必需的）-状态的角色（在用户界面中用于指示要选择哪个小部件，请参见下文）
*类型号的`common.states`（可选）属性，带有可能状态的对象'{'value'：'valueName'，'value2'：'valueName2'，0：'OFF'，1：'ON'}`
*`common.workingID`（字符串，可选）-如果此状态具有辅助状态WORKING。如果前几部分与实际名称相同，则必须在此写上全名或最后一部分。用于HM.LEVEL，通常值为“ WORKING”
...
####频道说明
~~属性名称可以由适配器自由定义，用** bold **字体书写的属性除外。~~
...
*`common.compact`-对控制器说，如果需要，该适配器可以在同一过程中启动
...
*`common.dependencies`-类似于[[{“ js-controller”：“> = 2.0.0”}]]之类的数组，它描述此适配器需要哪些ioBroker模块。
...
*`common.keywords`-与package.json中的关键字相似，但是可以用多种语言进行定义。只是一个数组。
...
*`common.main`-适配器的启动文件。与package.json中的相同。
...
*`common.osDependencies.linux`-此适配器所需的debian / centos软件包数组（当然，只有带有apt，apt-get，yum作为软件包管理器的OS）
...
*`common.serviceStates`-[true / false或path]如果适配器可以传递其他状态。如果是，路径适配器/lib/states.js将被调用，并提供以下参数功能（对象，状态，实例，配置，回调）。函数必须传递点数组，其值应类似于函数（err，result）{result = [{id：'id1'，val：1}，{id：'id2'，val：2}]}}
...
*`common.titleLang`-**必填**所有受支持语言的适配器名称，例如{en：'Adapter'，de：'adapter'，ru：'Драйвер'}
*`common.title`-（不建议使用）更长的适配器名称，以在admin中显示
...
*`native`-预定义属性，可在index_m.html中并在运行时通过`adapter.config。<attribute>`访问，例如`{“端口”：1234，“密码”：“秘密”}`
