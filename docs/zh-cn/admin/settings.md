...
这是用于SSL / HTTPS通信的证书的中心点。证书由admin，web，simple-api，socketio使用。默认情况下安装标准证书。您无法使用它进行任何验证。它们仅用于SSL通信。由于证书是开放的，因此您应该使用自己的（自签名）证书，购买真实证书或切换到“让我们加密”。使用默认证书的通信是不安全的，如果有人想读取流量，则可以这样做。必须安装自己的证书。
例如。在linux下。
