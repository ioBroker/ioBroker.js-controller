...
##标题行
标题栏包含最重要过程的图标。每个图标都有上下文帮助。只需用鼠标停留在图标上一会儿即可。
...
选择此图标时，仅显示有更新的适配器（切换功能），可更新适配器的磁贴具有绿色标题。如果没有适配器的更新，则会显示相应的消息。
...
**注意：** **使用此选项可能会导致ioBroker安装出现问题** GitHub适配器可能无法正常工作（因为它们仍在开发中）。建议等待稳定版本而不是使用此选项。如果使用此图标来安装适配器，那么将下载不应该在生产系统中使用的适配器的开发版本。
