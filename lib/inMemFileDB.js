...
class InMemoryFileDB {
    ...
    constructor(settings) {
        ...
        this.dataDir = path.normalize(path.join(__dirname, '..', this.dataDir) + path.sep);
        ...
        this.backupDir = this.settings.backup.path || (path.join(this.dataDir, this.settings.fileDB.backupDirName) + path.sep);
    ...
    handleSubscribe(client, type, pattern, options, cb) {
        ...
        if (pattern instanceof Array) {
            pattern.forEach(pattern => {
                if (s.find(sub => sub.pattern === pattern)) return;
