...
class InMemoryFileDB {
    ...
    constructor(settings) {
        ...
        if (this.dataDir) {
            if (this.dataDir[0] === '.' && this.dataDir[1] === '.') {
                this.dataDir = __dirname + '/../' + this.dataDir;
            } else if (this.dataDir[0] === '.' && this.dataDir[1] === '/') {
                this.dataDir = __dirname + '/../' + this.dataDir.substring(2);
            }
        }
        this.dataDir = path.normalize(this.dataDir);
        this.dataDir = this.dataDir.replace(/\\/g, '/');
        if (this.dataDir[this.dataDir.length - 1] !== '/') this.dataDir += '/';
        ...
        this.datasetName = this.dataDir + this.settings.fileDB.fileName;
        this.stateTimer = null;

        this.backupDir = this.settings.backup.path || (this.dataDir + this.settings.fileDB.backupDirName + '/');
        ...
        if (fs.existsSync(this.datasetName)) {
            try {
                this.dataset = JSON.parse(fs.readFileSync(this.datasetName).toString());
            } catch (e) {
                this.log.error(this.namespace + ' Cannot parse ' + this.datasetName + ': ' + e);
                if (fs.existsSync(this.datasetName + '.bak')) {
                    try {
                        this.dataset = JSON.parse(fs.readFileSync(this.datasetName + '.bak').toString());
                    } catch (e) {
        ...
        /** @type {string|string[]} */
        let parts = this.datasetName.split('/');
        parts.pop();
        parts = parts.join('/');
    ...
    handleSubscribe(client, type, pattern, options, cb) {
        ...
        if (!client._subscribe[type]) client._subscribe[type] = [];
    ...
    deleteOldBackupFiles() {
        ...
        for (let f = files.length - 1; f >= 0; f--) {
            if (!files[f].endsWith(this.settings.fileDB.fileName + '.gz')) {
                files.splice(f, 1);
            }
        }
        ...
        while (files.length > this.settings.backup.files) {
            ...
            if (limit > ms) {
                try {
                    ...
                    this.log.error(`Cannot delete file "${this.backupDir + file}: ${JSON.stringify(e)}`);
                }
            }
        }
    }
        ...
        try {
            ...
            if (!this.settings.backup.disabled) {
                ...
                if (this.settings.backup.period && (!this.lastSave || now - this.lastSave > this.settings.backup.period)) {
                    this.lastSave = now;
                    const backFileName = this.backupDir + this.getTimeStr(now) + '_' + this.settings.fileDB.fileName + '.gz';
    ...
    publishAll(type, id, obj) {
        ...
        for (const i in clients) {
            if (clients.hasOwnProperty(i)) {
                this.publishToClients(clients[i], type, id, obj);
            }
        }
    ...
    destroy() {
        if (this.stateTimer) this.saveState();
    }
}
