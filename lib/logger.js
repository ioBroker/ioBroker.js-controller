...
let Seq;
try {
    Seq = require('@rubenbez/winston-seq').default;
} catch (ex) {
...
const IoSeq = Seq && class extends Seq {
    ...
    log(info, callback) {
        ...
        if (msgParts) {
            ioInfo.meta.instance = msgParts[1];
            ioInfo.meta.processid = msgParts[2];
            ioInfo.message = msgParts[3];
        } else {
            ioInfo.meta.instance = 'js-controller';
        }
        super.log(ioInfo, callback);
    }
};
