        ...
        if (userOptions.transport) {
            ...
            for (const f in userOptions.transport) {
                ...
                if (userOptions.transport[f].type === 'file' && userOptions.transport[f].enabled !== false) {
                    ...
                    userOptions.transport[f].auditFile = userOptions.transport[f].filename + '-audit.json';

                    userOptions.transport[f].createSymlink = (userOptions.transport[f].createSymlink !== undefined) ? userOptions.transport[f].createSymlink : true;
                    userOptions.transport[f].symlinkName = (userOptions.transport[f].symlinkName !== undefined) ? userOptions.transport[f].symlinkName : (path.basename(userOptions.transport[f].filename + '.current.log'));
                    ...
                    userOptions.transport[f].zippedArchive = true;
                    if (userOptions.transport[f].maxFiles === null && userOptions.maxDays) {
                        userOptions.transport[f].maxFiles = userOptions.maxDays + 'd';
                    }

                    const _log = new DailyRotateFile(userOptions.transport[f]);
                    _log.on('archive', gzName =>
                        _log.log({
                            level: os.platform().match(/^win/) ? 'info' : 'debug',
                            message: 'host.' + hostname + ' archived ' + gzName
                        })
                    );
