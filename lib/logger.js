...
const logger = function (level, files, noStdout, prefix) {
    ...
    if (typeof level === 'object') {
        ...
        let winstonFormat = winston.format.timestamp({ format: timestamp});
...
//                    userOptions.transport[f].timestamp   = timestamp; // TODO format.timestamp()
    ...
    const log = new winston.createLogger(options);
