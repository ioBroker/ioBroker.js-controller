                    ...
                    for (var i = 0; i < files.length; i++) {
                        ...
                        if (match) {
                            ...
                            if (date < for3days) {
                                // delete file
                                try {
                                    this.transports.ioBroker.log('info', 'host.' + hostname + ' Delete log file ' + files[i]);
									...
									this.log(os.platform().match(/^win/) ? 'info' : 'error', 'host.' + hostname + ' Cannot delete file "' + path.normalize(this.transports.ioBroker.dirname + '/' + files[i]) + '": ' + e);
                                }
                            }
                        }
                    }
