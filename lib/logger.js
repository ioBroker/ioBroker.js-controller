...
var logger = function (level, files, noStdout, prefix) {
    ...
    var isNpm = (__dirname.replace(/\\/g, '/').toLowerCase().indexOf('node_modules/' + tools.appName + '.js-controller') != -1);
        ...
        if (userOptions.transport) {
            ...
            for (var f in userOptions.transport) {
                if (userOptions.transport[f].type == 'file' && userOptions.transport[f].enabled !== false) {
                    ...
                    if (!fName) userOptions.transport[f].ioBrokerLog = true;

                    userOptions.transport[f].handleExceptions = false;
                    userOptions.transport[f].name        = !fName ? 'ioBroker' : 'dailyRotateFile' + fName;
                    fName++;
...
//                    userOptions.transport[f].maxsize  = (userOptions.transport[f].maxsize  !== undefined) ? userOptions.transport[f].maxsize : defaultMaxSize;
        ...
        for (var i = 0; i < files.length; i++) {
            var opt = {
                name:         !i ? 'ioBroker' : 'dailyRotateFile' + i,
    ...
    log.getFileName = function () {
        if (this.transports && this.transports.ioBroker) {
            return this.transports.ioBroker.dirname + '/' + this.transports.ioBroker._getFile();
        } else {
            return '';
        }
    };
    ...
    log.activateDateChecker = function (isEnabled, daysCount) {
        if (!isEnabled && this._fileChecker) {
            ...
            this._fileChecker = setInterval(function () {
                if (this.transports.ioBroker && fs.existsSync(this.transports.ioBroker.dirname)) {
                    var files = fs.readdirSync(this.transports.ioBroker.dirname);
                    ...
                    for (var i = 0; i < files.length; i++) {
                        ...
                        if (match) {
                            ...
                            if (date < for3days) {
                                // delete file
                                try {
                                    this.transports.ioBroker.log('info', 'host.' + os.hostname() + ' Delete log file ' + files[i]);
                                    fs.unlinkSync(this.transports.ioBroker.dirname + '/' + files[i]);
                                } catch (e) {
									// there is a bug under windows, that file stays opened and cannot be deleted
									this.log(os.platform().match(/^win/) ? 'info' : 'error', 'host.' + os.hostname() + ' Cannot delete file "' + path.normalize(this.transports.ioBroker.dirname + '/' + files[i]) + '": ' + e);
                                }
                            }
                        }
                    }
