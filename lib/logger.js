        ...
        if (userOptions.transport) {
            for (var f in userOptions.transport) {
                if (userOptions.transport[f].type == 'file' && userOptions.transport[f].enabled !== false) {
                    ...
                    userOptions.transport[f].name        = 'ioBroker';
...
//                    userOptions.transport[f].maxsize  = (userOptions.transport[f].maxsize  !== undefined) ? userOptions.transport[f].maxsize : defaultMaxSize;
                    ...
                    var _dailyRotateFile = winston.add(DailyRotateFile, userOptions.transport[f]);
                    ...
                    userOptions.transport[f].handleExceptions = userOptions.transport[f].handleExceptions || false;

                    var syslog = winston.add(SysLog, userOptions.transport[f]);
                    syslog.handleExceptions = function (message, info, next, err) {
                        console.error(message);
                    };
                }
            }
        ...
        for (var i = 0; i < files.length; i++) {
            var dailyRotateFile_ = winston.add(DailyRotateFile, {
                    name:         'ioBroker',
                ...
                dailyRotateFile_.handleExceptions = function (message, info, next, err) {
                    console.error(message);
                };

            if (!dailyRotateFile) dailyRotateFile = dailyRotateFile_;

            options.transports.push(dailyRotateFile_);
        }
    }
    ...
    for (var t = 0; t < options.transports.length; t++) {
        options.transports[t].handleExceptions = false;
    }
