...
const IoSeq = Seq && class extends Seq {
    ...
    log(level, msg, meta, callback) {
        ...
        if (typeof msg === 'function') { // Can this ever happen?
            callback = msg;
            meta = null;
            msg = '';
        }
        ...
        meta.hostname = tools.getHostName();
        const msgParts = msg.match(/^([^.]+\.[0-9]+) \(([^)]+)\) (.*)$/);
        if (msgParts) {
            meta.instance = msgParts[1];
            meta.processid = msgParts[2];
            msg = msgParts[3];
        } else {
            meta.instance = 'js-controller';
        }
            ...
            Object.keys(userOptions.transport).forEach(f => {
                ...
                if (transport.type === 'file' && transport.enabled !== false) {
                    ...
                    if (transport.serverUrl) {
                        transport.levelMapper = transport.levelMapper || function (level) {
                        ...
                        try {
                            options.transports.push(new IoSeq(transport));
                        } catch (err) {
