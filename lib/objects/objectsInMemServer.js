...
function ObjectsInMemServer(settings) {
    ...
    this.setDefaultAcl = function (acl, options, callback) {
        if (!options || !options.checked) {
            // default ACL is mapped by user rights to delete
            checkObjectRights(null, options, 'delete', function (err, options) {
                if (err) {
                    return callback(err);
                } else {
                    return that.setDefaultAcl.call(acl, options, callback);
                }
            }.bind(this));
        } else if (!acl) {
            return callback('Invalid parameter');
        } else if (JSON.stringify(acl) !== JSON.stringify(defaultNewAcl)) {
            defaultNewAcl = JSON.parse(JSON.stringify(acl));
            var count = 0;
            ...
            callback(null, count);
            if (objects['system.config'] && objects['system.config'].common) objects['system.config'].common.defaultNewAcl = JSON.parse(JSON.stringify(defaultNewAcl));
    ...
    function socketEvents(socket /*, user*/) {
        ...
        socket.on('setDefaultAcl', function (acl, options, callback) {
            that.setDefaultAcl.apply(that, arguments);
        });
