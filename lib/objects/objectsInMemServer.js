...
function ObjectsInMemServer(settings) {
        ...
        try {
            ...
            if (typeof callback === 'function') {
                if (fileOptions[id][name] !== null && fileOptions[id][name] !== undefined) {
                    ...
                    callback(utils.consts.ERROR_NOT_EXISTS);
                }
            }
        } catch (e) {
    ...
    function _unlink(id, name, options, callback) {
        try {
            ...
            if (fs.existsSync(objectsDir + id + '/' + name)) {
                ...
                if (typeof callback === 'function') {
                    setImmediate(() => callback(utils.consts.ERROR_NOT_EXISTS));
                }
            }
            // Store dir description
        ...
        checkFileRights(id, name, options, utils.consts.ACCESS_WRITE, (err, options) => {
            if (err) {
                ...
                if (!options.acl.file['delete']) {
                    if (typeof callback === 'function') callback(utils.consts.ERROR_PERMISSION);
                } else {
    ...
    function _rename(id, oldName, newName, options, callback) {
        try {
            ...
            if (fs.existsSync(objectsDir + id + '/' + oldName)) {
                ...
                if (typeof callback === 'function') callback(utils.consts.ERROR_NOT_EXISTS);
            }
        } catch (e) {
            if (typeof callback === 'function') callback(e.message);
        }
    ...
    this.subscribeConfig = (pattern, options, callback) => {
        let socket = this;
    ...
    function _getObjects(keys, options, callback, dontModify) {
        ...
        for (let i = 0; i < keys.length; i++) {
            if (utils.checkObject(objects[keys[i]], options, utils.consts.ACCESS_READ)) {
                ...
                result.push({error: utils.consts.ERROR_PERMISSION});
            }
        }
        if (typeof callback === 'function') {
            setImmediate(callback, null, result);
    ...
    function _initWebServer(settings, server) {
        ...
        try {
            if (settings.secure) {
                ...
                server.server = require('https').createServer(settings.certificates, (req, res) => {
                    res.writeHead(501);
                    res.end('Not Implemented');
                });
            } else {
                server.server = require('http').createServer((req, res) => {
                    res.writeHead(501);
                    res.end('Not Implemented');
                });
            }
