...
function ObjectsInMemServer(settings) {
    ...
    this.enableFileChache = function (enabled, options, callback) {
        ...
        if (!options || !options.checked) {
            checkObjectRights(null, options, 0x2/*write*/, function (err, options) {
                if (err) {
                    ...
                    return this.enableFileChache(enabled, options, callback);
                }
            }.bind(this));
            return;
        }
...
/*                    state: {
    ...
    this.writeFile = function (id, name, data, options, callback) {
        ...
        try {
            ...
            try {
                ...
                log.error('Check the permissions! Or call "sudo chmod 777 * -R" in ' + tools.appName +' dir');
        ...
        for (var j = 0; j < _files.length; j++) {
            ...
            if (fs.existsSync(objectsDir + id + '/' + name + _files[j])) {
                ...
                try {
                    ...
                    log.error('Cannot read permssions of  ' + objectsDir + id + '/' + name + _files[j] + ': ' + e);
                }
    ...
    this._getAllFilesInDir = function (id, name, options, callback, result) {
        result = result || [];
        this.readDir(id, name, options, function (err, files) {
            var count = 0;
            if (result) {
                for (var f = 0; f < files.length; f++) {
                    if (files[f].isDir) {
                        count++;
                        that._getAllFilesInDir(id, name + '/' + files[f].file, options, function (err, _result) {
                            if (err) log.error('_getAllFilesInDir: ' + err);
                            if (!--count) callback(null, _result);
                        }, result);
                    } else {
    ...
    this._addFile = function (id, name, options, zip, callback) {
        this.readFile(id, name, options, function (err, data, mime) {
    ...
    this.readDirAsZip = function (id, name, options, callback) {
        ...
        this._getAllFilesInDir(id, name, options, function (err, files) {
            if (err) log.error('getDirAsZip: ' + err);
            var count = 0;
            if (files) {
                var zip = new AdmZip();
                for (var f = 0; f < files.length; f++) {
                    count++;
                    that._addFile(id, files[f], options, zip, function (err) {
                        if (!--count) callback(err, zip.toBuffer());
                        zip = null;
                    });
                }
            }
    ...
    function socketEvents(socket, user) {
        ...
        socket.on('enableFileChache', function (enabled, options, callback) {
            that.enableFileChache.apply(that, arguments);
        });
        ...
        socket.on('readDirAsZip', function (id, name, options, callback) {
            that.readDirAsZip.apply(that, arguments);
        });

        socket.on('writeDirAsZip', function (id, name, options, data, callback) {
            that.writeDirAsZip.apply(that, arguments);
        });
