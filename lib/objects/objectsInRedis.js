...
function ObjectsInMem(settings) {
    ...
    this.setObject = function (id, obj, options, callback) {
        ...
        if (that.objects && that.objects[id] && that.objects[id].common) {
            for (var i = 0; i < preserveSettings.length; i++) {
                ...
                if (that.objects[id].common[preserveSettings[i]] && (!obj.common || obj.common[preserveSettings[i]] === undefined)) {
                    if (!obj.common) obj.common = {};
                    obj.common[preserveSettings[i]] = that.objects[id].common[preserveSettings[i]];
                }
            }
        }

        log.debug('objectsInRedis setObject ' + id);
        if (that.objects) that.objects[id] = clone(obj);
    ...
    this.extendObject = function (id, obj, options, callback) {
        ...
        this.getObject(id, function (err, data) {
            that.objects[id] = extend(true, data || {}, obj);
            that.objects[id]._id = id;
