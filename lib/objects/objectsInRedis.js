...
const StatesRedis = require(__dirname + '/../states/statesInRedis.js');
...
function ObjectsInRedis(settings) {
    ...
    // -------------- OBJECT FUNCTIONS -------------------------------------------
    function checkObject(id, obj, options, flag) {
        // TODO: we must read from redis the object at this place
    ...
    function checkObjectRights(id, options, flag, callback) {
        ...
        // if object not yet read from redis
        if (id) {
            if (objects && objects[id]) {
                if (!checkObject(id, objects[id], options, flag)) {
                    return callback('permissionError', options);
                } else {
                    callback(null, options, objects[id]);
                }
            } else {
                return states.getConfig(id, function (err, obj) {
                    if (!checkObject(id, obj, options, flag)) {
                        return callback('permissionError', options);
                    } else {
                        callback(null, options, obj);
                    }
                });
            }
        }
    ...
    this.getConfigKeys = function (pattern, options, callback, dontModify) {
        ...
        if (objects) {
            for (let id in objects) {
                ...
                if (r.test(id) && checkObject(id, objects[id], options, 'list')) {
                    result.push(id);
                }
            }
        }
    ...
    this.getObjects = function (keys, options, callback, dontModify) {
        ...
        for (let i = 0; i < keys.length; i++) {
            if (checkObject(keys[i], objects[keys[i]], options, 4 /*read*/)) {
    ...
    this.setObject = function (id, obj, options, callback) {
        ...
        if (!options || !options.checked) {
            checkObjectRights(id, options, 0x2/*write*/, function (err, options, oldObj) {
                options.oldObj = oldObj;
                if (err) {
                    if (typeof callback === 'function') callback(err);
                } else {
        ...
        let oldObj = options.oldObj;
        if (!oldObj || (objects && objects[id])) {
            oldObj = objects[id];
        }
        // read object
        if (!oldObj) {
            return states.getConfig(id, function (err, oldObj) {
                options.oldObj = oldObj || true;
        ...
        if (oldObj && typeof oldObj === 'object' && oldObj.common) {
        ...
        if (defaultNewAcl && (!oldObj || typeof oldObj !== 'object' || !oldObj.acl)) {
        ...
        if (id === 'system.config' && obj && obj.common && oldObj && typeof oldObj === 'object' && oldObj.common && JSON.stringify(obj.common.defaultNewAcl) !== JSON.stringify(oldObj.common.defaultNewAcl)) {
            if (objects) {
                objects[id] = obj;
            }
        ...
        if (oldObj && typeof oldObj === 'object') {
            tools.checkNonEditable(oldObj, obj);
        }

        log.silly('objectsInRedis setObject ' + id);
        if (objects) {
            objects[id] = clone(obj);
        }
    ...
    this.delObject = function (id, options, callback) {
        ...
        if (!options || !options.checked) {
            checkObjectRights(id, options, 'delete', function (err, options, obj) {
                options.oldObj = obj;
                if (err) {
                    if (typeof callback === 'function') callback(err);
                } else {
        ...
        if (oldObj && typeof oldObj === 'object' && oldObj.common && oldObj.common.dontDelete) {
        ...
        if (objects['_design/' + design]) {
            ...
            states.getConfig('_design/' + design, function (err, obj) {
                ...
                if (obj && obj.views && obj.views[search]) {
                    objects[obj._id] = obj;
                    that._applyView(obj.views[search], params, options, callback);
                } else {
    ...
    this.getObjectList = function (params, options, callback) {
        ...
        for (let id in objects) {
            if (!checkObject(id, objects[id], options, 'read')) continue;
            if (params) {
                if (params.startkey && id < params.startkey) continue;
                if (params.endkey   && id > params.endkey)   continue;
    ...
    this.extendObject = function (id, obj, options, callback) {
        ...
        this.getObject(id, function (err, data) {
            ...
            that.setObject(id, newObj, function (err, res) {
    ...
    this.findObject = function (idOrName, type, options, callback) {
        ...
        if (objects[idOrName] && (!type || (objects[idOrName].common && objects[idOrName].common.type === type))) {
            ...
            // Assume it is name
            for (let id in objects) {
                if (!checkObject(id, objects[id], options, 4/*read*/)) continue;
