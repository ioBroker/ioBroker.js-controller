...
class ObjectsInMemoryFileDB extends InMemoryFileDB {
    ...
    saveFileSettings(id, force) {
        ...
        id !== undefined && this.writeIds.indexOf(id) === -1 && this.writeIds.push(id);
    ...
    _setObject(id, obj, options, callback) {
        ...
        if (id === 'system.config' &&
            ...
            JSON.stringify(obj.common.defaultNewAcl) !== JSON.stringify(this.dataset[id].common.defaultNewAcl)) {
            this.dataset[id] = obj;
    ...
    _setObjectDirect(id, obj, callback) {
        ...
        if (this.isEquivalentValue(obj, this.dataset[id], 'ts')) { // check if object has changed, ignore field ts
    ...
    _extendObject(id, obj, options, callback) {
        ...
        let oldObj = JSON.parse(JSON.stringify(this.dataset[id]));
        ...
        if (!this.isEquivalentValue(oldObj, this.dataset[id], 'ts')) { // check if object has changed, ignore field ts
    ...
    isEquivalentValue(a, b, ignoreField) {
        ...
        const lengthModA = a[ignoreField] !== undefined ? 1 : 0;
        const lengthModB = b[ignoreField] !== undefined ? 1 : 0;

        // If number of properties is different,
        // objects are not equivalent
        if ((aProps.length - lengthModA) !== (bProps.length - lengthModB)) {
        ...
        for (let i = 0; i < aProps.length; i++) {
            ...
            if (propName === ignoreField) continue;
            ...
            if (typeof a[propName] === 'object') {
                if (!this.isEquivalentValue(a[propName], b[propName])) {
                    return false;
                }
            }
            else {
