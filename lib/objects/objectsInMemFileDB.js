...
class ObjectsInMemoryFileDB extends InMemoryFileDB {
    ...
    constructor(settings) {
        ...
        this.objectsDir = this.dataDir + 'files/';
    ...
    saveFileSettings(id, force) {
        ...
        if (force) {
            ...
            for (let _id = 0; _id < this.writeIds.length; _id++) {
                try {
                    fs.writeFileSync(this.objectsDir + this.writeIds[_id] + '/_data.json', JSON.stringify(this.fileOptions[this.writeIds[_id]]));
                } catch (e) {
                    this.log.error(this.namespace + ' Cannot write files: ' + this.objectsDir + this.writeIds[_id] + '/_data.json: ' + e.message);
                }
            }
            this.writeIds = [];
        } else {
            this.writeTimer = setTimeout(() => {
                ...
                for (let id = 0; id < this.writeIds.length; id++) {
                    try {
                        fs.writeFileSync(this.objectsDir + this.writeIds[id] + '/_data.json', JSON.stringify(this.fileOptions[this.writeIds[id]]));
                    } catch (e) {
                        this.log.error(this.namespace + ' Cannot write files: ' + this.objectsDir + this.writeIds[id] + '/_data.json: ' + e.message);
                    }
                }
                this.writeIds = [];
            }, 1000);
    ...
    checkFile(id, name, options, flag, callback) {
        const acl = (this.fileOptions[id] && this.fileOptions[id][name]) || {};
    ...
    _writeFile(id, name, data, options, callback) {
        try {
            try {
                ...
                if (!fs.existsSync(this.objectsDir + id)) fs.mkdirSync(this.objectsDir + id);
            } catch (e) {
                this.log.error(this.namespace + ' Cannot create directories: ' + this.objectsDir + id + ': ' + e.message);
            ...
            if (!this.fileOptions[id][name]) {
                this.fileOptions[id][name] = {createdAt: Date.now()};
            }
            if (!this.fileOptions[id][name].acl) {
            ...
            try {
                ...
                this.mkpathSync(this.objectsDir + id + '/', name);
                // Store file
                fs.writeFileSync(this.objectsDir + id + '/' + name, data, {'flag': 'w', 'encoding': isBinary ? 'binary' : 'utf8'});
                // Store dir description
                this.saveFileSettings(id);
            } catch (e) {
                this.log.error(this.namespace + ' Cannot write files: ' + this.objectsDir + id + '/' + name + ': ' + e.message);
                if (typeof callback === 'function') callback(e.message);
                return;
            }
    ...
    writeFile(id, name, data, options, callback) {
        ...
        try {
            if (!this.fileOptions[id]) {
                if (fs.existsSync(this.objectsDir + id + '/_data.json')) {
                    try {
                        this.fileOptions[id] = JSON.parse(fs.readFileSync(this.objectsDir + id + '/_data.json', 'utf8'));
                    } catch (e) {
                        this.log.error(this.namespace + ' Cannot parse ' + this.objectsDir + id + '/_data.json: ' + e);
                    }
                } else {
                    this.fileOptions[id] = {};
                }
    ...
    _readFile(id, name, options, callback) {
        try {
            if (!this.fileOptions[id]) {
                if (fs.existsSync(this.objectsDir + id + '/_data.json')) {
                    try {
                        this.fileOptions[id] = JSON.parse(fs.readFileSync(this.objectsDir + id + '/_data.json', 'binary'));
                    } catch (e) {
                        this.log.error(this.namespace + ' Cannot parse ' + this.objectsDir + id + '/_data.json: ' + e);
                        this.fileOptions[id] = {};
                    }
            ...
            if (!this.files[id][name] || this.settings.connection.noFileCache || options.noFileCache) {
                if (fs.existsSync(this.objectsDir + id + '/' + name)) {
                    ...
                    this.files[id][name] = fs.readFileSync(this.objectsDir + id + '/' + name);
            ...
            if (typeof callback === 'function') {
                if (this.fileOptions[id][name] !== null && this.fileOptions[id][name] !== undefined) {
                    ...
                    callback(null, this.files[id][name], this.fileOptions[id][name].mimeType);
                } else {
    ...
    _unlink(id, name, options, callback) {
        try {
            ...
            if (fs.existsSync(this.objectsDir + id + '/' + name)) {
                const stat = fs.statSync(this.objectsDir + id + '/' + name);

                if (stat.isDirectory()) {
                    // read all entries and delete every one
                    const fdir = fs.readdirSync(this.objectsDir + id + '/' + name);
                    let cnt = 0;
                    for (let f = 0; f < fdir.length; f++) {
                        cnt++;
                        this.unlink(id, name + '/' + fdir[f], options, err => {
                            if (!--cnt) {
                                this.log.debug('Delete directory ' + id + '/' + name);
                                try {
                                    fs.rmdirSync(this.objectsDir + id + '/' + name);
                                } catch (e) {
                                    this.log.error('Cannot delete directory "' + id + '/' + name + '": ' + e);
                                }
                                if (typeof callback === 'function') {
                    ...
                    this.log.debug('Delete file ' + id + '/' + name);
                    try {
                        fs.unlinkSync(this.objectsDir + id + '/' + name);
                    } catch (e) {
                        this.log.error('Cannot delete file "' + id + '/' + name + '": ' + e);
                    }
                    if (typeof callback === 'function') {
                        setImmediate(() => callback());
                    }
    ...
    _readDir(id, name, options, callback) {
        ...
        if (fs.existsSync(this.objectsDir + id + '/' + name)) {
            try {
                const dirFiles = fs.readdirSync(this.objectsDir + id + '/' + name);
        ...
        for (let j = 0; j < _files.length; j++) {
            ...
            if (fs.existsSync(this.objectsDir + id + '/' + name + _files[j])) {
                const stats = fs.statSync(this.objectsDir + id + '/' + name + _files[j]);
                ...
                try {
                    ...
                    this.log.error(this.namespace + ' Cannot read permssions of  ' + this.objectsDir + id + '/' + name + _files[j] + ': ' + e);
                }
    ...
    _rename(id, oldName, newName, _options, callback) {
        try {
            ...
            if (this.fileOptions[id][oldName]) {
                const type = this.fileOptions[id][oldName];
                delete this.fileOptions[id][oldName];
                this.fileOptions[id][newName] = type;
                fs.writeFileSync(this.objectsDir + id + '/_data.json', JSON.stringify(this.fileOptions[id]));
            }
            if (this.files[id] && this.files[id][oldName]) {
                const data = this.files[id][oldName];
                delete this.files[id][oldName];
                this.files[id][newName] = data;
            }
            if (fs.existsSync(this.objectsDir + id + '/' + oldName)) {
                fs.renameSync(this.objectsDir + id + '/' + oldName, this.objectsDir + id + '/' + newName);
                if (typeof callback === 'function') callback();
            } else {
        ...
        try {
            ...
            for (const f in this.fileOptions[id]) {
                ...
                if (regEx.test(f) && utils.checkFile(this.fileOptions[id][f], options, utils.CONSTS.ACCESS_WRITE)) {
                    ...
                    if (fs.existsSync(this.objectsDir + id + '/' + f)) {
                        ...
                        const stats = fs.statSync(this.objectsDir + id + '/' + f);
                        const parts = f.split('/');
            ...
            // Store dir description
            if (changed) fs.writeFileSync(this.objectsDir + id + '/_data.json', JSON.stringify(this.fileOptions[id]));
        ...
        try {
            ...
            for (const f in this.fileOptions[id]) {
                ...
                if (regEx.test(f) && utils.checkFile(this.fileOptions[id][f], options, utils.CONSTS.ACCESS_WRITE)) {
                    ...
                    if (fs.existsSync(this.objectsDir + id + '/' + f)) {
                        stat = fs.statSync(this.objectsDir + id + '/' + f);

                        if (stat.isDirectory()) {
                            if (dirs.indexOf(f) === -1) dirs.push(f);
                        } else {
                            fs.unlinkSync(this.objectsDir + id + '/' + f);
                        }
                    }
                    const parts = f.split('/');
                    const fileName = parts.pop();
            ...
            for (let d = 0; d < dirs.length; d++) {
                try {
                    const _files = fs.readdirSync(this.objectsDir + id + '/' + dirs[d]);

                    if (_files.length) {
                        console.this.log('Directory ' + id + '/' + dirs[d] + ' is not empty');
                    } else {
                        fs.rmdirSync(this.objectsDir + id + '/' + dirs[d]);
                    }
                } catch (e) {
    ...
    _mkdir(id, dirname, _options, callback) {
        try {
            ...
            if (!fs.existsSync(this.objectsDir + id + '/' + dirname)) {
                fs.mkdirSync(this.objectsDir + id + '/' + dirname);
                if (typeof callback === 'function') callback();
            } else {
    ...
    _enableFileCache(enabled, _options, callback) {
        ...
        if (typeof callback === 'function') {
            setImmediate(() => callback(null, this.settings.connection.noFileCache));
        }
    }
    ...
    _setObject(id, obj, options, callback) {
        ...
        this.publishAll('objects', id, obj);
        if (typeof callback === 'function') {
            setImmediate(callback, null, {id: id});
        }
        if (!this.stateTimer) {
            this.stateTimer = setTimeout(() => this.saveState, 5000);
        }
    }
    ...
    _delObject(id, _options, callback) {
        if (this.dataset[id]) {
            ...
            this.publishAll('objects', id, null);
            if (typeof callback === 'function') {
        ...
        for (const id in this.dataset) {
            ...
            if (this.dataset[id]) {
                ...
                try {
                    ...
                    console.this.log('Cannot execute map: ' + e.message);
    ...
    _getObjectView(design, search, params, options, callback) {
        if (this.dataset['_design/' + design]) {
            if (this.dataset['_design/' + design].views && this.dataset['_design/' + design].views[search]) {
                ...
                console.this.log('Cannot find search "' + search + '" in "' + design + '"');
                callback({status_code: 404, status_text: 'Cannot find search "' + search + '" in "' + design + '"'});
            }
        } else {
            console.this.log('Cannot find view "' + design + '"');
    ...
    _extendObject(id, obj, options, callback) {
        ...
        this.publishAll('objects', id, this.dataset[id]);

        if (typeof callback === 'function') {
            setImmediate(() => callback(null, {id: id, value: this.dataset[id]}, id));
        }
    ...
    _findObject(idOrName, type, options, callback) {
        ...
        if (this.dataset[idOrName] && (!type || (this.dataset[idOrName].common && this.dataset[idOrName].common.type === type))) {
            callback(null, idOrName, this.dataset[idOrName].common.name);
        } else {
            // Assume it is name
            for (const id in this.dataset) {
                if (!this.dataset.hasOwnProperty(id) || !utils.checkObject(this.dataset[id], options, utils.CONSTS.ACCESS_READ)) continue;
