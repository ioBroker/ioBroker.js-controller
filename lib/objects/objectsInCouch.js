...
function ObjectsCouch(settings) {
    ...
    var nano = require('nano')({
        ...
        log: function (id, args) {
            log.debug(settings.namespace + ' ' + id + ' ' + JSON.stringify(args));
        }
    });
    var couch;
    var change;
    ...
    var __construct = (function () {
        ...
        db = tools.appName + (settings.namespace ? '_' + settings.namespace : '');

        change = settings.change || function (id, obj) {
            log.debug(settings.namespace + ' couchdb change: ' + id + ' ' + JSON.stringify(change));
        };
    ...
    this.subscribe = function (pattern) {
        ...
        log.debug(settings.namespace + ' couchdb follow ' + pattern);
        feed.follow();
    };
    ...
    this.getObjectView = function (design, search, params, callback) {
        couch.view(design, search, params, function (err, doc) {
            if (err) {
                log.debug(settings.namespace + ' couchdb getObjectView ' + design + ' ' + search + ' ' + err);
            }
            callback(err, doc);
        });
    };
    ...
    this.getObjectList = function (params, callback) {
        couch.list(params, function (err, doc) {
            if (err) {
                log.debug(settings.namespace + ' couchdb getObjectList ' + err);
            }
            callback(err, doc);
        });
    };
    ...
    this.setObject = function (id, obj, callback) {
        ...
        log.debug(settings.namespace + ' couchdb setObject ' + id);
    ...
    this.getObject = function (id, callback) {
        log.debug(settings.namespace + ' couchdb getObject ' + id);
    ...
    function dbConnect(database, create, callback) {
        nano.db.get(database, function (err, body) {
            if (!err) {
                ...
                log.debug(settings.namespace + ' couchdb using db ' + database);
                ...
                if (err.status_code) {
                    switch (err.status_code) {
                        ...
                        case 404:
                            log.info(settings.namespace + ' couchdb ' + database + ' doesn\'t exist');
                        ...
                        default:
                            log.error(settings.namespace + ' couchdb' + err);
                            break;
                    }
                } else if (err.code) {
                    switch (err.code) {
                        case 'ECONNREFUSED':
                            log.error(settings.namespace + ' couchdb connection refused');
                            break;

                        default:
                            log.error(settings.namespace + ' couchdb connection error: ' + err.code);
    ...
    function dbCreate(name, callback) {
        nano.db.create(name, function (err, body) {
            if (!err) {
                log.info(settings.namespace + ' couchdb database ' + name + ' created!');
                if (callback) callback();
            } else {
                log.error(settings.namespace + ' couchdb error while creating db ' + name);
            }
        });
    }
