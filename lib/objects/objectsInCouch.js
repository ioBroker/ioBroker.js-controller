...
function ObjectsCouch(settings) {
    ...
    var nano = require('nano')({
        ...
        log: function (id, args) {
            log.silly(settings.namespace + ' ' + id + ' ' + JSON.stringify(args));
        }
    });
    var couch;
    var change;
    ...
    var __construct = (function () {
        ...
        change = settings.change || function (id, obj) {
            log.silly(settings.namespace + ' couchdb change: ' + id + ' ' + JSON.stringify(change));
        };
    ...
    this.subscribe = function (pattern) {
        ...
        log.silly(settings.namespace + ' couchdb follow ' + pattern);
        feed.follow();
    };
    ...
    this.getObjectView = function (design, search, params, callback) {
        couch.view(design, search, params, function (err, doc) {
            if (err) {
                log.silly(settings.namespace + ' couchdb getObjectView ' + design + ' ' + search + ' ' + err);
            }
            callback(err, doc);
        });
    };
    ...
    this.getObjectList = function (params, callback) {
        couch.list(params, function (err, doc) {
            if (err) {
                log.silly(settings.namespace + ' couchdb getObjectList ' + err);
            }
            callback(err, doc);
        });
    };
    ...
    this.setObject = function (id, obj, callback) {
        ...
        log.silly(settings.namespace + ' couchdb setObject ' + id);
    ...
    this.getObject = function (id, callback) {
        log.silly(settings.namespace + ' couchdb getObject ' + id);
    ...
    function dbConnect(database, create, callback) {
        nano.db.get(database, function (err, body) {
            if (!err) {
                ...
                log.silly(settings.namespace + ' couchdb using db ' + database);
