...
class ObjectsInMemoryServer extends ObjectsInMemoryFileDB {
    ...
    _normalizeId(idWithNamespace) {
        ...
        return {id: id, namespace: ns, name: name, isMeta: isMeta};
    }
    ...
    _socketEvents(handler) {
        ...
        handler.on('script', (data, responseId) => {
            ...
            if (data[0] === 'exists') {
                ...
                const scriptDesign = data[1].match(/^-- design: ([a-z0-9A-Z-.]+)\s/);
                if (scriptDesign && scriptDesign[1]) {
                    const design = scriptDesign[1];
                    let search = null;
                    const scriptSearch = data[1].match(/^-- search: ([a-z0-9A-Z-.]*)\s/);
                    ...
                    this.log.silly('Register LUA Script: ' + scriptChecksum + ' = ' + JSON.stringify(this.knownScripts[scriptChecksum]));
        ...
        handler.on('evalsha', (data, responseId) => {
            ...
            if (data[2] === this.namespaceObj && data.length > 4) {
                ...
                this.log.silly('Script converted to getObjectView: design=' + scriptDesign + ', search=' + scriptSearch);
                this.getObjectView(scriptDesign, scriptSearch, {startkey: data[3], endkey: data[4], include_docs: true}, (err, objs) => {
                    if (err) {
        ...
        handler.on('mget', (data, responseId) => {
            ...
            if (namespace === this.namespaceObj) {
                ...
                data.forEach((dataId) => {
                    ...
                    if (namespace !== this.namespaceObj) {
                        keys.push(null);
                        this.log.warn('Go MEGT request for non Object-ID in Objects-ID chunk for ' + namespace + ' / ' + dataId);
                        return;
                    }
                    keys.push(id);
                });
                ...
                if (isMeta) {
                    ...
                    data.forEach((dataId) => {
                        ...
                        if (namespace !== this.namespaceFile) {
                            response.push(null);
                            this.log.warn('Go MEGT request for non File ID in File-ID chunk for ' + dataId);
                            return;
                        }
        ...
        handler.on('error', err => this.log.error(this.namespace + ' Redis states: ' + err));
    }
     ...
     */
    _initSocket(socket) {
        this.log.silly('Handling new Redis Objects connection');
        const options = {
            log: this.log,
            logScope: 'Objects',
    ...
    _initRedisServer(settings, server) {
        ...
        this.log.info(this.namespace + ' ' + (settings.secure ? 'Secure ' : '') + ' Redis inMem-objects listening on port ' + (settings.port || 9001));
    }
}
