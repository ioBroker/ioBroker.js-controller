...
class ObjectsInMemoryServer extends ObjectsInMemoryFileDB {
        ...
        handler.on('script', (data, responseId) => {
            ...
            if (data[0] === 'exists') {
                ...
                if (scriptDesign && scriptDesign[1]) {
                    ...
                    if (this.settings.connection.enhancedLogging) this.log.silly(this.namespace + ' Register View LUA Script: ' + scriptChecksum + ' = ' + JSON.stringify(this.knownScripts[scriptChecksum]));
                    ...
                    if (this.settings.connection.enhancedLogging) this.log.silly(this.namespace + ' Register Func LUA Script: ' + scriptChecksum + ' = ' + JSON.stringify(this.knownScripts[scriptChecksum]));
                    handler.sendBulk(responseId, scriptChecksum);
                } else {
        ...
        handler.on('evalsha', (data, responseId) => {
            ...
            if (this.knownScripts[data[0]].design) {
                ...
                if (data[2] === this.namespaceObj && data.length > 4) {
                    ...
                    if (this.settings.connection.enhancedLogging) this.log.silly(this.namespace + ' Script transformed into getObjectView: design=' + scriptDesign + ', search=' + scriptSearch);
                ...
                if (this.settings.connection.enhancedLogging) this.log.silly(this.namespace + ' Script transformed into _applyView: func=' + scriptFunc.map);
            ...
            if (namespace === this.namespaceObj) {
                ...
                if (isMeta) {
                    ...
                    try {
                        ...
                        handler.sendError(responseId, err);
                        return;
                    }
                    handler.sendString(responseId, 'OK');
                }
        ...
        handler.on('rename', (data, responseId) => {
            ...
            if (oldDetails.namespace === this.namespaceFile) {
                if (oldDetails.id !== newDetails.id) {
                    return handler.sendError(responseId, new Error('id needs to stay the same'));
                }
        ...
        handler.on('keys', (data, responseId) => {
            ...
            if (namespace === this.namespaceObj) {
                this.getKeys(id, (err, result) => {
                    if (err || !result) {
                        return handler.sendError(responseId, new Error('ERROR ' + err)); // TODO
                    }
                    for (let i = 0; i < result.length; i++) {
     ...
     */
    _initSocket(socket) {
        if (this.settings.connection.enhancedLogging) this.log.silly(this.namespace + ' Handling new Redis Objects connection');
        const options = {
            log: this.log,
