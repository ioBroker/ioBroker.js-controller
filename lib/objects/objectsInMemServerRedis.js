...
class ObjectsInMemoryServer extends ObjectsInMemoryFileDB {
    ...
    _socketEvents(handler) {
        ...
        handler.on('exists', async (data, responseId) => {
            ...
            if (namespace === this.namespaceObj) {
                try {
                    ...
                    handler.sendInteger(responseId, obj ? 1 : 0);
                } catch (e) {
        ...
        handler.on('scan', (data, responseId) => {
            ...
            return _handleScanOrKeys(data[2], responseId, true);
        });
        ...
        handler.on('keys', (data, responseId) => {
            ...
            return _handleScanOrKeys(data[0], responseId);
        });
         ...
         * @private
         */
        const _handleScanOrKeys = (pattern, responseId, isScan= false) => {
            ...
            if (namespace === this.namespaceObj) {
                ...
                handler.sendError(responseId, new Error(`KEYS/SCAN-UNSUPPORTED for namespace ${namespace}: Pattern=${pattern}`));
            }
        };
