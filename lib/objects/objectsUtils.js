...
function checkFileRights(objects, id, name, options, flag, callback) {
    ...
    if (!options.acl) {
        objects.getUserGroup(options.user, (user, groups, acl) => {
            options.acl    = acl || {};
            options.groups = groups;
            options.group  = groups ? groups[0] : null; // todo it could lead to problems if e.g. admin groups will come after user groups
...
function getDefaultAdminRights(acl, isState) {
    acl = acl || {};
    acl.file = {
        list:     true,
        read:     true,
...
function getUserGroup(objects, user, callback) {
    ...
    if (users[user]) {
        return callback.call(this, null, user, users[user].groups, users[user].acl);
    }
