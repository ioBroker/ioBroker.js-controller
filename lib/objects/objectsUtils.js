...
function checkFileRights(objects, id, name, options, flag, callback) {
    ...
    objects.checkFile(id, name, options, flag, (err, options, opt) => {
        if (err) {
            ...
            return tools.maybeCallbackWithError(callback, null, opt);
        }
    });
...
function getUserGroup(objects, user, callback) {
    if (!user || typeof user !== 'string' || !user.match(regUser)) {
        ...
        // deep copy
        return tools.maybeCallbackWithError(callback, objects, `invalid user name: ${user}`, user, [], deepClone(defaultAcl.acl));
    }
    if (users[user]) {
        return tools.maybeCallbackWithError(callback, objects, null, user, users[user].groups, users[user].acl);
    }
...
function checkObjectRights(objects, id, object, options, flag, callback) {
    ...
    if (id && !checkObject(object, options, flag)) {
        tools.maybeCallbackWithError(callback, ERROR_PERMISSION, options);
    } else {
        tools.maybeCallbackWithError(callback, null, options);
    }
}
