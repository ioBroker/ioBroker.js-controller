...
function ObjectsInMemClient(settings) {
    ...
    if (!log) {
        log = {
            silly: _msg => {/*console.log(msg);*/},
            debug: _msg => {/*console.log(msg);*/},
            info:  _msg => {/*console.log(msg);*/},
    ...
    (function __construct () {
        ...
        if (typeof settings.change === 'function') {
            client.on('message', (pattern, channel, message) => {
    ...
    this.subscribe = function (pattern, options, callback) {
        ...
        if (!client) return;
        client.emit('subscribe', pattern, options, callback);
    };

    this.unsubscribe = function (pattern, options, callback) {
        if (typeof options === 'function') {
            callback = options;
            options = null;
        }
        const pos = subscribes.indexOf(pattern);
        if (pos !== -1) subscribes.splice(pos, 1);

        if (!client) return;
        client.emit('unsubscribe', pattern, options, callback);
    };
