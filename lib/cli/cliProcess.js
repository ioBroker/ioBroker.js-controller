...
const { getObjectFrom, getInstanceName, normalizeAdapterName } = require('./cliTools.js');
    ...
    setAllAdaptersEnabled(enabled) {
        ...
        dbConnect((objects) => {
            objects.getObjectList({ startkey: 'system.adapter.', endkey: 'system.adapter.\u9999' }, (err, objs) => {
                // Find all instance objects
                const adapterInstances = objs.rows
                    .filter(row => row.value.type === 'instance')
                    .map(row => row.value)
                    ;
                // Create a promise for each. setInstanceEnabled only starts/stops when necessary
                const instancePromises = adapterInstances
                    .filter(obj => !obj.common.enabled)
                    .map(obj => setInstanceEnabled(objects, obj, enabled))
                    ;
                // Now wait for all instances to be started/stopped
    ...
    setAdapterEnabled(adapter, enabled, restartIfRunning) {
        ...
        dbConnect((objects) => {
            objects.getObjectList({ startkey: `system.adapter.${adapter}.`, endkey: `system.adapter.${adapter}.\u9999` }, (err, objs) => {
                ...
                setInstanceEnabled(objects, instance, enabled, restartIfRunning)
                    .then(() => callback());
            });
        });
    }
        ...
        if (!require('os').platform().match(/^win/)) {
            daemon.on('stopped', () => {
                ...
                if (fs.existsSync(killAllScriptPath)) {
                    ...
                    let child = require('child_process').spawn(killAllScriptPath, []);
