...
async function enumInstances(objects, adapter) {
    ...
    // filter with dot at the end to avoid returning other adapters starting with desired adapters name
    return data.filter(it => it._id.startsWith(`system.adapter.${adapter}.`));
}
