...
module.exports = class CLIHost extends CLICommand {
    ...
    remove(args) {
        ...
        dbConnect((objects, states, isOffline) => {
            tools.poorMansAsync(function* () {
                ...
                // Delete it
                try {
                    yield objects.delObjectAsync(hostToDelete._id);
                } catch (err) {
                    CLI.error.cannotDeleteObject(hostToDelete._id, err);
                    return void callback(30);
                }

                // Also delete its states
                const hostStates = yield enumObjects(objects, 'state', `system.host.${hostname}.`);
                ...
                for (const instance of instancesToRename) {
                    yield this.changeInstanceHost(objects, instance, newHostname);
                }
        ...
        dbConnect((objects, states, isOffline) => {
            tools.poorMansAsync(function* () {
                ...
                if (configData.system && configData.system.hostname !== newHostname) {
                    ...
                    fs.writeFileSync(configFileName, JSON.stringify(configData), 'utf8');
                }
     ...
     */
    changeInstanceHost(objects, instance, newHostname) {
