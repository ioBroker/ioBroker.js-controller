...
const { enumHosts, enumObjects, getObjectFrom } = require('./cliTools');
        ...
        dbConnect((objects, states, isOffline) => {
            tools.poorMansAsync(function* () {
                ...
                for (const host of hosts) {
                    ...
                    // TODO: Rename only the hostname we're supposed to rename!
                    // TODO: Also rename all instances!
                    ...
                    // Rename its states
                    const hostStates = yield enumObjects(objects, 'state', `system.host.${oldHostname}.`);
                    for (const state of hostStates) {
                        ...
                        Object.assign(state, {
                            _id: state._id.replace(`system.host.${oldHostname}`, `system.host.${newHostname}`),
                            from: getObjectFrom(),
                            ts: Date.now()
                        });
                        // And save it
