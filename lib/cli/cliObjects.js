...
module.exports = class CLIObjects extends CLICommand {
    ...
    constructor(options) {
        super(options);
        // Ensure the required options were given
        this.requireOption('objects');
    }
     ...
     */
    chmod(args) {
        const { callback, objects, dbAction } = this.options;
        ...
        dbAction(() => {
            objects.chmodObject(pattern, { user: 'system.user.admin', object: modeObject, state: modeState }, (err, processed) => {
                ...
                this.printObjectList(err, processed);
            });
        });
    }
     ...
     */
    get(args) {
        const { callback, objects, pretty, dbAction } = this.options;
        /** @type {[string, string]} */
     ...
     */
    printObjectList(err, objList) {
        // TODO: is this supposed to be here?
        const { callback, objects, states } = this.options;
        if (err) {
            console.error(err);
