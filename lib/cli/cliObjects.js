...
module.exports = class CLIObjects extends CLICommand {
    ...
    _collectObjects(objects, params, callback, _types, _result) {
        ...
        if (!_types.length) {
            ...
            objects.getObjectView('system', _types.pop(), params, (err, res) => {
                res.rows && res.rows.forEach(item => _result.push(item.value));
                setImmediate(() => this._collectObjects(objects, params, callback, _types, _result));
            });
        }
    }
        ...
        if (!ids || !ids.length) {
            ...
            objects.delObject(id, () => {
                tools.removeIdFromAllEnums(objects, id).then(() => setImmediate(this._deleteObjects, objects, ids, callback))
                    .catch(e => {
                        console.warn(`Could not remove ${id} from enums: ${e}`);
                        setImmediate(this._deleteObjects, objects, ids, callback);
                    });
            });
        }
    }
        ...
        dbConnect(objects => {
            if (id.endsWith('*')) {
                ...
                this._collectObjects(objects, params, result => {
                    ...
                    // ask
                    if (args.indexOf('-f') === -1 && args.indexOf('/y') === -1 && args.indexOf('--yes') === -1) {
                ...
                objects.delObject(id, err => {
                    if (err) {
                        ...
                        tools.removeIdFromAllEnums(objects, id).then(() => {
                            ...
                            CLI.error.cannotDeleteObjectFromEnums(id, e);
                            return void callback(3);
                        });
                    }
                });
