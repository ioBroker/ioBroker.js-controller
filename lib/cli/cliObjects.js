...
module.exports = class CLIObjects extends CLICommand {
    ...
    async _collectObjects(objects, params, callback) {
        const types = ['state', 'channel', 'device', 'enum', 'instance', 'host', 'adapter', 'meta', 'config', 'group', 'user', 'info', 'script'];
        const result = [];
        ...
        if (!ids || !ids.length) {
            ...
            for (const id of ids) {
                try {
                    ...
                    console.warn(`Could not delete object or remove "${id}" from enums: ${e}`);
                }
            }
            return tools.maybeCallback(callback);
        }
