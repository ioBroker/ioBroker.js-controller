...
const errorMessages = Object.freeze({
    ...
    specifyInstance: (/** @type {string} */ adapter) => `The adapter "${adapter}" has multiple instances! Please specify which one should be started, e.g. "${adapter}.0".`,
    ...
    ) => `A host with the name "${hostname}" already exists!`
});
...
const warnings = Object.freeze({
    noInstancesFoundOnHost: (
        /** @type {string?} */ hostname
    ) => hostname != undefined ? `No instances found for host "${hostname}"!` : `No instances found!`
});
