...
module.exports = class CLICompact extends CLICommand {
    ...
    setCompactModeHost(targetState) {
        ...
        dbConnect((_objects, _states, isOffline) => {
            ...
            if (!isOffline) {
                console.log();
                console.log('Please stop ioBroker for this change to take effect!')
            }
            return void callback();
        });
    }
    ...
    statusCompactModeInstance(instance) {
        ...
        dbConnect((objects, _states, isOffline, _objectDbType, config) => {
            if (!this.config.system || !this.config.system.compact) {
                console.log('Compact mode is disabled for this host. This setting will have no effect!');
                console.log();
            }
            objects.getObject('system.adapter.' + instance, (err, obj) => {
                if (!err && obj) {
                    ...
                    console.log('Compact group:          ' + (obj.common.compactGroup || 0));
                    return void callback();
                } else {
            ...
            objects.getObject('system.adapter.' + instance, (err, obj) => {
                if (!err && obj) {
                    ...
                    if (obj.common.compactGroup === undefined) {
                        obj.common.compactGroup = obj.common.compactGroup || 0;
