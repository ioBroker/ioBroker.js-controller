 ...
 *        
 */
function copyAttributes(oldObj, newObj, originalObj) {
    for (let attr in oldObj) {
        if (oldObj.hasOwnProperty(attr)) {
            if (typeof oldObj[attr] !== 'object' || oldObj[attr] instanceof Array) {
                if (oldObj[attr] === '__no_change__' && originalObj) {
                    newObj[attr] = originalObj[attr];
                } else
                if (oldObj[attr] === '__delete__') {
                ...
                copyAttributes(oldObj[attr], newObj[attr], originalObj && originalObj[attr]);
            }
        }
    }
}
...
function checkNonEditable(oldObject, newObject) {
    if (!oldObject) return;
    if (!oldObject.nonEdit && !newObject.nonEdit) return;
