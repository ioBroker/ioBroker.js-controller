...
/**
 * Validates types of obj.common properties, if invalid types are used, an error is thrown.
 * If attributes are not provided, no error is thrown
 *
 * @param {object} obj an object which will be validated
 * @throws Error if a property has the wrong type
 */
function validateObjectCommonProperties(obj) {
    if (!obj || !obj.common) return;

    if (obj.common.name !== undefined && typeof obj.common.name !== 'string') {
        throw new Error(`obj.common.name has wrong type (${typeof obj.common.name}) but has to be string`);
    }

    if (obj.common.type !== undefined) {
        if (typeof obj.common.type !== 'string') {
            throw new Error(`obj.common.type has wrong type ${typeof obj.common.type} but has to be string`);
        }

        // if object type indicates a state, check that common type matches
        const allowedStateTypes = ['number', 'string', 'boolean', 'array', 'object', 'mixed', 'file'];
        if (obj.type === 'state' && !allowedStateTypes.includes(obj.common.type)) {
            throw new Error(`obj.common.type has an invalid value (${obj.common.type}) but has to be one of ${allowedStateTypes.toString()}`);
        }
    }

    if (obj.common.read !== undefined && typeof obj.common.read !== 'boolean') {
        throw new Error(`obj.common.read has wrong type ${typeof obj.common.read} but has to be boolean`);
    }

    if (obj.common.write !== undefined && typeof obj.common.write !== 'boolean') {
        throw new Error(`obj.common.write has wrong type ${typeof obj.common.write} but has to be boolean`);
    }

    if (obj.common.role !== undefined && typeof obj.common.role !== 'string') {
        throw new Error(`obj.common.role has wrong type ${typeof obj.common.role} but has to be string`);
    }

    if (obj.common.desc !== undefined && typeof obj.common.desc !== 'string') {
        throw new Error(`obj.common.desc has wrong type ${typeof obj.common.desc} but has to be string`);
    }
}
...
module.exports = {
    ...
    sendDiagInfo,
    upToDate,
    validateObjectCommonProperties,
    checkNonEditable,
    copyAttributes,
    getDiskInfo,
    wrapES6Class,
