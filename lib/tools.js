...
function getInstalledInfo(hostRunningVersion) {
    ...
    let ioPackage;
    try {
        ioPackage = JSON.parse(fs.readFileSync(path.join(fullPath, 'io-package.json'), 'utf8'));
    } catch (e) {
        console.error(`Cannot get installed host information: ${e}`);
    }
    const package_  = fs.existsSync(path.join(fullPath, 'package.json')) ? JSON.parse(fs.readFileSync(path.join(fullPath, 'package.json'), 'utf8')) : {};
    ...
    if (fs.existsSync(path.join(__dirname, '..', '..', '..', 'node_modules', `${module.exports.appName.toLowerCase()}.js-controller`)) ||
 ...
 *
 * @param {function(any):void|undefined} callback - callback function to be executed
 * @param {any} [param] - parameter which will be returned by the callback function or by the Promise
 * @returns {Promise}
 */
function maybeCallback(callback, param) {
    if (typeof callback === 'function') {
        // if function we call it with given param
        setImmediate(callback, param);
    } else {
 ...
 *
 * @param {function(Error=, any=):void|undefined} callback - callback function to be executed
 * @param {any} [error] - error which will be used by the callback function. If callback is not a function and
 ...
 * @returns {Promise}
 */
function maybeCallbackWithError(callback, error, param) {
    ...
    if (typeof callback === 'function') {
        setImmediate(callback, error, param);
    } else if (error) {
