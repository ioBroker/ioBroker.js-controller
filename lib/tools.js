    ...
    for (let attr in oldObj) {
        if (oldObj.hasOwnProperty(attr)) {
            if (typeof oldObj[attr] !== 'object' || oldObj[attr] instanceof Array) {
                ...
                copyAttributes(oldObj[attr], newObj[attr]);
            }
        }
    }
}
...
/**
 * Internal function to initialize nonEdit object
 *
 * @alias _replaceNoChange
 * @memberof tools
 * @param {object} obj object
 * @param {object} nonEdit object.nonEdit of the same object
 *
 */
function _replaceNoChange(obj, nonEdit) {
    if (!nonEdit) {
        nonEdit = obj.nonEdit;
        if (!nonEdit) return;
    }
    for (let attr in nonEdit) {
        if (nonEdit.hasOwnProperty(attr)) {
            if (typeof nonEdit[attr] === 'object') {
                _replaceNoChange(obj[attr], nonEdit[attr]);
            } else if (nonEdit[attr] === '__no_change__') {
                if (obj[attr] !== undefined) {
                    nonEdit[attr] = obj[attr];
                }
            }
        }
    }
...
function checkNonEditable(oldObject, newObject) {
    if (!oldObject) return;
    if (!oldObject.nonEdit) return;
    _replaceNoChange(oldObject);
    // restore settings
    copyAttributes(oldObject.nonEdit, newObject);
}
...
function uuid(givenMac, callback) {
    ...
    if (fs.existsSync('/etc/iob-vendor.json')) {
        try {
            const vendor = require('/etc/iob-vendor.json');
            if (vendor && vendor.vendor) {
                u = (vendor.vendor.uuid_prefix || 'CO') + u;
            }
        } catch (e) {
            u = 'CO' + u;
        }
