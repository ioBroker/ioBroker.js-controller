...
/**
 * @param {ioBroker.ObjectsDB} _objects
 * @param {*} callback
 */
 ...
 * @param {(err: Error | null | undefined, sources: Record<string, any>, hash?: any) => void} callback
 *
 */
 ...
 * @alias getHostInfo
 * @memberof Tools
 * @param {ioBroker.ObjectsDB} objects
 * @param {ioBroker.GenericCallback<HostInfo>} callback return result
 *        <pre><code>
 *            function (err, result) {
...
function getHostInfo(objects, callback) {
    ...
    const cpus = os.cpus();
    const osPlatform = os.platform(); // is equal to process.platform
    const nicePlatform = osPlatform === 'win32' ? 'Windows'
        : osPlatform === 'darwin' ? 'OSX'
            : osPlatform;
    /** @type {HostInfo} */
    const data = {
        Platform:     nicePlatform,
        os:           osPlatform,
 ...
 *
 * @param {ioBroker.ObjectsDB} objects
 * @param {ioBroker.StatesDB} states
 * @param {*} namespace
 * @param {*} q
 */
 ...
 * @memberof tools
 * @param {ioBroker.ObjectsDB} objects object to access objects db
 ...
 * @param {ioBroker.ObjectsDB} objects objects DB
 ...
 * @param {T} args - as many arguments as needed, which will be returned by the callback function or by the Promise
 * @returns {T extends [] ? Promise<void> : T extends [any?] ? Promise<T[0]> : Promise<T>} - if Promise is resolved with multiple arguments, an array is returned
 */
...
/**
 * Helper type to create correct return type inference for maybeCallbackWithError if `error` has type `any`
 * https://stackoverflow.com/a/61626123/10179833
 * @template T
 * @typedef {0 extends (1 & T) ? true : false} IsAny<T> */

// maybeCallbackWithError has a pretty complicated return type, which basically does the following:
// If the error is set (not null or undefined), return Promise<any>
// If there is no argument: return Promise<any> if the error has type `any`, otherwise Promise<void>
// If there is exactly one argument, return a Promise with that argument's type
// If there is more than one argument, return a Promise which resolves to an array type (all arguments)
 ...
 *
 * @template {any[]} T
 * @template {Error | string | null | undefined} TError
 * @param {((error: Error | null | undefined, ...args: any[]) => void) | null | undefined} callback - callback function to be executed
 * @param {TError} error - error which will be used by the callback function. If callback is not a function and
 ...
 * @param {T} args - as many arguments as needed, which will be returned by the callback function or by the Promise
 * @returns {[TError] extends [null | undefined] ? (T extends [] ? ([true] extends [IsAny<TError>] ? Promise<any> : Promise<void>) : T extends [any?] ? Promise<T[0]> : Promise<T>) : Promise<any>} - if Promise is resolved with multiple arguments, an array is returned
 */
function maybeCallbackWithError(callback, error, ...args) {
    if (error !== undefined && error !== null && !(error instanceof Error)) {
        // @ts-ignore if it's not a real Error, we convert it into one
