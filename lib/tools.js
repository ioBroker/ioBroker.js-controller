...
function getMac(callback) {
    ...
    require('child_process').exec(command, (err, stdout, _stderr) => {
        if (err) {
            callback(err);
        } else {
            let macAddress;
...
function getNpmVersion(adapter, callback) {
    ...
    exec(cliCommand, {timeout: 2000}, (error, stdout, _stderr) => {
        let version;
        if (error) {
            // command failed
...
function getRepositoryFile(urlOrPath, additionalInfo, callback) {
    ...
    if (!urlOrPath) {
        ...
        if (additionalInfo) {
            // Add some information to user-agent, like chrome, IE and Firefox do
            agent = `${additionalInfo.name}, RND: ${randomID}, Node:${additionalInfo.node}, V:${additionalInfo.controller}`;
        }
...
function getSystemNpmVersion(callback) {
    ...
    try {
        ...
        exec('npm -v', {encoding: 'utf8', env: newEnv}, (error, stdout) => {//, stderr) {
            if (timeout) {
    ...
    if (diskusage) {
        ...
        try {
            if (platform === 'Windows' || platform === 'win32') {
                ...
                exec('wmic logicaldisk get size,freespace,caption', {encoding: 'utf8'}, (error, stdout) => {//, stderr) {
                    if (stdout) {
