...
/**
 * Find the adaper main file as full path
 *
 * @memberof tools
 * @param {string} adapter - adapter name of the adapter, e.g. hm-rpc
 * @returns {Promise<string>}
 */
async function getAdapterMainFile(adapter) {
    const adapterDir = getAdapterDir(adapter);
    if (!adapterDir) {
        return Promise.reject(new Error(`Could not find adapter dir of ${adapter}`));
    }
    ...
    for (const mainFile of possibleMainFiles) {
        ...
        if (await fs.pathExists(fullFileName)) {
            return Promise.resolve(fullFileName);
        }
    }

    return Promise.reject(new Error(`Could not find main file of ${adapter}`));
}
