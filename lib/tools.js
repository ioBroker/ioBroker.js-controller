...
function getIoPack(sources, name, callback) {
    getJson(sources[name].meta, function (ioPack) {
        ...
        if (!ioPack) {
            sources._failCounter = sources._failCounter || [];
            sources._failCounter.push(name);
...
function getRepositoryFile(urlOrPath, callback) {
    ...
    if (!urlOrPath) {
        ...
        for (var name in sources) {
            ...
            if (!sources[name].version && sources[name].meta) {
                ...
                getIoPack(sources, name, function (ignore, name) {
                    toRead--;
                    if ((!toRead || sources._failCounter.length > 10) && timeout) {
                        var err = null;
                        if (sources._failCounter) {
                            if (sources._failCounter.length > 10) {
                                console.log('[' + new Date() + '] Looks like there is no internet.');
                                err = 'Looks like there is no internet.';
                            } else {
                                err = 'Following packages cannot be read: ' + (sources._failCounter ? sources._failCounter.join(', ') : '');
                            }
                            delete sources._failCounter;
                        }

                        if (delete sources._failCounter !== undefined) delete sources._failCounter;
        ...
        if (!toRead) {
            if (callback) callback(null, sources);
        } else {
        ...
        getJson(urlOrPath, function (sources) {
            if (sources) {
                for (var name in sources) {
                    ...
                    if (!sources[name].version && sources[name].meta) {
                        ...
                        getIoPack(sources, name, function (ignore, name) {
                            toRead--;
                            if ((!toRead || (sources._failCounter && sources._failCounter.length > 10)) && timeout) {
                                var err;
                                if (sources._failCounter) {
                                    if (sources._failCounter.length > 10) {
                                        console.log('[' + new Date() + '] Looks like there is no internet.');
            ...
            if (!toRead) {
                ...
                timeout = setTimeout(function () {
                    if (timeout) {
                        console.log('Timeout by read all package.json (' + (count / 2) + ') seconds');
                        clearTimeout(timeout);
                        if (callback) callback('Timeout by read all package.json (' + count + ') seconds', sources);
                        timeout = null;
                        callback = null;
