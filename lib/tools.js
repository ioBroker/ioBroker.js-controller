 ...
 */
async function getOrderedInstances(objects, logger, logPrefix='') {
    const instances = {'1': [], '2': [], '3': [], 'admin': []};
    this.allowedTiers = [1, 2, 3];
    ...
    for (const row of doc.rows) {
        if (row && row.value) {
            ...
            } else if (row.value.common && this.allowedTiers.includes(parseInt(row.value.common.tier))) {
