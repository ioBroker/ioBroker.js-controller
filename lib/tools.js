...
function getMac(callback) {
    ...
    require('child_process').exec(command, {windowsHide: true}, (err, stdout, _stderr) => {
        if (err) {
            ...
            while (true) {
                match = macRegex.exec(stdout, {windowsHide: true});
                if (!match) break;
                macAddress = match[0];
...
function createUuid(_objects, callback) {
    ...
    _objects.getObject('system.meta.uuid', (err, obj) => {
        if (!err && obj && obj.native && obj.native.uuid) {
            ...
            uuid(res => {
                _uuid = res;
                if (fs.existsSync('/etc/iob-vendor.json') && fs.readFileSync('/etc/iob-vendor.json')) {
                    try {
                        ...
                        console.error('Cannot parse /etc/iob-vendor.json');
                    }
                }
