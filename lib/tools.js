...
function getAdapterDir(adapter) {
    ...
    if (!adapterPath) {
        ...
        let parts = path.normalize(adapterPath).split(/[\\\/]/g);
        parts.pop();
        return parts.join('/');
    }
}
...
function getDiskInfo(platform, callback) {
    ...
    if (diskusage) {
        try {
            let path = platform === 'win32' ? __dirname.substring(0, 2) : '/';
    ...
    return new Promise((resolve, reject) => {
        objects.getObjectView('system', 'state', {startkey: namespace + '.', endkey: namespace + '.\u9999', include_docs: false}, (err, _states) => {
            if (err) {
                ...
                states.getStates(keys, (err, values) => {
                    // Get only states, that have ack = true
                    keys = keys.filter((id, i) => values[i] && values[i].ack);
