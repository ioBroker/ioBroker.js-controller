 ...
 * @memberof tools
 * @param {object} _objects object to access objects db
 * @param {string} id the object id which will be deleted from enums
 * @returns {Promise}
 *
 */
function removeIdFromAllEnums(_objects, id) {
    return new Promise((resolve, reject) => {
        _objects.getObjectView('system', 'enum', {startkey: '', endkey: '\u9999'}, (err, res) => {
            if (err) {
                reject(err);
            } else {
                const promises = [];
                for (const obj of res.rows) {
                    ...
                    if (idx !== -1) {
                        ...
                        promises.push(new Promise(resolve => {
                            _objects.setObject(obj.value._id, obj.value, err => {
