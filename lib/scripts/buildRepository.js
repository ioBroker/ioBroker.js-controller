...
if (module.exports && module.parent) {
    ...
    if (process.argv.indexOf('--latest') === -1) {
        tools.getRepositoryFile('https://github.com/' + tools.appName + '/' + tools.appName + '.repositories/raw/master/sources-dist-stable.json', function (err, data) {
            if (err) {
                console.error(err);
                process.exit(1);
            }
        ...
        tools.getRepositoryFile('https://github.com/' + tools.appName + '/' + tools.appName + '.repositories/raw/master/sources-dist.json', function (err, data) {
            if (err) {
                console.error(err);
                process.exit(1);
            }
