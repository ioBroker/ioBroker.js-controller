...
function killPidsScript(callback) {
    checkRoot(function (isRoot) {
        ...
        if (isRoot) {
            fs.writeFileSync(__dirname + '/../killall.sh', "sudo pgrep -f '^io.*' | sudo xargs kill -KILL\nsudo pgrep -f '^node-red*' | sudo xargs kill -KILL");
        } else {
            fs.writeFileSync(__dirname + '/../killall.sh', "pgrep -f '^io.*' | xargs kill -KILL\pgrep -f '^node-red*' | xargs kill -KILL");
        }
