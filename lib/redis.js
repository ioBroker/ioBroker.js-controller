...
function StateRedis(settings) {
    ...
    this.getStates = function (keys, callback) {
        ...
        client.mget(keys, function (err, obj) {
            if (err) {
                log.warn('redis mget ' + ((!obj) ? 0 :  obj.length) + ' ' + keys + ',err: ' + err);
            } else {
                log.debug('redis mget ' + ((!obj) ? 0 :  obj.length) + ' ' + keys);
            }
    ...
    this.setConfig = function (id, obj, callback) {
        ...
        client.set(id, JSON.stringify(obj), function (err) {
            ...
            setTimeout(function () {client.publish(id, JSON.stringify(obj));}, 0);
        });
    };
    ...
    this.delConfig = function (id, callback) {
        ...
        client.del(id, function (err) {
            if (typeof callback === 'function') callback(err);
            setTimeout(function () {client.publish(id, null);}, 0);
        });
    };
