...
function StateRedis(settings) {
    ...
    var namespaceMessage =  (settings.namespaceMessage || 'message') + '.';
    ...
    this.pushMessage = function (id, state, callback) {
        client.lpush(namespaceMessage + id, JSON.stringify(state), function (err, obj) {
            client.incr(namespaceMessage + id + ".changed");
            if (typeof callback === 'function') callback(err, obj);
        });
    };

    this.lenMessage = function (id, callback) {
        client.llen(namespaceMessage + id, function (err, obj) {
            if (typeof callback === 'function') callback(err, obj);
        });
    };

    this.getMessage = function (id, callback) {
        client.lpop(namespaceMessage + id, function (err, obj) {
            if (typeof callback === 'function') callback(err, obj);
        });
    };

    this.subscribeMessage = function (id) {
        log.debug('redis psubscribe ' + namespaceMessage + id);
        sub.psubscribe(namespaceMessage + id + ".changed");
        sub.psubscribe(namespaceMessage + id);
    };
