function List(options) {
    ...
    this.showFile = function (adapter, path, file) {
        ...
        var size = file.stats.size.toString();
    ...
    this.listAdaptersFiles = function (adapters, callback) {
        if (adapters && adapters.length) {
            var adapter = adapters.pop();
            this.listDirectory(adapter, function (files) {
                files.sort(sortFiles);
                for (var k = 0; k < files.length; k++) {
                    that.showFile(files[k].adapter, files[k].path, files[k].file);
                }

                that.listDirectory(adapter + '.admin', function (files) {
                    files.sort(sortFiles);
                    for (var k = 0; k < files.length; k++) {
                        that.showFile(files[k].adapter, files[k].path, files[k].file);
                    }
            ...
            case 'f':
                objects.getObjectList({startkey: 'system.adapter.', endkey: 'system.adapter.\u9999'}, function (err, objs) {
                    ...
                    objects.getObjectList({startkey: 'system.adapter.', endkey: 'system.adapter.\u9999'}, function (err, objs) {
                        ...
                        that.listAdaptersFiles(adapters, function () {
                            setTimeout(function () {
                                processExit();
                            }, 1000);
                        });
