...
function mkdirP(p, cb, _made) {
    ...
    fs.mkdir(p, mode, err => {
        if (!err) {
            ...
            mkdirP(path.dirname(p), (err, made) => {
                if (err) {
                    cb && cb(err, made);
                } else {
                    mkdirP(p, cb, made);
...
const sfs = {
    ...
    stageAsync: (filename, data, options) => {
        ...
        return fs_writeFileAsync(tmpname, data, options)
            .then(() => tmpname);
    },
    ...
    readFileAsync: pathname =>
        new Promise((resolve, reject) =>
            fs.readFile(pathname, 'utf8', (err, text) => 
                err ? reject(err) : resolve(parsePythonConf(text)))),
    ...
    const store = {
        ...
        keypairs: {
            ...
            setAsync: (keypath, keypair, format) =>
                mkdirpAsync(path.dirname(keypath))
                    .then(() => {
        ...
        accounts: {
            ...
            _getAccountIdByEmail: args => {
                ...
                if ('string' !== typeof email) {
                    ...
                    return fs_readdirAsync(args.accountsDir)
                        .then(nodes => {
                            log(args.debug, 'success reading arg.accountsDir');

                            return Promise.all(nodes.map(node =>
                                fs_readFileAsync(path.join(args.accountsDir, node, 'regr.json'), 'utf8')
                                    .then(text => {
