...
class BackupRestore {
    ...
    _setObjHelper(_index, _objects, callback) {
        ...
        if (_objects[_index].doc && _objects[_index].doc._rev) {
            delete _objects[_index].doc._rev;
        }

        this.objects.setObject(_objects[_index].id, _objects[_index].doc, err => {
            if (err) {
                console.warn('host.' + hostname + ' Cannot restore ' + _objects[_index].id + ': ' + err);
            }

            if ((_index % 200) === 0) {
                console.log('host.' + hostname + ' Processed ' + _index + '/' + _objects.length + ' objects');
            }
            _index++;
