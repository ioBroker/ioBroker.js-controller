...
class BackupRestore {
    ...
    createBackup(name, noConfig, callback) {
        ...
        this.objects.getObjectList({include_docs: true}, (err, res) => {
            ...
            this.states.getKeys('io.*', (err, keys) => {
                ...
                this.states.getStates(keys, (err, obj) => {
                    ...
                    Promise.all(promises).then(() => this.validateBackupAfterCreation(name))
                        .then(() => this.packBackup(name, callback)).catch(e => {
                            console.log(e);
                            this.processExit(1); // TODO: create exit code
                        });
                });
            });
    ...
    reloadAdaptersObjects(callback, dirs, index) {
        if (!dirs) {
            dirs = [];
            let _modules;
            let p = require('path').normalize(__dirname + '/../../node_modules');

            if (fs.existsSync(p)) {
                if (p.indexOf('js-controller') === -1) {
                    _modules = fs.readdirSync(p);
                    if (_modules) {
                    ...
                    p = require('path').normalize(__dirname + '/../../../node_modules');
                    if (fs.existsSync(p)) {
                        _modules = fs.readdirSync(p);
                        if (_modules) {
                            const regEx = new RegExp('^' + tools.appName + '\\.', 'i');
    ...
    validateBackup(name) {
        return new Promise(resolve => {
            ...
            tar.extract({
                ...
                if (!backupJSON.objects || !backupJSON.objects.length) {
                    console.log('Backup does not contain valid objects');
                    this.processExit(1); // TODO: meaninful exit code
                } // endIf

                this._removeFolderRecursive(tmpDir + '/backup/').then(resolve);
            });
        });
    } // endValidateBackup
