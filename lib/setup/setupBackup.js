...
function BackupRestore(options) {
    ...
    this.createBackup = function (name, noConfig, callback) {
    ...
    function reloadAdaptersObjects(callback, dirs, index) {
        if (!dirs) {
            ...
            if (index < dirs.length) {
                ...
                upload.uploadAdapter(adapterName, false, true, () => {
                    upload.uploadAdapter(adapterName, true, true, () => {
                        let pkg = null;
                        if (!dirs[index]) {
                            console.error('Wrong');
                        }
