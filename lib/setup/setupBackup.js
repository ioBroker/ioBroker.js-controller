                ...
                states.getStates(keys, (err, obj) => {
                    ...
                    for (let j = 0; j < result.objects.length; j++) {
                        ...
                        if (result.objects[j].value.type === 'meta' &&
                            ...
                            count++;
                            copyDir(result.objects[j].id, '', tmpDir + '/backup/files/' + result.objects[j].id, () => {
                                count--;
                                if (!count) {
                                    // todo: store letsencrypt files too => change it as letsencrypt will be better integrated
                                    const configDir = tools.getConfigFileName().split('/');
                                    configDir.pop();
                                    configDir.push('letsencrypt');
                                    const letsEncrypt = configDir.join('/');
                ...
                _setObjHelper(0, restore.objects, () => {
                    ...
                    uploadUserFiles(tmpDir + '/backup/files', () => {
                        ...
                        reloadAdaptersObjects(function () {
                            // Reload host objects
                            const pckgio = JSON.parse(fs.readFileSync(__dirname + '/../../io-package.json', 'utf8'));
                            reloadAdapterObject(0, pckgio ? pckgio.objects : null, () => {
                                if (restartOnFinish) {
