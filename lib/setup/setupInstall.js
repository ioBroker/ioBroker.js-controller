...
function Install(options) {
    ...
    const unsafePermAlways = [tools.appName.toLowerCase() + '.zwave', tools.appName.toLowerCase() + '.amazon-dash'];
    let  JSZip;
    ...
    this.uploadStaticObjects = function (adapter, adapterConf, callback) {
        ...
        checkDependencies(adapterConf.common.dependencies, params, () => {
        ...
        return getObjectViewAsync('system', 'instance', {startkey, endkey}, null).then(doc => {
            if (doc.rows.length === 0) {
                ...
                const newObjs = doc.rows
                    .filter(row => {
                        if (row && row.value && row.value._id && (!row.value.common || !row.value.common.host || row.value.common.host === hostname)) {
                            return true;
                        } else {
                            if (row && row.value && row.value._id && notDeleted.indexOf(row.value._id) === -1) {
