...
function Install(options) {
    ...
    function extractFiles(fileName, targetName, callback) {
        ...
        zip.loadAsync(fs.readFileSync(fileName)).then(() => {
            ...
            for (const fName in zip.files) {
                if (!zip.files.hasOwnProperty(fName) || !fName || fName[fName.length - 1] === '/') {
                    continue;
                }
                count++;
    ...
    this.downloadPacket = function (repoUrl, packetName, options, stoppedList, callback) {
        ...
        if (sources[packetName]) {
            ...
            if (url && url.match(/\/tarball\/master$/)) {
                // Install node modules
                return that.npmInstallWithCheck(url, options, false, () => {
                    // command succeeded
                    typeof callback === 'function' && callback(_callback => enableAdapters(stoppedList, true, _callback), packetName);
                });
            }
                ...
                for (const dName in allDeps) {
                    ...
                    if (!isFound) {
                        ...
                        if (globalDeps[dName] !== undefined) {
                            instances = objs.rows.filter(obj => obj && obj.value && obj.value.common && obj.value.common.name === dName);
                        }

                        if (deps[dName] !== undefined) {
                            // local dep get all instances on same host
                            const locInstances = objs.rows.filter(obj => obj && obj.value && obj.value.common && obj.value.common.name === dName && obj.value.common.host === hostname);
                            if (locInstances.length  === 0) {
                                console.error(`host.${hostname} Required adapter "${dName}" not found on this host`);
                            }
                            // rest of the check can be done equally for both types
                            instances = instances ? instances.concat(locInstances) : locInstances;
                        }

                        // we check, that all existing instances match
                        for (const instance of instances) {
                            if (!semver.satisfies(instance.value.common.version, version)) {
                                console.error(`host.${hostname} Invalid version of "${dName}". Installed "${instance.value.common.version}", required "${version}`);
                                return processExit(EXIT_CODES.INVALID_DEPENDENCY_VERSION);
                            } else {
                                isFound = true;
                            }
    ...
    this.installAdapter = (adapter, repoUrl, callback) => {
        ...
        packetManager.install(adapterConf.common.osDependencies && adapterConf.common.osDependencies[process.platform])
                    ...
                    objects.getObjectView('system', 'instance', {startkey: 'system.adapter.' + adapter + '.', endkey: 'system.adapter.' + adapter + '.\u9999'}, null, (err, res) => {
                        ...
                        for (let i = 0; i < adapterConf.instanceObjects.length; i++) {
                            ...
                            if (adapterConf.instanceObjects[i].common && adapterConf.instanceObjects[i].common.def !== undefined) {
                                defStates.push({id: adapterConf.instanceObjects[i]._id, val: adapterConf.instanceObjects[i].common.def});
                            }
                        }
