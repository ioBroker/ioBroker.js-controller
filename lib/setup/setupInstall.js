...
function Install(options) {
    ...
    function enableAdapters(adapters, isEnable, callback) {
        ...
        if (adapters) {
            ...
            for (let i = 0; i < adapters.length; i++) {
                ...
                adapters[i].ts = ts;
                objects.setObject(adapters[i]._id, adapters[i], () => {
                    if (!--count) callback();
                });
            }
        }
        ...
        if (sources[packetName]) {
            ...
            if (!url && packetName !== 'example') {
                ...
                that.npmInstallWithCheck(tools.appName.toLowerCase() + '.' + packetName + (version ? '@' + version : ''), options, false, () => {
                    // command succeeded
                    enableAdapters(stoppedList, true, () => callback && callback(packetName));
                });
                return;
            }
        ...
        tools.getFile(url, name + '.zip', tmpFile => {
            ...
            extractFiles(tmpFile, path.join(__dirname + '/../../tmp/', name), error => {
                ...
                if (dirs.length) {
                    ...
                    if (fs.existsSync(source + '/io-package.json')) {
                        ...
                        ncp(source, destination, err => {
                            ...
                            if (typeof callback === 'function') {
                                enableAdapters(stoppedList, true, () => callback && callback(name, packetIo));
                            }
                    ...
                    typeof callback === 'function' && callback(name, 'Packet is empty');
    ...
    function installAdapter(adapter, callback) {
        ...
        if (!fs.existsSync(adapterDir + '/io-package.json')) {
            ...
            that.downloadPacket(null, adapter, null, () => installAdapter(adapter, callback));
            return;
        }
        installCount = 0;
        let adapterConf;
    ...
    this.createInstance = function (adapter, options, callback) {
        ...
        objects.getObject('system.adapter.' + adapter, (err, doc) => {
            ...
            if (err || !doc || !doc.common.installedVersion) {
                installAdapter(adapter, () => that.createInstance(adapter, options, callback));
                return;
            }
                    ...
                    objects.getObjectView('system', 'instance', {startkey: 'system.adapter.' + adapter + '.', endkey: 'system.adapter.' + adapter + '.\u9999'}, null, (err, res) => {
                        ...
                        if (options.instance !== undefined) {
                            ...
                            if (res.rows.find(obj => parseInt(obj.id.split('.').pop(), 10) === instance)) {
                                ...
                                processExit(EXIT_CODES.INSTANCE_YET_EXISTS);
                                return;
                            }
                        } else {
                            // find max instance
    ...
    this.enumerateAdapterInstances = function enumerateInstances(knownObjIDs, notDeleted, adapter, instance) {
        ...
        return getObjectViewAsync('system', 'instance', {}, null).then(doc => {
            ...
            if (err !== tools.ERRORS.ERROR_NOT_EXISTS) console.error('host.' + hostname + ' error: ' + err);
        });
    };
    ...
    this.enumerateAdapterChannels = function enumerateAdapterChannels(knownObjIDs, adapter, instance) {
        ...
        return getObjectViewAsync('system', 'channel', {}, null).then(doc => {
            ...
            err !== tools.ERRORS.ERROR_NOT_EXISTS && console.error('host.' + hostname + ' error: ' + err);
        });
    };
    ...
    this.deleteWWW = tools.poorMansAsync(function* (adapter) {
        for (const file of [
            ...
            try {
                ...
                if (e.message !== tools.ERRORS.ERROR_NOT_EXISTS) console.error(`Cannot delete ${file} files folder: ${e}`);
            }
        }
        ...
        for (const objId of [
            ...
            try {
                ...
                if (e.message !== tools.ERRORS.ERROR_NOT_EXISTS) console.error('host.' + hostname + ' error: ' + e);
            }
        }
    });
    ...
    this.deleteAdapterStates = tools.poorMansAsync(function* (stateIDs) {
        ...
        while (stateIDs.length > 0) {
            ...
            try {
                ...
                if (e.message !== tools.ERRORS.ERROR_NOT_EXISTS) console.error(e);
            }
        }
    });
