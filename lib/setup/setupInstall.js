...
function Install(options) {
    ...
    this.enumerateAdapters = tools.poorMansAsync(function* (knownObjIDs, adapter) {
        ...
        //   endkey: system.adapter.mqtt.\u9999
        // matches system.adapter.mqtt AND system.adapter.mqtt-client
        const adapterRegex1 = new RegExp(`^system\\.adapter\\.${adapter}$`);
        const adapterRegex2 = new RegExp(`^system\\.adapter\\.${adapter}\\.`);

        // TODO: can there even be multiple adapter objects for a single adapter?
        try {
            const doc = yield getObjectViewAsync('system', 'adapter', {});
            doc.rows = doc.rows
                .filter(row => row && row.value && row.value._id)
                .filter(row => adapterRegex1.test(row.value._id) || adapterRegex2.test(row.value._id))
            ;
            if (doc.rows.length !== 0) {
