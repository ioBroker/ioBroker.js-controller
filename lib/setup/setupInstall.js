function Install(options) {
    ...
    function installAdapter(adapter, callback) {
        ...
        if (!fs.existsSync(adapterDir + '/io-package.json')) {
            console.log('we need to download package ... retry after that, count = ' + installCount);
            if (installCount === 2) {
                ...
                if (objs && objs.rows && objs.rows.length) {
                    for (var i = 0; i < deps.length; i++) {
                        ...
                        if (dName === 'js-controller') {
                            console.log('requirement is js-controller ... cnt=' + cnt)
                            // Check only version
                            if (version !== null) {
                        ...
                        if (!isFound) {
                            for (var t = 0; t < objs.rows.length; t++) {
                                if (objs.rows[t] && objs.rows[t].value && objs.rows[t].value.common && objs.rows[t].value.common.name === dName) {
                                    console.log('a version of ' + dName + 'is already installed: ' + objs.rows[t].value.common.version + ' vs. needed ' + version + ' cnt=' + cnt);
                                    if (version !== null) {
                                        if (!semver) semver = require('semver');
                        ...
                        if (!isFound) {
                            console.log('check dependencies: we need (not found) ' + dName + ' cnt=' + cnt);
                            cnt++;
        ...
        if (!fs.existsSync(adapterDir + '/node_modules')) {
            console.log('we need to install adapter using npm for ' + adapterDir);
            // Install node modules
            ...
            console.log('adapter files found for, no need to npm install ' + adapterDir);
