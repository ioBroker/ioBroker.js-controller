                    ...
                    objects.getObjectView('system', 'instance', {startkey: 'system.adapter.' + adapter + '.', endkey: 'system.adapter.' + adapter + '.\u9999'}, null, (err, res) => {
                        ...
                        function setObjs() {
                            if (objs.length > 0) {
                                ...
                                try {
                                    tools.validateObjectCommonProperties(obj);
                                } catch (e) {
                                    // todo: in the future we will not create this object
                                    console.warn(`Type check failed on ${obj._id}: ${e.message}`);
                                    console.warn('This object will not be created in future versions. Please report this to the developer.');
                                }
    ...
    this.deleteWWW = async (adapter) => {
        for (const file of [
            ...
            try {
                ...
                e !== tools.ERRORS.ERROR_NOT_FOUND && e.message !== tools.ERRORS.ERROR_NOT_FOUND && console.error(`Cannot delete ${file} files folder: ${e}`);
            }
        }
