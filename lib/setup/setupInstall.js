function Install(options) {
    ...
    this.downloadPacket = function (repoUrl, packetName, options, stoppedList, callback) {
        ...
        if (!repoUrl || typeof repoUrl !== 'object') {
            getRepository(repoUrl, function (sources) {
                that.downloadPacket(sources, packetName, options.useSystemNpm, stoppedList, callback);
            });
            return;
        }
        var version;
        ...
        if (!options.useSystemNpm) {
            ...
            // force system npm
            if (1 || config.network.useSystemNpm) options.useSystemNpm = true;
        }
        ...
        if (sources[packetName]) {
            ...
            if (!url && packetName !== 'example') {
                ...
                that.npmInstall(tools.appName + '.' + packetName + (version ? '@' + version : ''), options, false, function () {
                    // command succeeded
            ...
            if (url && url.match(/\/tarball\/master$/)) {
                // Install node modules
                that.npmInstall(url, options, false, function () {
                    // command succeeded
                    enableAdapters(stoppedList, true, function () {
                        if (callback) callback(packetName);
                    });
    ...
    this.npmInstall = function (npmUrl, options, debug, callback) {
        ...
        var cmd = 'npm install ' + npmUrl + (options.unsafePerm ? ' --unsafe-perm ' : '') + ' --production --prefix "' + cwd + '"';
    ...
    function installAdapter(adapter, callback) {
        ...
        console.log('host.' + hostname + ' install adapter ' + adapter + ' installCount =' + installCount);

        if (!fs.existsSync(adapterDir + '/io-package.json')) {
            console.log('we need to download package ... retry after that, count = ' + adapter + '/' + installCount);
            if (installCount === 2) {
                ...
                if (objs && objs.rows && objs.rows.length) {
                    for (var i = 0; i < deps.length; i++) {
                        ...
                        if (dName === 'js-controller') {
                            console.log('requirement is js-controller ... cnt=' + adapter + '/' + installCount + '/' + cnt)
                            // Check only version
                            if (version !== null) {
                        ...
                        if (!isFound) {
                            for (var t = 0; t < objs.rows.length; t++) {
                                if (objs.rows[t] && objs.rows[t].value && objs.rows[t].value.common && objs.rows[t].value.common.name === dName) {
                                    console.log('a version of ' + dName + 'is already installed: ' + objs.rows[t].value.common.version + ' vs. needed ' + version + ' cnt=' + adapter + '/' + installCount + '/' + cnt);
                                    if (version !== null) {
                                        if (!semver) semver = require('semver');
                        ...
                        if (!isFound) {
                            console.log('check dependencies: we need (not found) ' + dName + ' cnt=' + adapter + '/' + installCount + '/' + cnt);
                            cnt++;
                            that.createInstance(dName, _options, function (name) {
                                upload.uploadAdapter(dName, true, false, function () {
                                    console.log('first uploadAdapter called for dep ' + adapter);
                                    upload.uploadAdapter(dName, false, false, function () {
                                        console.log('second uploadAdapter called for dep ' + adapter);
                                        cnt--;
        ...
        if (!fs.existsSync(adapterDir + '/node_modules')) {
            console.log('we need to install adapter ' + adapter + ' using npm for ' + adapterDir);
            // Install node modules
            ...
            console.log('adapter files found for ' + adapter + ', no need to npm install ' + adapterDir);
            upload.uploadAdapter(adapter, true, true, function () {
                console.log('first uploadAdapter called for ' + adapter);
                upload.uploadAdapter(adapter, false, true, function () {
                    console.log('second uploadAdapter called for ' + adapter);
    ...
    function callInstallOfAdapter(adapter, config, callback) {
        ...
        console.log('callInstallOfAdapter ' + adapter);
