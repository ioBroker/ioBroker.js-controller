                ...
                if (!useSystemNpm) {
                    ...
                    tools.initNpm(cwd, function (er, npm) {
                        npm.commands.install(['iobroker.' + packetName], function (er, data) {
                            if (er) {
                                console.error('host.' + hostname + ' Cannot install iobroker.' + packetName + ': ' + er);
                                processExit(25);
                            }
                    ...
                    if (yargs && yargs.argv && yargs.argv.debug) {
                        child.stdout.pipe(process.stdout);
                    }
                    child.on('exit', function (code, signal) {
                        if (code) {
                            console.error('host.' + hostname + ' Cannot install iobroker.' + packetName + ': ' + code);
                            processExit(25);
                        }
                        // command succeeded
                        enableAdapters(stoppedList, true, function () {
