...
function Users(options) {
    ...
    this.enableUser = function (user, enable, callback) {
        ...
        if (user.match(/^system\.user\./)) user = user.substring('system.user.'.length);
        ...
        if (!user && !password) {
            ...
            prompt.get(schema, function (err, result) {
                that.checkPassword(result.username, result.password, (err, res) => {
                    if (err || !res) {
                        typeof callback === 'function' && callback('Password for user "' + result.username + '" does not matched' + (err ? ': ' + err : ''));
                    } else {
    ...
    this.enableGroup = function (group, enable, callback) {
        ...
        if (group.match(/^system\.group\./)) group = group.substring('system.group.'.length);
