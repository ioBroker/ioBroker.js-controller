...
function Vendor(options) {
    ...
    this.checkVendor = (password) => {
        let data;
        if (fs.existsSync('/etc/iob-vendor.json')) {
            try {
                data = JSON.parse(fs.readFileSync('/etc/iob-vendor.json'));
            } catch (e) {
                return Promise.reject(`cannot read or parse "/etc/iob-vendor.json": ${JSON.stringify(e)}`);
            }
        } else {
            return Promise.reject('/etc/iob-vendor.json does not exist');
        }
