                ...
                if (obj && obj.native) {
                    if (JSON.stringify(obj.native.vendor) !== JSON.stringify(vendor)) {
                        ...
                        return objects.setObjectAsync(obj._id, obj)
                            ...
                            .catch(err => logger.error(`Cannot update system.config: ${err.toString()}`));
                    }
                }
            }));
        }
        ...
        return Promise.all(promises)
                                        ...
                                        if (obj) {
                                            ...
                                            if (originalObj !== JSON.stringify(obj)) {
                                                ...
                                                return objects.setObjectAsync(obj._id, obj)
                                                    ...
                                                    .catch(err => logger.error(`Cannot update "${obj._id}": ${err.toString()}`));
                                            }
                                        } else {
                                            return objects.setObjectAsync(_id, _newObj)
                                                .then(() => logger.info(`object "${_newObj._id}" updated`))
                                                .catch(err => logger.error(`Cannot update ${_id} "${JSON.stringify(_newObj)}": ${err.toString()}`));
                                        }
                                    }));
                            })(id, data.objects[id]);
                        } else {
            ...
            .then(() => {
                if (data.model) {
                    ...
                    return objects.getObject('system.host.' + hostname, obj => {
                        if (obj && obj.common) {
