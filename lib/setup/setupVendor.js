...
function Vendor(options) {
    ...
    this.checkVendor = (file, password) => {
                ...
                if (obj && obj.native) {
                    if (obj.native.uuid !== uuid) {
                        ...
                        return objects.setObjectAsync('system.meta.uuid', obj)
                            .then(() => console.log('object system.meta.uuid updated: ' + uuid))
                            .catch(err => console.error(`Cannot update system.meta.uuid: ${err}`));
                    }
                } else {
                    return objects.setObjectAsync('system.meta.uuid', {
                        ...
                        .then(() => console.log('object system.meta.uuid created: ' + uuid))
                        .catch(err => console.error(`Cannot create system.meta.uuid: ${err}`));
                }
            }));
        }
                ...
                if (obj && obj.common) {
                    if ((model.name  && model.name !== 'JS controller' && obj.common.title === 'JS controller') ||
                        ...
                        return objects.setObjectAsync(obj._id, obj)
                            .then(() => console.log(`object "system.host.${hostname}" updated`))
                            .catch(err => console.error(`Cannot update "system.host.${hostname}": ${err}`));
                    }
                }
            }));
        }
        ...
        if (data.iobroker) {
            ...
            console.log('Update iobroker.json file');
            ...
            promises.push(objects.getObject('system.config').then(obj => {
                if (obj && obj.native) {
                    if (JSON.stringify(obj.native.vendor) !== JSON.stringify(vendor)) {
                        ...
                        obj.nonEdit.password = password;
                        return objects.setObjectAsync(obj._id, obj)
                            .then(() => console.log('object system.config updated'))
                            .catch(err => console.error(`Cannot update system.config: ${err}`));
                    }
                }
            }));
        }
                            ...
                            _promises.push(objects.getObject(_id).then(obj => {
                                if (obj) {
                                    ...
                                    if (originalObj !== JSON.stringify(obj)) {
                                        ...
                                        return objects.setObjectAsync(obj._id, obj)
                                            .then(() => console.log(`object "${obj._id}" updated`))
                                            .catch(err => console.error(`Cannot update "${obj._id}": ${err}`));
                                    }
                                } else {
