...
function Setup(options) {
    ...
    function migrateObjects(newConfig, oldConfig, rl, callback) {
        if (oldConfig && (oldConfig.states.type !== newConfig.states.type || oldConfig.objects.type !== newConfig.objects.type)) {
            ...
            if (fromMaster) {
                if (!toMaster) {
                    const answer = rl.question(`It appears that this host should be connected to an existing ioBroker System as Slave. Data Migration will be skipped. Is this correct? [Y/n]: `, {
                        limit: /^[YyNnJj]?$/,
                        defaultInput: 'Y'
                    });
            ...
            if (answer === 'Y' || answer === 'y' || answer === 'J' || answer === 'j') {
                ...
                return dbConnect(params, (objects, states, isOffline) => {
                    if (!isOffline) {
