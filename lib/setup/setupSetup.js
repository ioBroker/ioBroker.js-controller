...
function Setup(options) {
    ...
    async function informAboutPlugins(systemConfig, callback) {
        ...
        try {
            ...
            console.error('Can not read js-controller io-package.json. Ignore plugins defined there.');
            ioPackage = null;
        }
        try {
        ...
        if (ioPackage && ioPackage.common && ioPackage.common.plugins) {
            Object.keys(ioPackage.common.plugins).forEach(plugin => {
                if (ioPackage.common.plugins[plugin].enabled !== false) {
                    plugins[plugin] = ioPackage.common.plugins[plugin];
                }
            });
        }
        if (ioConfig && ioConfig.plugins) {
            Object.keys(ioConfig.plugins).forEach(plugin => {
                if (!plugins[plugin] && ioConfig.plugins[plugin].enabled !== false) {
                    plugins[plugin] = ioConfig.plugins[plugin];
                }
            });
        }
        ...
        for (let i = 0; i < pluginNames.length; i++) {
            ...
            if (!pluginInfos.PLUGIN_INFOS[plugin]) {
            ...
            const infoHeadLine = pluginInfos.PLUGIN_INFOS[plugin].headline[systemLang] || pluginInfos.PLUGIN_INFOS[plugin].headline.en;
            const infoText = pluginInfos.PLUGIN_INFOS[plugin].text[systemLang] || pluginInfos.PLUGIN_INFOS[plugin].text.en;
    ...
    function setupReady(systemConfig, callback) {
        if (!callback) {
    ...
    this.setupCustom = function (callback) {
        ...
        if (stype === otype && stype !== 'file' && shost === ohost) {
