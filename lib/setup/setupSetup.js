            ...
            rl.question('Host / Unix Socket of objects DB(' + otype + '), default[127.0.0.1]: ', ohost => {
                ...
                rl.question('Port of objects DB(' + otype + '), default[' + op + ']: ', userPort => {
                    ...
                    if (!userPort) {
                        ...
                        if (userPort.includes(',')) {
                            oport = userPort.split(',');
                    ...
                    rl.question('Type of states DB [(f)file, (r)edis], default [' + otype + ']: ', stype => {
                        if (!stype) {
                            stype = otype;
                        } else {
                        ...
                        rl.question('Host / Unix Socket of states DB (' + stype + '), default[' + (Array.isArray(ohost) ? ohost.join(',') : ohost) + ']: ', shost => {
                            if (!shost) {
                                shost = ohost;
                            } else {
                            ...
                            rl.question('Port of states DB (' + stype + '), default[' + sp + ']: ', userPort => {
                                ...
                                if ((stype === 'file' && (shost === 'localhost' || shost === '127.0.0.1')) ||
                                    ...
                                    rl.question('Data directory (file), default[../' + tools.getDefaultDataDir() + ']: ', dir => {
                                        ...
                                        rl.question('Host name of this machine [' + (originalConfig ? originalConfig.hostname || require('os').hostname() : require('os').hostname()) + ']: ', hname => {
                                            if (!hname) {
                                                hname = (originalConfig && originalConfig.hostname) || '';
