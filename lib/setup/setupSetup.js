...
function Setup(options) {
    ...
    this.setupCustom = callback => {
        ...
        try {
            ...
            getDefaultObjectsPort = require(path).defaultPort;
        } catch {
        ...
        let op = getDefaultObjectsPort(ohost);
        let oSentinel = otype === 'redis' && ohost.includes(',');
        ...
        try {
            const path = require.resolve(`@iobroker/db-states-${otype}`);
        } catch {
        ...
        try {
            ...
            getDefaultStatesPort = require(path).defaultPort;
        } catch {
        ...
        let sp = getDefaultStatesPort(shost);
        let sSentinel = stype === 'redis' && shost.includes(',');
