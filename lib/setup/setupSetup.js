            ...
            if (checkCertificateOnly) {
                ...
                if (certObj) {
                    objects.getObject('system.certificates', (err, obj) => {
                        if (obj && obj.native && obj.native.certificates && obj.native.certificates.defaultPublic) {
                            tools.getCertificateInfo(obj.native.certificates.defaultPublic, (err, cert) => {
                                if (!err && cert) {
                                    // check, if certificate is invalid (too old or keylength too short)
                                    if ((Date.parse(cert.validityNotAfter) <= Date.now()) || (cert.keyLength < 2048)) {
                                        // generate new certificates
                                        let newCert = tools.generateDefaultCertificates();
