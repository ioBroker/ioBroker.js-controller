        ...
        for (const adapter in adapters) {
            ...
            if (adapter === 'js-controller') {
                ...
                if (!iopack || !iopack.common || !iopack.common.version) {
                    return 'No version of "js-controller"';
                }
                if (!semver.satisfies(iopack.common.version, adapters[adapter])) {
                    return 'Invalid version of js-controler. Required ' + adapters[adapter];
                }
            } else {
                try {
                ...
                if (!iopack || !iopack.common || !iopack.common.version) {
                    return 'No version of "' + adapter + '"';
                }
                if (!semver.satisfies(iopack.common.version, adapters[adapter])) {
                    return 'Invalid version of "' + adapter + '"';
                }
            }
        }
        return '';
