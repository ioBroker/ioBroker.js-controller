                ...
                if (dName === 'js-controller') {
                    ...
                    if (deps[dName] !== undefined) {
                        ...
                        if (locInstances.length  === 0) {
                            return Promise.reject(new Error(`Required adapter "${dName}" not found on this host`));
                        }
                    ...
                    for (const instance of instances) {
                        if (!semver.satisfies(instance.value.common.version, version)) {
                            return Promise.reject(new Error(`Invalid version of "${dName}". Installed "${instance.value.common.version}", required "${version}`));
                        }
                        isFound = true;
                    }
