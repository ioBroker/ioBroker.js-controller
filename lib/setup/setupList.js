                        ...
                        for (let i = 0; i < objs.rows.length; i++) {
                            ...
                            if (!reg || reg.test(objs.rows[i].value._id) || (objs.rows[i].value.common && reg.test(objs.rows[i].value.common.name))) {
                                ...
                                if (this.config.system && this.config.system.compact) {
                                    text += ', compact ' + (objs.rows[i].value.common.compact && objs.rows[i].value.common.runAsCompactMode ? 'enabled (group ' + (objs.rows[i].value.common.compactGroup || 0) + ')' : 'disabled');
                                }
