            ...
            switch (type) {
                ...
                case 'i':
                    objects.getObjectList({startkey: 'system.adapter.', endkey: 'system.adapter.\u9999'}, (err, objs) => {
                        ...
                        const lines = [];
                        for (let i = 0; i < objs.rows.length; i++) {
                            if (objs.rows[i].value.type !== 'instance') {
                                continue;
                            }
                            if (!reg || reg.test(objs.rows[i].value._id) || (objs.rows[i].value.common && reg.test(objs.rows[i].value.common.name))) {
                                if (flags.enabled  && !objs.rows[i].value.common.enabled) {
                                    continue;
                                }
                                if (flags.disabled && objs.rows[i].value.common.enabled) {
                                    continue;
                                }
                                if (flags.port     && objs.rows[i].value.native.port   === undefined) {
                                    continue;
                                }
                                if (flags.ssl      && objs.rows[i].value.native.secure === undefined) {
                                    continue;
                                }
                                if (flags.ip       && objs.rows[i].value.native.bind   === undefined) {
                                    continue;
                                }
                                ...
                                if (name && name.length < 22) {
                                    name += new Array(22 - name.length).join(' ');
                                }

                                let text = id + ': ' + (name || '') + ' - ' +
                                    (objs.rows[i].value.common.enabled ? ' enabled' : 'disabled');

                                if (this.config.system && this.config.system.compact && objs.rows[i].value.common.compact) {
                                    text += ', compact ' + (objs.rows[i].value.common.compact && objs.rows[i].value.common.runAsCompactMode ? 'enabled (group ' + (objs.rows[i].value.common.compactGroup !== undefined ? objs.rows[i].value.common.compactGroup : 1) + ')' : 'disabled');
                                }
                        ...
                        for (let i = 0; i < objs.rows.length; i++) {
                            ...
                            if (!reg || reg.test(objs.rows[i].value._id) || (objs.rows[i].value.common && reg.test(objs.rows[i].value.common.name))) {
                                ...
                                if (objs.rows[i].value.common.acl && objs.rows[i].value.common.acl.other) {
                                    text += ' ';
                                    let others = '';
                                    for (const r in objs.rows[i].value.common.acl.other) {
