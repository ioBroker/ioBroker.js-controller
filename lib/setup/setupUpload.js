function Upload(options) {
    ...
    function getAllInstances(adapters, callback) {
        ...
        for (var i = 0; i < adapters.length; i++) {
            ...
            if (adapters[i].indexOf('.') == -1) {
                getInstances(adapters[i], function (inst) {
                    ...
                    console.log('getAllInstances ' + count + ' - callback at 1');
        ...
        console.log('getAllInstances ' + count + ' - callback at 0');
        if (!count) callback(instances);
    }
            ...
            if (adapterConf.common.restartAdapters) {
                ...
                if (adapterConf.common.restartAdapters.length && adapterConf.common.restartAdapters[0]) {
                    getAllInstances(adapterConf.common.restartAdapters, function (instances) {
                        if (!instances.length) {
                            console.log('checkRestartOther nothing to restart');
                            callback();
                            return;
                        }
                        instancesCount=instances.length;
                        console.log('loop over '+instancesCount);
                        for (var r = 0; r < instances.length; r++) {

                            objects.getObject(instances[r], function (err, obj) {
                                console.log('Restart "' + obj._id);
    ...
    this.uploadAdapter = function (adapter, isAdmin, forceUpload, subTree, callback) {
        ...
        console.log('start uploadAdapter ' + adapter + '/' + isAdmin + '/' + forceUpload);
        ...
        function done(err, res) {
            if (err) {
                console.log('error in upload for ' + adapter + '/' + isAdmin + '/' + forceUpload + ': ' + err);
                callback();
            } else {
                console.log('got ' + dir);
                files = res;
        ...
        function upload(adapter) {
            var file;
            if (!files.length) {
                console.log('call callback after finished upload ' + adapter + '/' + isAdmin + '/' + forceUpload);
                if (!isAdmin) {
                ...
                objects.setObject(id, {
                    ...
                    if (!isAdmin) {
                        checkRestartOther(adapter, function () {
                            console.log('after err: call walk after restarts ' + adapter + '/' + isAdmin + '/' + forceUpload);
                            walk(dir, done);
                        });
                    } else {
                        console.log('after err: call walk without restarts ' + adapter + '/' + isAdmin + '/' + forceUpload);
                        walk(dir, done);
                    }
                });
            } else {
                if (!forceUpload) {
                    console.log('no err, no force, callback ' + adapter + '/' + isAdmin + '/' + forceUpload);
                    if (typeof callback === 'function') callback(adapter);
                } else {
                    rev = res.rev;
                    if (!isAdmin) {
                        console.log('no err: call walk after restarts ' + adapter + '/' + isAdmin + '/' + forceUpload);
                        checkRestartOther(adapter, function () {
                            walk(dir, done);
                        });
                    } else {
                        console.log('no err: call walk without restarts ' + adapter + '/' + isAdmin + '/' + forceUpload);
                        walk(dir, done);
                    }
                }
            }
