...
function Upload(options) {
        ...
        getHosts(true, liveHosts => {
            ...
            getInstances(adapter, true, objects => {
                ...
                if (instance && instance.common.host !== hostname) {
                    process.stdout.write('Send upload command to host "' + instance.common.host + '"... ');
    ...
    function upload(adapter, isAdmin, files, id, rev, callback, _maxFiles) {
        ...
        if (!files.length) {
            ...
            if (file === '.gitignore') {
                return upload(adapter, isAdmin, files, id, rev, callback, _maxFiles);
            }
    ...
    this.uploadAdapter = (adapter, isAdmin, forceUpload, subTree, callback) => {
        ...
        if (!fs.existsSync(dir)) {
            console.error(`Directory "${dir || ('for ' + adapter + (isAdmin ? '.admin' : ''))}" was not found! Nothing was uploaded or deleted.`);
        ...
        if (!iopack) {
            ...
            try {
                ...
                if (adapterDir) {
                    ...
                    console.error(`Cannot find io-package.json for "${name}"`);
                }
                iopack = null;
            }
        }
