        ...
        if (!isErase) {
            ...
            objects.readDir(adapter, path, null, (err, files) => {
                ...
                if (!err) {
                    ...
                    err && console.error('Cannot read directory: ' + err);
                    callback(_files, _dirs);
                }
            });
        }
    ...
    this.uploadAdapter = (adapter, isAdmin, forceUpload, subTree, callback) => {
        ...
        function upload(adapter) {
            ...
            if (!files.length) {
                ...
                const mimeType = (mime.getType || mime.lookup)(file);
                let attName;
        ...
        eraseFolder(cfg && cfg.common && cfg.common.eraseOnUpload, isAdmin ? adapter + '.admin' : adapter, '/', (files, dirs) => {
            if (files) {
