        ...
        objects.getObject(id, function (err, res) {
            if (err || !res) {
                objects.setObject(id, {
                    ...
                    if (!isAdmin) {
                        checkRestartOther(adapter, function () {
                            console.log('after err: call walk after restarts');
                            walk(dir, done);
                        });
                    } else {
                        console.log('after err: call walk without restarts');
                        walk(dir, done);
                    }
                });
            } else {
                if (!forceUpload) {
                    ...
                    rev = res.rev;
                    if (!isAdmin) {
                        console.log('no err: call walk after restarts');
                        checkRestartOther(adapter, function () {
                            walk(dir, done);
                        });
                    } else {
                        console.log('no err: call walk without restarts');
                        walk(dir, done);
                    }
                }
            }
