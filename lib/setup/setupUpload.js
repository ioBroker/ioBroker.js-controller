...
function Upload(options) {
    ...
    function getInstances(adapter, withObjects, callback) {
    ...
    function getHosts(onlyAlive, callback) {
        ...
        objects.getObjectList({startkey: 'system.host.', endkey: 'system.host.\u9999'}, (err, arr) => {
            ...
            if (!err && arr && arr.rows) {
                for (let i = 0; i < arr.rows.length; i++) {
                    if (arr.rows[i].value.type !== 'host') continue;
        ...
        states.onChange = (id, msg) => {
            if (id.endsWith(from)) {
                if (msg.command === 'log' || msg.command === 'error' || msg.command === 'warn') {
                    ...
                    callback && callback(msg && msg.message);
                    callback = null;
    ...
    this.uploadAdapterFull = (adapters, callback) => {
        ...
        getHosts(true, liveHosts => {
            ...
            getInstances(adapter, true, objects => {
    ...
    function upload(adapter, isAdmin, files, id, rev, logger, callback, _maxFiles) {
        ...
        if (!files.length) {
            ...
            // do not uplaod '.gitignore' files. Todo: add other exceptions
            if (file === '.gitignore') {
    ...
    this.uploadAdapter = (adapter, isAdmin, forceUpload, subTree, logger, callback) => {
        ...
        if (!fs.existsSync(adapterDir)) {
            console.error(`Directory "${adapterDir || ('for ' + adapter + (isAdmin ? '.admin' : ''))}" was not found! Nothing was uploaded or deleted.`);
        ...
        if (!fs.existsSync(dir)) {
            ...
            (isAdmin || cfg.common.onlyWWW) && console.error(`Directory "${dir || ('for ' + adapter + (isAdmin ? '.admin' : ''))}" was not found! Nothing was uploaded or deleted.`);
            return typeof callback === 'function' && callback(adapter);
        }
        ...
        if (adapter === 'admin' && !isAdmin) {
            ...
            logger.log('This should never happens!');
            return typeof callback === 'function' && callback(adapter);
        }
        ...
        eraseFolder(cfg && cfg.common && cfg.common.eraseOnUpload, isAdmin ? adapter + '.admin' : adapter, '/', logger, (files, _dirs) => {
            if (files) {
                ...
                objects.getObject(id, (err, res) => {
                    ...
                    if (err || !res) {
                        objects.setObject(id, {
                            ...
                            if (!isAdmin) {
                                checkRestartOther(adapter, () => setTimeout(() => upload(adapter, isAdmin, files, id, res && res.rev, logger, callback), 25));
                            } else {
