...
class PacketManager {
    constructor(options) {
        ...
        if (process.platform !== 'win32') {
            !this.manager && this._readyPromises.push(this._detectManager()
                ...
                .then(isSudo => {
                    if (isSudo) {
                        return this._isSudoAvailableForManager().then(res => {
                            // sudo is available for packet manager
                            if (res) this.sudo = true;
                        });
                    }
                })
            );
    ...
    install(packets) {
        ...
        return this.ready()
            ...
            .catch(() => this.logger && this.logger.warn('Some native dependencies could not be installed. Please install them manually'));
    }
}
