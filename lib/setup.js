                                ...
                                rl.question('Port of states DB (' + stype + '), default[' + sp + ']: ', function (sport) {
                                    ...
                                    if ((stype == 'file' && (shost == 'localhost' || shost == '127.0.0.1')) ||
                                        ...
                                        rl.question('Data directory (file), default[../' + tools.getDefaultDataDir() + ']: ', function (dir) {
                                            ...
                                            fs.writeFileSync(tools.getConfigFileName(), JSON.stringify(config));
                                        });
                                    } else {
                                        rl.close();
                                        config = require(__dirname + '/../conf/iobroker-dist.json');
            ...
            if (adapter) {
                ...
                tools.getRepositoryFile(repoUrl, function (links) {
                    ...
                    upgradeAdapterHelper(links, result, 0, false, function () {
                        upgradeController(links, false, function () {
                            processExit();
                        });
                    });
                });
...
function setupWindows(path, callback) {
    fs.writeFileSync(path + '/iobroker.bat', 'node node_modules/iobroker.js-controller/iobroker.js %1 %2 %3 %4 %5');
    ...
    for (var f = 0; f < files.length; f++) {
        cnt++;
        copyFile(__dirname + '/../install/windows/' + files[f], __dirname + '/../' + path + '/' + files[f], function () {
            cnt--;
            if (!cnt) {
                // Call npm install node-windows
                var exec = require('child_process').exec;
                // js-controller installed as npm
                console.log('npm install node-windows --production --prefix "' + __dirname + '/../' + path + '/"');
                var child = exec('npm install node-windows --production --prefix "' + __dirname + '/../' + path + '/"');
                ...
                child.on('exit', function () {
                    ...
                    var child = exec('node "' + __dirname + '/../' + path + '/install.js"');
                    child.stderr.pipe(process.stdout); // TODO this produces unwanted newlines :-(
                    // liner.js from http://strongloop.com/strongblog/practical-examples-of-the-new-node-js-streams-api/ could solve this
                    child.on('exit', function () {
...
function setupLinux(path, callback) {
    fs.writeFileSync(path + '/iobroker', "node node_modules/iobroker.js-controller/iobroker.js $1 $2 $3	$4 $5');", {mode: '777'});
    console.log('Write "./iobroker start" to start the ioBroker');
    // check if /etc/init.d/ exists
    if (fs.existsSync('/etc/init.d')) {
        // copy iobroker.sh from install/linux to /etc/init.d/
        copyFile(__dirname + '/../install/linux/iobroker.sh', '/etc/init.d/iobroker.sh');
        var txt = fs.readFileSync('/etc/init.d/iobroker.sh');
        var parts = __dirname.replace(/\\/g, '/').split('/');
        parts.pop();
        txt = txt.replace(/@@path@@/g, parts.join('/') + '/');
        fs.writeFileSync('/etc/init.d/iobroker.sh', txt, '777');
...
function downloadPacket(repoUrl, packetName, callback) {
    ...
    if (sources[packetName]) {
        ...
        if (!url && packetName != 'example') {
            ...
            var exec = require('child_process').exec;
            var cmd = 'npm ';
            var options = {cwd: __dirname.replace(/\\/g, '/')};
            if (fs.existsSync(__dirname + '/../../../node_modules/iobroker.js-controller')) {
                // js-controller installed as npm
                cmd += 'install ';
                options.cwd = options.cwd.split('/');
                options.cwd.splice(options.cwd.length - 3, 3);
                options.cwd = options.cwd.join('/');
            } else if (fs.existsSync(__dirname + '/../../iobroker.' + packetName)) {
                cmd += 'install ';
            } else if (fs.existsSync(__dirname + '/../node_modules/iobroker.' + packetName)) {
                cmd += 'install '; //'update ';
            } else  {
                cmd += 'install ';
            }
            cmd += 'iobroker.' + packetName + ' --production';
            console.log(cmd + ' [' + options.cwd + ']');
