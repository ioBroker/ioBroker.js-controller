...
function dbConnect(onlyCheck, params, callback) {
    ...
    setTimeout(() => {
        ...
        if (onlyCheck) {
            console.log('States  ' + (isStatesConnected ? 'connected' : 'NOT connected'));
            console.log('Objects ' + (isObjectConnected ? 'connected' : 'NOT connected'));
            if (typeof callback === 'function') callback(states, objects, true, config.objects.type, config);
            return;
        }
