...
function processCommand(command, args, params, callback) {
    ...
    switch (command) {
        ...
        case 'package': {
            ...
            tools.getRepositoryFile(null, (_err, sources) => {
                if (sources) {
        ...
        case 'host': {
            ...
            dbConnect(params, (_objects, _states, isOffline) => {
                ...
                let count = 0;
                // find first host
                objects.getObjectList({startkey: 'system.host.', endkey: 'system.host.\u9999'}, (_err, objs) => {
                    if (!oldHostname) {
                        let hostCount = 0;
                    ...
                    for (let i = 0; i < objs.rows.length; i++) {
                        ...
                        const hNameM = objs.rows[i].value._id.match(/^system\.host\.([^.]+)(\..*)?$/);
                        let hName = null;
                        if (hNameM) {
                            hName = hNameM[1];
                        } else {
                    ...
                    if (!err && repos && repos.native && repos.native.repositories) {
                        ...
                        if (repos.native.repositories[active]) {
                            ...
                            if (!repos.native.repositories[active].json) {
                                ...
                                // Load it
                                tools.getRepositoryFile(repos.native.repositories[active].link, (_err, sources) => {
