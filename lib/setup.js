 ...
 *
 *  7'2014-2017 bluefox <dogafox@gmail.com>
...
function initYargs() {
    yargs = require('yargs')
            .usage('Commands:\n' +
                ...
                tools.appName + ' file read <iobroker-path-to-read> [<filesystem-path-to-write>]\n' +
                tools.appName + ' file write <filesystem-path-to-read> <iobroker-path-to-read> \n' +
                ...
                tools.appName + ' multihost/mh <enable|disable> [--secure true|false]\n' +
                tools.appName + ' multihost/mh browse\n' +
                tools.appName + ' multihost/mh connect\n' +
...
function processCommand(command, args, params, callback) {
    ...
    switch (command) {
                        ...
                        if (isFirst) {
                            ...
                            install.createInstance('admin', {enabled: true}, function () {
                                ...
                                try {
                                    ...
                                    if (path) {
                                        install.createInstance('discovery', {enabled: true}, function () {
                                            callback();
                                        });
                                    }
                                } catch (e) {
            ...
            (function () {
                ...
                dbConnect(params, function () {
                    ...
                    install.npmInstall(url, true, false, function (_url, installDir) {
                        ...
                        if (!name) {
                            var m = url.match(/ioBroker\.([-_\w\d]+)\//);
                            if (m) {
                                name = m[1];
                            } else {
                                m = url.match(/iobroker\.([-_\w\d]+)\//);
                                if (m) name = m[1];
                            }
                        }
                    ...
                    if (answer === 'y' || answer === 'yes' || answer === 'ja' || answer === 'j') {
                        dbConnect(params, function () {
                            objects.delObject('system.meta.uuid', function (err) {
                                if (err) {
                                    console.log('uuid not found: ' + err);
                                } else {
        ...
        case 'repo':
            (function () {
                Objects =       require(__dirname + '/objects');
                var repoName =   args[0]; // Repo url or name
                dbConnect(params, function () {
                        ...
                        if (err || !obj) {
                            ...
                            if (obj.native.repositories) {
                                ...
                                for (var r in obj.native.repositories) {
                                    if (!obj.native.repositories.hasOwnProperty(r)) continue;
                                    if (r === repoName) found = true;
                                    console.log(r + ': ' + obj.native.repositories[r].link);
                                }
                                objects.getObject('system.config', function (err, obj) {
                                    if (obj && (!repoName || !found)) {
                                    ...
                                    if (repoName) {
                                        if (found) {
                                            ...
                                            objects.setObject('system.config', obj, function (err) {
                                                ...
                                                if (err) {
                                                    console.error('Cannot set active repo: ' + err);
                                                    callback(103);
                                                } else {
                                                    callback();
                                                }
                                            ...
                                            console.warn('Unknown repository name: ' + repoName);
                                            callback(104);
                                        }
                                    } else {
                                        callback();
        ...
        case 'mh':
            (function () {
                var cmd = args[0];
                if (cmd !== 'c' && cmd !== 'connect' && cmd !== 'b' && cmd !== 'browse' && cmd !== 'e' && cmd !== 'enable' && cmd !== 'd' && cmd !== 'disable') {
                    console.log('Invalid parameters. Follwing is possible: enable, browse, connect');
                    callback(1);
                } else {
                    ...
                    if (cmd === 'e' || cmd === 'enable') {
                        ...
                        if (params.secure !== undefined && config.multihostService.secure !== params.secure) {
                            changed = true;
                    ...
                    if (cmd === 'd' || cmd === 'disable') {
                        ...
                        if (config.objects.type === 'file' && config.objects.host !== '127.0.0.1' && config.objects.host !== 'localhost') {
                            console.log('Server accept connections for objects only on localhost.');
                            config.objects.host = '127.0.0.1';
                            changed = true;
                        } else if (config.objects.type === 'redis') {
                            console.log('Please check the binding of redis service.')
                        }
                        if (config.states.type === 'file' && config.states.host !== '127.0.0.1' && config.states.host !== 'localhost') {
                            console.log('Server accept connections for states only on localhist.');
                            config.states.host = '127.0.0.1';
                            changed = true;
                        } else if (config.states.type === 'redis') {
                            console.log('Please check the binding of redis service.')
                        }
                            ...
                            if (err) {
                                ...
                                if (!list || !list.length) {
                                    ...
                                    rl.question('Please select host [1]: ', function (answer) {
                                        ...
                                        if (!list[answer]) {
                                            rl.close();
                                            console.error('Invalid index: ' + answer);
                                            callback(1);
                                        } else {
                                            if (list[answer].auth) {
                                                ...
                                                var schema = {
                                                    properties: {
                                                        password: {
                                                            description: 'Enter admin password (you have 30 seconds):',
                                                            pattern:     /^[^'"]*$/,
                                                ...
                                                prompt.get(schema, function (err, password) {
                                                    if (password && password.password) {
                                                        mhClient.connect(list[answer].ip, password.password, function (err, oObjects, oStates) {
                                                            if (err) {
                                                                console.error('Cannot connect to "' + list[answer].ip + '": ' + err);
                                                                callback(1);
        ...
        default:
            if (params.v || params.version) {
                ...
                if (command) {
                    try {
                        iopckg = require('iobroker.' + command + '/package.json');
                    } catch (err) {
