...
function setup(callback, ignoreIfExist) {
    ...
    if (!fs.existsSync(__dirname + '/../conf/iobroker.json')) {
        ...
        config.dataDir    = __dirname.substring(-1 * '/lib'.length) + '/data';
...
function dbConnect(callback) {
    ...
    var timeout = setTimeout(function () {
        ...
        objects = new Objects({
            ...
            connected: function () {
                console.log('js-controller is not running.')
...
function deleteAdapter(adapter, callback) {
            ...
            if (doc.rows.length !== 0) {
                ...
                for (var i = 0; i < doc.rows.length; i++) {
                    ...
                    if (!adapterConf.common.noRepository) {
                        ...
                        if (fs.existsSync(__dirname + '/../../iobroker.' + adapter)) {
    ...
    if (fs.existsSync(__dirname + '/../../iobroker.' + adapter + '/io-package.json')) {
