...
function initYargs() {
    yargs = require('yargs')
        ...
        .command(['restart', 'r'], 'Restarts js-controller', yargs => {
            yargs
                .command('<adapter>[.<instance>]', 'Restarts a specified adapter instance', {});
        })
        .command('debug <adapter-or-instance>', 'Starts a Node.js debugging session for the adapter instance', {
            ip: {
        ...
        .command(['add <adapter> [<desiredNumber>] [<repositoryUrl>]', 'a <adapter> [<desiredNumber>] [<repositoryUrl>]'], 'Add instance of adapter', {
            enabled: {
        ...
        .command(['install <adapter> [<desiredNumber>] [<repositoryUrl>]', 'i <adapter> [<desiredNumber>] [<repositoryUrl>]'], 'Installs a specified adapter', {})
        ...
        .command(['delete <adapter> [<instance>]', 'del <adapter> [<instance>]'], 'Remove adapter or/and instance', {})
        ...
        .command(['message <adapter-or-instance> <command> [<message>]', 'msg <adapter> <command> [<message>]'], 'Send message to adapter instance/s', {})
        ...
        .command(['status [all|<adapter>.<instance>]', 'isrun [all|<adapter>.<instance>]'], 'Status of ioBroker or adapter instance', {})
...
function processCommand(command, args, params, callback) {
    ...
    switch (command) {
        ...
        case 'i': {
            ...
            let name =      params.adapter;
            let instance =  params.desiredNumber;
        ...
        case 'rm': {
            ...
            if (!pattern) {
                console.log('No file path found. Example: "rm /vis.0/main/*"');
                return void callback(EXIT_CODES.INVALID_ARGUMENTS);
            }
            dbConnect(params, () => {
                // extract id
        ...
        case 'set': {
            const instance = params.adapterInstance;
            if (!instance) {
        ...
        case 'f': {
            ...
            dbConnect(params, async (objects, _states, isOffline, objectType) => {
                if (cmd === 'read' || cmd ==='r') {
                    const toRead = params.iobrokerPathToRead;
                    const parts = toRead.replace(/\\/g, '/').split('/');

                    const path = (params.filesystemPathToWrite || process.cwd()).replace(/\\/g, '/').split('/');
                    ...
                    const toRead = params.filesystemPathToRead || '';
                    const parts = toRead.replace(/\\/g, '/').split('/');

                    const path = (params.iobrokerPathToWrite || '').replace(/\\/g, '/').split('/');
                    ...
                    const toDelete = params.iobrokerPathToDelete;
        ...
        case 'repo': {
            ...
            const repoName       = params.name; // Repo url or name
            let repoUrl          = params.url; // Repo url or name
            if (repoUrlOrCommand !== 'add' && repoUrlOrCommand !== 'del' && repoUrlOrCommand !== 'set' && repoUrlOrCommand !== 'show' && repoUrlOrCommand !== 'addset') {
                repoUrl = params.name || repoUrlOrCommand;
