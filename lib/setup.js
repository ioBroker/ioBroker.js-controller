        ...
        if (!fs.existsSync(__dirname + '/../node_modules/iobroker.' + name) && !fs.existsSync(__dirname + '/../adapter/' + name)) {
            downloadPacket(repoUrl, name, function () {
                if (yargs.argv._[0] != 'install') {
                    dbConnect(function () {
                        createInstance(name, yargs.argv.enabled, yargs.argv.host || hostname, function () {
                            process.exit();
                        });
                    });
                }
...
function downloadPacket(repoUrl, packetName, callback) {
    ...
    if (sources[packetName]) {
        ...
        if (!url && packetName != 'example') {
            ...
            if (fs.existsSync(__dirname + '/../../iobroker.' + packetName)) {
                cmd += 'install ';
            } else if (fs.existsSync(__dirname + '/../node_modules/iobroker.' + packetName)) {
                cmd += 'install '; //'update ';
            } else if (fs.existsSync(__dirname + '/../../../node_modules/iobroker.js-controller')) {
            ...
            if (doc.rows.length !== 0) {
                ...
                for (var i = 0; i < doc.rows.length; i++) {
                    ...
                    if (fs.existsSync(__dirname + '/../node_modules/iobroker.' + adapter + "/web/") && adapterConf.common.webservers) {
                        if (typeof adapterConf.common.webservers == "string") adapterConf.common.webservers = [adapterConf.common.webservers];
                        for (var r = 0; r < adapterConf.common.webservers.length; r++) {
                            if (fs.existsSync(__dirname + '/../adapter/' + adapterConf.common.webservers[r] + '/' + adapter + '/')) {
                                // Delete files from adapter/<name>/web to adapter/<webName>/
                                console.log('delete ' + __dirname + '/../adapter/' + adapterConf.common.webservers[r] + '/' + adapter + '/');
                                tools.rmdirRecursiveSync(__dirname + '/../adapter/' + adapterConf.common.webservers[r] + '/' + adapter + '/');
                            }
                            if (fs.existsSync(__dirname + '/../node_modules/iobroker.' + adapterConf.common.webservers[r] + '/' + adapter + '/')) {
                                // Delete files from adapter/<name>/web to adapter/<webName>/
                                console.log('delete ' + __dirname + '/../node_modules/iobroker.' + adapterConf.common.webservers[r] + '/' + adapter + '/');
                                tools.rmdirRecursiveSync(__dirname + '/../node_modules/iobroker.' + adapterConf.common.webservers[r] + '/' + adapter + '/');
                            }
                        }
                    }
