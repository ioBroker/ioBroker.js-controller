    ...
    switch (command) {
        ...
        case 'host':
            (function () {
                ...
                let newHostname;
                if (!change) {
                    console.warn('Please write "' + tools.appName + ' host this" to use actual hostname ("' + tools.getHostName() + '") in ' + tools.appName + ' for all instances.');
                    callback(1);
                }
                ...
                if (change === 'set') {
                    ...
                    newHostname = args[1];
                    if (!newHostname) {
                        console.error('To change host name call: "' + tools.appName + ' host set <newHostname>"');
                        callback(34);
                    }
                    ...
                    if (!oldHostname) {
                        console.error('Host to remove is not defined. Usage: "' + tools.appName + ' host remove <hostname>"');
                        callback(34);
                    }
                } else {
                ...
                dbConnect(params, (_objects, _states, isOffline) => {
                    ...
                    objects.getObjectList({startkey: 'system.host.', endkey: 'system.host.\u9999'}, (err, objs) => {
                        if (!oldHostname) {
                            ...
                            if (hostCount > 1) {
                                console.warn('More than one host found. Please write "' + tools.appName + ' host <oldHostname> to use actual hostname ("' + tools.getHostName() + '") in ' + tools.appName + ' for related instances.');
                                callback(30);
                            }
