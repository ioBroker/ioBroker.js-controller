...
function processCommand(command, args, params, callback) {
    ...
    switch (command) {
        ...
        case 'i': {
            ...
            if (!cli.tools.validateAdapterOrInstanceIdentifier(name)) {
                showHelp();
            ...
            if (cmd !== 'c' && cmd !== 'connect' && cmd !== 's' && cmd !== 'status' && cmd !== 'b' && cmd !== 'browse' && cmd !== 'e' && cmd !== 'enable' && cmd !== 'd' && cmd !== 'disable') {
                ...
                dbConnect(params, (objects, states, isOffline) => {
                    ...
                    if (cmd === 'b' || cmd === 'browse') {
                        ...
                        if (params.persist) {
                            // if persist -> activate discovery server in config json
                            mh.enable(true, err => {
                                if (err) {
                                    console.error(err);
                                    return void callback(1);
                            ...
                            // we cannot start the server immediately if controller is not running
                            console.log(`${tools.appName} is not running. Please start the controller first or use the "persist" flag, to activate the discovery server permanently`);
                            return void callback(100);
                        } else {
                            // else just send it to the controller with temp information
                            states.pushMessage(`system.host.${tools.getHostName()}`, {
                                command: 'updateMultihost',
                                message: {temp: true},
                                from: 'setup'
