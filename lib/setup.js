...
switch (yargs.argv._[0]) {
    ...
    case 'list':
        (function () {
            var type = yargs.argv._[1];
            dbConnect(function () {
                switch (type) {
                    case 'objects':
                    case 'o':
                        objects.getObjectList(null, function (err, objs) {
                            var reg = yargs.argv._[2] ? new RegExp(pattern2RegEx(yargs.argv._[2])) : null;
                    ...
                    case 'states':
                    case 's':
                        states.getKeys(yargs.argv._[2] || null, function (err, keys) {
                            if (err) {
                                console.error(err);
                                processExit(23);
                            }
                    ...
                    case 'a':
                        objects.getObjectList({startkey: 'system.adapter.', endkey: 'system.adapter.\u9999'}, function (err, objs) {
                            var reg = yargs.argv._[2] ? new RegExp(pattern2RegEx('system.adapter.' + yargs.argv._[2])) : null;
                            for (var i = 0; i < objs.rows.length; i++) {
                    ...
                    case 'u':
                        objects.getObjectList({startkey: 'system.user.', endkey: 'system.user.\u9999'}, function (err, objs) {
                            var reg = yargs.argv._[2] ? new RegExp(pattern2RegEx('system.user.' + yargs.argv._[2])) : null;
                            for (var i = 0; i < objs.rows.length; i++) {
                    ...
                    case 'g':
                        objects.getObjectList({startkey: 'system.group.', endkey: 'system.group.\u9999'}, function (err, objs) {
                            var reg = yargs.argv._[2] ? new RegExp(pattern2RegEx('system.group.' + yargs.argv._[2])) : null;
                            console.log('');
                            ...
                            for (var i = 0; i < objs.rows.length; i++) {
                                ...
                                if (!reg || reg.test(objs.rows[i].value._id) || (objs.rows[i].value.common && reg.test(objs.rows[i].value.common.name))) {
                                    ...
                                    if (objs.rows[i].value.common.acl && objs.rows[i].value.common.acl.user) {
                                        text += ' ';
                                        text += (objs.rows[i].value.common.acl.user.write  ? '+': '-') + ' ';
                                        text += (objs.rows[i].value.common.acl.user.create ? '+': '-') + ' ';
                                        text += (objs.rows[i].value.common.acl.user.delete ? '+': '-') + ' ';
                                        text += '|';
                                    } else {
                                        text += '       |';
                                    }
                    ...
                    case 'e':
                        objects.getObjectList({startkey: 'enum.', endkey: 'enum.\u9999'}, function (err, objs) {
                            var reg = yargs.argv._[2] ? new RegExp(pattern2RegEx('enum.' + yargs.argv._[2])) : null;
                            for (var i = 0; i < objs.rows.length; i++) {
                    ...
                    case 'f':
                        objects.getObjectList({startkey: 'system.adapter.', endkey: 'system.adapter.\u9999'}, function (err, objs) {
                            var adapter = yargs.argv._[2] || null;;

                            var adapters = [];
                            for (var i = 0; i < objs.rows.length; i++) {
                                if (objs.rows[i].value.type != 'adapter') continue;
                                if (adapter && objs.rows[i].value.common.name != adapter) continue;
                                adapters.push(objs.rows[i].value.common.name);
                            }
                            objects.getObjectList({startkey: 'system.adapter.', endkey: 'system.adapter.\u9999'}, function (err, objs) {
                                for (var i = 0; i < objs.rows.length; i++) {
                                    if (objs.rows[i].value.type != 'instance') continue;
                                    if (adapter && objs.rows[i].value._id.substring(15) != adapter) continue;
                                    adapters.push(objs.rows[i].value._id.substring(15));
                                }
                                listAdaptersFiles(adapters, function () {
                                    setTimeout(function () {
                                        processExit();
                                    }, 1000);
                                });
...
function listAdaptersFiles(adapters, callback) {
    if (adapters && adapters.length) {
        var adapter = adapters.pop();
        listDirectory(adapter, '', function () {
            listDirectory(adapter + '.admin', '', function () {
                listAdaptersFiles(adapters, callback);
            });
        });
    } else {
...
function listDirectory(adapter, path, callback) {
    objects.readDir(adapter, path, null, function (err, files) {
        ...
        for (var f = 0; f < files.length; f++) {
            ...
            var text = '';
            if (files[f].acl){
                text += (files[f].isDir ? 'd' : '-') + perm2str(files[f].acl.permissions || 0);
                var owner = files[f].acl.owner;
                if (owner) {
                ...
                text += ' ' + owner;
                var group = files[f].acl.ownerGroup;
                if (group) {
                ...
                text += (files[f].isDir ? 'd' : '-') + '?????????' + Array(31).join(' ');
            }
            var size = files[f].stats.size.toString();
            if (size.length < 6) size = Array(6 - size.length).join(' ') + size;

            text += ' ' + size + ' ' + adapter + path + '/' + files[f].file;
            if (files[f].isDir) {
                text += '/';
                console.log(text);
                count++;
                listDirectory(adapter, path + '/' + files[f].file, function () {
                    count--;
