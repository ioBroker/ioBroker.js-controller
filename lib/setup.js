...
function setup(callback, ignoreIfExist) {
    ...
    if (!fs.existsSync(tools.getConfigFileName())) {
        ...
        mkpathSync('', config.dataDir);
        // Create default data dir
        fs.writeFileSync(tools.getConfigFileName(), JSON.stringify(config));

        try {
            // Create iobroker.sh and bat
            if (config.dataDir.toLowerCase().indexOf('iobroker.js-controller') == -1) {
                ...
                if (platform == 'linux' || platform == 'darwin') {
                    fs.writeFileSync(path + '/iobroker', "node node_modules/iobroker.js-controller/iobroker.js $1 $2 $3	$4 $5');");
                    console.log('Write "sudo bash iobroker start" to start the ioBroker');
