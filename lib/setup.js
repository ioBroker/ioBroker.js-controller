...
function processCommand(command, args, params, callback) {
    ...
    switch (command) {
        ...
        case 'object': {
            ...
            const CLIObjects = require('./cli/cliObjects.js');
            const objectsCommand = new CLIObjects({
                objects,
                callback,
                states,
            });
            ...
            if (cmd === 'chmod') {
                // TODO: All this argument parsing should probably be part of cliObjects.js
                ...
                dbConnect(params, () => {
                    objects.chmodObject(pattern, {user: 'system.user.admin', object: modeObject, state: modeState}, (err, processed) => {
                        ...
                        objectsCommand.printObjectList(err, processed);
                    });
                });
            } else
            if (cmd === 'chown') {
            ...
            if (cmd === 'list' || cmd === 'l') {
                ...
                dbConnect(params, () => {
                    objects.getObjectList(pattern, {user: 'system.user.admin', sorted: true}, (err, processed) => {
                        objectsCommand.printObjectList(
                            err,
            ...
            if (id) {
                if (cmd === 'get') {
                    ...
                    let propPath = args[2];
                    dbConnect(params, () => {
                        objectsCommand.get(id, propPath);
                    });
                } else if (cmd === 'set') {
                    ...
                    dbConnect(params, () => {
                        objectsCommand.set(id, propPath, value);
                    });
                } else if (cmd === 'del' || cmd === 'delete') {
                    // iobroker object del objectid
                    // Deletes an object
                    dbConnect(params, () => {
                        objectsCommand.delete(id);
                    });
                } else {
                    cli.error.unknownCommand('object', cmd);
                    return void callback(3);
                }
            }
            break;
            ...
            if (id) {
                dbConnect(params, () => {
                    if (cmd === 'get') {
                        ...
                        cli.error.unknownCommand('state', 'cmd');
                        showHelp();
                        return void callback(1);
                    }
                });
