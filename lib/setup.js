    ...
    switch (command) {
        ...
        case 'host':
            (function () {
                ...
                let newHostname;
                if (!change) {
                    console.warn('Parameter missing. To set current hostname use: "' + tools.appName + ' host this"');
                    callback(1);
                }
                ...
                if (change === 'set') {
                    ...
                    newHostname = args[1];
                    if (!newHostname) {
                        console.error('Parameter missing. To set new hostname use: "' + tools.appName + ' host set <newHostname>"');
                        callback(34);
                    }
                    ...
                    if (!oldHostname) {
                        console.error('Paramter missing. To remove host use: "' + tools.appName + ' host remove <oldHostname>"');
                        callback(34);
                    }
                } else {
                ...
                dbConnect(params, (_objects, _states, isOffline) => {
                    ...
                    objects.getObjectList({startkey: 'system.host.', endkey: 'system.host.\u9999'}, (err, objs) => {
                        if (!oldHostname) {
                            ...
                            if (hostCount > 1) {
                                console.warn('More than one host found. Specify old hostname to rename to current hostname. Use: "' + tools.appName + ' host <oldHostname>"');
                                callback(30);
                            }
