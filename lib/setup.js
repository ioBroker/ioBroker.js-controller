        ...
        states.getKeys('io.*', function (err, keys) {
            ...
            states.getStates(keys, function (err, obj) {
                ...
                for (var i = 0; i < result.objects.length; i++) {
                    // Read all files
                    if (result.objects[i].value.type == 'meta' && result.objects[i].value.common && result.objects[i].value.common.type == 'meta.user'){
                        count++;
                        copyDir(result.objects[i].id, '', __dirname + '/../tmp/backup/files/' + result.objects[i].id, function () {
                            count--;
                            if (!count) {
...
function reloadAdaptersObjects(callback, dirs, index) {
    if (!dirs) {
        ...
        if (fs.existsSync(__dirname + '/../../../node_modules/iobroker.js-controller')) {
            ...
            for (var i = 0; i < _modules.length; i++) {
                if (_modules[i].match(/^iobroker\./i &&
