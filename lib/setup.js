...
function initYargs() {
    yargs = require('yargs')
            .usage('Commands:\n' +
                ...
                tools.appName + ' update [repository url] [--updatable/--u] [--installed/--i] [--force/--f]\n' +
    ...
    switch (command) {
        ...
        case 'isrun':
            (() => {
                dbConnect(params, (objects, states, isOffline) => {
                    ...
                    let exitCode = isOffline ? 100 : undefined;
                    if (isOffline) {
        ...
        case 'host':
            (function () {
                ...
                let newHostname;
                if (!change) {
                    console.warn('Parameter missing. To set hostname for ' + tools.appName + ' to "' + tools.getHostName() + '" use: "' + tools.appName + ' host this"');
                    callback(1);
                }
                ...
                if (change === 'set') {
                    ...
                    newHostname = args[1];
                    if (!newHostname) {
                        console.error('Parameter missing. To set new hostname instead of "' + oldHostname + '" use: "' + tools.appName + ' host set <hostname>"');
                        callback(34);
                    }
                    ...
                    if (!oldHostname) {
                        console.error('Paramter missing. To remove host use: "' + tools.appName + ' host remove <hostname>"');
                        callback(34);
                    }
                } else {
                ...
                dbConnect(params, (_objects, _states, isOffline) => {
                    ...
                    objects.getObjectList({startkey: 'system.host.', endkey: 'system.host.\u9999'}, (err, objs) => {
                        if (!oldHostname) {
                            ...
                            if (hostCount > 1) {
                                console.warn('More than one host found. Specify the old hostname that is going to be renamed to "' + tools.getHostName() + '". Use: "' + tools.appName + ' host <hostname>"');
                                callback(30);
                            }
                        ...
                        for (let i = 0; i < objs.rows.length; i++) {
                            ...
                            if (obj.type === 'host') {
                                if (hName === newHostname && change !== 'remove') {
                                    hostCount++;
                                    if (hostCount > 1) {
                                        console.error('Host with current hostname "' + hName + '" found. Cannot rename it.');
                                        continue;
                                    }
                                }
                            }
                ...
                objects = new Objects({
                    ...
                    connected: () => {
                        ...
                        if (isStatesConnected && typeof callback === 'function') {
                            callback(objects, states, config.objects.type === 'file', config.objects.type);
                        }
                    }
                });
            } else {
