                                    ...
                                    rl.question('Port of states DB (' + stype + '), default[' + sp + ']: ', function (sport) {
                                        ...
                                        if ((stype == 'file' && (shost == 'localhost' || shost == '127.0.0.1')) ||
                                            ...
                                            rl.question('Data directory (file), default[../' + tools.getDefaultDataDir() + ']: ', function (dir) {
                                                ...
                                                rl.close();
                                                var config = require(__dirname + '/../conf/' + tools.appName + '-dist.json');
            ...
            if (name) {
                dbConnect(function () {
                    ...
                    if (name == 'all') {
                        ...
                        if (name.indexOf('.') !== -1) {
                            if (!subTree) {
                                console.log('Please specify target name, like:\n iorboker /file/picture.png /vis.0/main/img/picture.png');
                                processExit(1);
                            }
            ...
            dbConnect(function (_objects, _states, isOffline) {
                ...
                objects.getObjectList({startkey: 'system.host.', endkey: 'system.host.\u9999'}, function (err, objs) {
                    ...
                    for (var i = 0; i < objs.rows.length; i++) {
                        ...
                        if (obj.type == 'host') {
                            if ((hName == hostname && (change === 'this' || change === 'self')) || change == hName) {
                                hostCount++;
                                if (hostCount > 1) {
                        ...
                        if (change !== 'this' && change != hName && change !== 'self') continue;
                    ...
                    objects.getObjectList({startkey: 'system.adapter.', endkey: 'system.adapter.\u9999'}, function (err, objs) {
                        for (var i = 0; i < objs.rows.length; i++) {
                            if (objs.rows[i].value.type != 'instance') continue;
                            if (change !== 'this' && change !== 'self' && change != objs.rows[i].value.common.host) continue;
...
function restartController(callback) {
    ...
    var out;
    var err;
    var dir = tools.getDefaultDataDir();
    var parts = dir.split('/');
    parts.pop();
    dir = parts.join('/');

    try {
        if (!fs.existsSync(dir + '/log')) fs.mkdirSync(__dirname + '/../log');
        if (fs.existsSync(dir + '/log/restart.log')) fs.unlinkSync(dir + '/log/restart.log');
		out = fs.openSync(dir + '/log/restart.log', 'a');
        err = fs.openSync(dir + '/log/restart.log', 'a');
    } catch (error) {
        console.error(error);
        out = 'ignore';
        err = 'ignore';
