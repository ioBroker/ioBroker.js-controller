                    ...
                    for (var i = 0; i < objs.rows.length; i++) {
                        ...
                        if (obj.type == 'host') {
                            if (hName == hostname || change == 'this' || change == hName) {
                                hostCount++;
                                if (hostCount > 1) {
                                    console.err('More than one host found. Cannot rename.');
                                    continue;
                                }
                            }
                        }
                        ...
                        if (hName != hostname) {
                            if (obj.type == 'host') {
                                ...
                                objects.delObject(obj._id, function (err) {
                                    if (err) console.err('Cannot delete object: ' + err);
                                    if (!--count) processExit();
                                });
                                ...
                                objects.setObject(obj._id, obj, function (err) {
                                    if (err) console.err('Cannot set object: ' + err);
                                    if (!--count) processExit();
                                });
                            } else {
                                // state
    ...
    case 'visdebug':
        (function () {
            ...
            dbConnect(function (_objects, _states, isOffline) {
                ...
                _objects.writeFile('vis', 'index.html', file);

                file = fs.readFileSync(visDir + '/www/edit.html.original');
                _objects.writeFile('vis', 'edit.html', file);
                ...
                _objects.writeFile('vis', 'cache.manifest', file);
                ...
                if (widgetset) {
                    _objects.readFile('vis', 'js/config.js', function (err, data) {
                        data = data.replace(/[\r\n|\r|\n]/g, '');
                        ...
                        if (!found) {
                            json.push(widgetset);
                            data =  data.replace(/"widgetSets":\s+.*};/, JSON.stringify(json, null, 2));
                            _objects.writeFile('vis', 'js/config.js', '"widgetSets":' + data + '};', function () {
                                // upload all files into vis
                                upload(adapterDir + '/widgets', 'vis', 'widgets/', function () {
                                    processExit();
                                });
                            });
                        } else {
...
function upload(dir, adapter, path, callback) {
    ...
    for (var d = 0; d < dirs.length; d++) {
        ...
        if (stat.isDirectory()) {
            count++;
            upload(dir + '/' + dirs[d], adapter, path + '/' + dirs[d], function () {
                count--;
