                    ...
                    if (cmd == "get") {
                        ...
                        if (!val) {
                            ...
                            if (ack !== undefined) {
                                ...
                                states.setState(id, val, ack, function () {
                                    processExit();
                                });
                            }
                        }
        ...
        states.getKeys('io.*', function (err, keys) {
            ...
            states.getStates(keys, function (err, obj) {
                ...
                for (var j = 0; j < result.objects.length; j++) {
                    // Read all files
                    if (result.objects[i].value.type == 'meta' &&
                        result.objects[i].value.common &&
                        result.objects[i].value.common.type == 'meta.user'){
                        count++;
...
function downloadPacket(repoUrl, packetName, useSystemNpm, callback) {
    ...
    if (!useSystemNpm) {
        var config = require(tools.getConfigFileName());
        if (config.network.useSystemNpm) useSystemNpm = true;
    }
...
function installNpm(adapter, callback) {
    ...
    if (fs.existsSync(path + '/package.json')) {
        ...
        if (config.network.useSystemNpm) {
...
function uuid(isMac) {
    ...
    if (isMac) {
        ...
        for (var n in ifaces) {
            for (var a = 0; a < ifaces[n].length; a++) {
                if (ifaces[n][a].mac && ifaces[n][a].mac != '00:00:00:00:00:00') {
                    mac = ifaces[n][a].mac;
                    break;
                }
            }
