    ...
    case 'object':
        (function () {
            ...
            if (cmd == "chmod") {
                ...
                dbConnect(function () {
                    objects.chmodObject(pattern, {user: 'system.user.admin', object: mode}, function (err, processed) {
                        if (err) {
                            console.error(err);
                        } else {
                            if (processed) {
            ...
            if (cmd == "list" || cmd == "l") {
                ...
                dbConnect(function () {
                    objects.getConfigKeys(pattern, {user: 'system.user.admin'}, function (err, keys) {
                        if (err) {
                            console.error(err);
                            processExit(33);
                        }
                        objects.getObjects(keys, {user: 'system.user.admin'}, function (err, processed) {
                            if (err) {
                                console.error(err);
                                processExit(33);
                            }
