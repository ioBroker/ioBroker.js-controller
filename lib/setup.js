    ...
    case 'stop':
        (function () {
            ...
            if (yargs.argv._[1]) {
                ...
                if (adapter.indexOf('.') == -1 && adapter != 'all') {
                    ...
                    if (adapter == 'all') {
                        objects.getObjectList({startkey: 'system.adapter.', endkey: 'system.adapter.\u9999'}, function (err, objs) {
                            ...
                            for (var i = 0; i < objs.rows.length; i++) {
                                ...
                                count++;
                                objects.getObject(objs.rows[i].id, function (err, obj) {
                                    if (!err && obj) {
                ...
                dbConnect(function () {
                    if (name == 'all') {
                        objects.getObjectList({startkey: 'system.adapter.', endkey: 'system.adapter.\u9999'}, function (err, objs) {
                            ...
                            uploadAdapterFull(adapters, function () {
                                processExit();
                            });
                        });
                    } else {
                        if (subTree) {
                            uploadAdapter(name, false, true, subTree, function () {
                                processExit();
                            });
                        } else {
                            uploadAdapter(name, true, true, function () {
                ...
                dbConnect(function () {
                    objects.chmodObject(pattern, {user: 'system.user.admin', object: modeObject, state: modeState}, function (err, processed) {
                        if (err) {
                            ...
                            if (processed) {
                                var List = require(__dirname + '/setupList.js');
                                var list = new List({
                                    states:      states,
                                    objects:     objects,
    ...
    case 'list':
        (function () {
            ...
            dbConnect(function () {
                ...
                list.list(yargs.argv._[1], yargs.argv._[2]);
            });
        })();
        break;
                    ...
                    if (!password) {
                        ...
                        prompt.get(schema, function (err, result) {
                            if (result) {
                                ...
                                //create user
                                addUser(user, result.password, function (err) {
                                    if (err) {
                                        console.log(err);
                                        processExit(30);
                                    } else {
                                        addUserToGroup(user, group, function (err) {
                                            if (err) {
                                                console.log(err);
                                                processExit(30);
                                            } else {
                        ...
                        addUser(user, password, function (err) {
                            if (err) {
                                console.log(err);
                                processExit(30);
                            } else {
                ...
                if (!password) {
                    ...
                    prompt.get(schema, function (err, result) {
                        if (result) {
                            ...
                            //create user
                            setPassword(user, pass, function (err) {
                                if (err) {
                                    console.log(err);
                                    processExit(30);
                                } else {
                    ...
                    setPassword(user, password, function (err) {
                        if (err) {
                            console.log(err);
                            processExit(30);
                        } else {
...
function uploadAdapterFull(adapters, callback) {
    ...
    uploadAdapter(adapter, true, true, function () {
        uploadAdapter(adapter, false, true, function () {
            setTimeout(function () {
                uploadAdapterFull(adapters, callback);
            }, 0);
        });
    });
}
...
function uploadAdapter(adapter, isAdmin, forceUpload, subTree, callback) {
    var rev;
        ...
        objects.getObjectView("system", "instance", {}, null, function (err, objs) {
            if (objs.rows.length) {
                for (var i = 0; i < deps.length; i++) {
                    ...
                    if (dName == 'js-controller') {
                        ...
                        if (version !== null) {
                            ...
                            if (!semver.satisfies(iopkg.version, version)) {
                                console.log('Invalid version of "' + dName + '". Installed "' + iopkg.version + '", required "' + version);
                                processExit(30);
                            } else {
                                isFound = true;
                            }
                            ...
                            if (objs.rows[t] && objs.rows[t].value && objs.rows[t].value.common && objs.rows[t].value.common.name == dName) {
                                ...
                                if (version !== null) {
                                    ...
                                    if (!semver.satisfies(objs.rows[t].value.common.version, version)) {
                                        console.log('Invalid version of "' + dName + '". Installed "' + objs.rows[t].value.common.version + '", required "' + version);
                                        processExit(30);
                                    } else {
                                        isFound = true;
                                    }
                    ...
                    if (!isFound) {
                        cnt++;
                        createInstance(dName, _options, function (name) {
    ...
    objects.getObjectView("system", "device", {}, function (err, doc) {
        if (err) {
            ...
            if (doc.rows.length !== 0) {
                ...
                var name = adapter + '.' + instance;
                for (var i = 0; i < doc.rows.length; i++) {
                    if (doc.rows[i].value._id && doc.rows[i].value._id.substring(0, name.length) == name) {
                        if (delObj.indexOf(doc.rows[i].value._id) == -1) delObj.push(doc.rows[i].value._id);
                        count++;
                    } else
                    if (doc.rows[i].value._id && doc.rows[i].value._id.substring(0, ("system.adapter." + name).length) == "system.adapter." + name) {
                        if (delObj.indexOf(doc.rows[i].value._id) == -1) delObj.push(doc.rows[i].value._id);
                        count++;
                    }
                }
