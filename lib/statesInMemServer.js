...
function StatesInMemory(settings) {
    ...
    this.setState = function (id, state, callback) {
        ...
        if (expire) {
            ...
            if (!expiresInterval) {
                ...
                if (lastExpire) expires[id] = expire - (new Date().getTime() - lastExpire);
            }
        }
        states[id] = obj;
    ...
    this.setSession = function (id, expire, obj, callback) {
        session[id] = obj || {};
        session[id]._expire = expire;
        if (!expiresInterval) {
            lastExpire = (new Date()).getTime();
            expiresInterval = setInterval(expiresCheck, 5000);
        } else {
            if (lastExpire) session[id]._expire = expire - (new Date().getTime() - lastExpire);
        }
