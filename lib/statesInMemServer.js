...
function StatesInMemory(settings) {
    ...
    if (dataDir && dataDir[0] == '.' && dataDir[1] == '.') dataDir = __dirname + '/../' + dataDir;
    ...
    function _initWebServer(settings, server) {
        try {
            if (settings.secure) {
                ...
                }).listen(settings.port || 9000, (settings.host && settings.host != '127.0.0.1' && settings.host != 'localhost') ? settings.host : undefined);
            } else {
