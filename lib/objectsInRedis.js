...
function ObjectsInMem(settings) {
        ...
        states.getConfig(id, function (err, _obj) {
            ...
            if (_obj) {
                ...
                states.setConfig(id, _obj, function () {
                    count--;
                    if (!count && callback) callback(null, null);
                });
            } else if (!count && callback) {
                callback(null, null);
            }
        });
    ...
    this.getObjectView = function (design, search, params, callback) {
        if (!objects) {
            if (processObjects) {
                setTimeout(function () {that.getObjectView(design, search, params, callback);}, 250);
            } else {
                processObjects = true;
    ...
    this.getObjectList = function (params, callback) {
        if (!objects) {
            if (processObjects) {
                setTimeout(function () {that.getObjectList(params, callback);}, 250);
            } else {
                processObjects = true;
                this._getObjects(function () {
                    processObjects = false;
