...
function Adapter(options) {
    ...
    if (fs.existsSync(__dirname + '/../node_modules/iobroker.' + options.name + '/io-package.json')) {
        this.ioPack = JSON.parse(fs.readFileSync(__dirname + '/../node_modules/iobroker.' + options.name + '/io-package.json'));
    } else if (fs.existsSync(__dirname + '/../adapter/' + options.name + '/io-package.json')) {
        this.ioPack = JSON.parse(fs.readFileSync(__dirname + '/../adapter/' + options.name + '/io-package.json'));
    }
    
    if (fs.existsSync(__dirname + '/../node_modules/iobroker.' + options.name + '/package.json')) {
        this.pack = JSON.parse(fs.readFileSync(__dirname + '/../node_modules/iobroker.' + options.name + '/package.json'));
    } else if (fs.existsSync(__dirname + '/../adapter/' + options.name + '/package.json')) {
        this.pack = JSON.parse(fs.readFileSync(__dirname + '/../adapter/' + options.name + '/package.json'));
    }
    ...
    initCouch(function () {
        if (that.inited) {
            ...
            if (that.log) that.log.error("Initiate called one more time!!! May be reconnection to DB.");
            return;
        }
        that.inited = true;
    ...
    function initCouch(cb) {
        ...
        that.deleteChannelFromEnum = function deleteChannelFromEnum(enumName, parentDevice, channelName, callback) {
            ...
            that.objects.getObjectView('system', 'enum', {startkey: enumName, endkey: enumName + '\u9999'}, function (err, res) {
                if (!err && res) {
                    ...
                    for (var i = 0; i < res.rows.length; i++) {
                        count++;
                        that.objects.getForeignObject(res.rows[i].id, function (err, obj) {
                            if (!err && obj && obj.common && obj.common.members) {
                                var pos = obj.common.members.indexOf(objId);
                                if (pos != -1) {
                                    obj.common.members.splice(pos, 1);
                                    count++;
                                    that.objects.setForeignObject(res.rows[i].id, obj, function (err) {
                                        count--;
        ...
        that.deleteChannel = function deleteChannel(parentDevice, channelName, callback) {
            ...
            channelName  = that.namespace + '.' + this._DCS2ID(parentDevice, channelName);
                        ...
                        that.objects.getObject(res.rows[i].id, function (err, obj) {
                            if (!err && obj && obj.common && obj.common.members) {
                                ...
                                if (pos != -1) {
                                    obj.common.members.splice(pos, 1);
                                    count++;
                                    that.objects.setObject(res.rows[i].id, obj, function (err) {
                                        count--;
                                        if (!count && callback) callback(err);
                                    });
                                }
        ...
        that.writeFile = function writeFile(filename, data, options, callback) {
            ...
            options = options || 'text/javascript';

            that.objects.writeFile(that.name, filename, data, options, callback);
        };
    }
    ...
    function initRedis(cb) {
        ...
        that.states = new StatesRedis({
            ...
            change: function (id, state) {
                ...
                if (that.logList && id.match(/\.logging$/)) {
                    that.logRedirect(state.val, id.substring(0, id.length - 'logging'.length));
                } else
                if (id == 'log.system.adapter.' + that.namespace) {
                    that.processLog();
                } else
                // If this is messagebox
                if (id == 'messagebox.system.adapter.' + that.namespace) {
                    // Read it from fifo list
                    that.states.getMessage('system.adapter.' + that.namespace, function (err, obj) {
                        if (obj) {
        ...
        that.sendTo = function sendTo(objName, command, message, callback) {
            ...
            if (objName.indexOf('.') == -1) {
                ...
                that.objects.getObjectView('system', 'instance', {startkey: objName + '.', endkey: objName + '.\u9999'}, function (err, obj) {
                    if (obj) {
                        for (var i = 0; i < obj.rows.length; i++) {
                            that.states.pushMessage(obj.rows[i].id, obj);
                        }
                    }
                });
            } else {
    ...
    function initLogging() {
        if (!options.logTransporter && !that.ioPack.common.logTransporter) {
            ...
            that.processLog = function () {
                that.states.getLog('system.adapter.' + that.namespace, function (err, obj) {
                    if (!err && obj) {
                        that.emit('log', obj);
    ...
    function initAdapter(adapterConfig) {
        ...
        that.log = new Log();
        if (options.instance === undefined) {
            that.version = that.pack ? that.pack.version : ((that.ioPack && that.ioPack.common) ? that.ioPack.common.version : 'unknown');
            that.log.info('starting. Version ' + that.version);
    ...
    function reportStatus() {
        ...
        that.states.setState(id + '.uptime', {val: process.uptime().toFixed(0), ack: true, from: id});
    }
