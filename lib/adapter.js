                ...
                that.getForeignObject('system.adapter.' + adapter, function (err, obj) {
                    if (!err && obj.common.children && obj.common.children.length) {
                        ...
                        for (var i = 0; i < obj.common.children.length; i++) {
                            that.pushMessage(obj.common.children + '.messagebox', {command: command, message: message});
                        }
                        if (callback) callback();
                    }
                });                
            } else {
                that.pushMessage('system.adapter.' + adapter + '.messagebox', {command: command, message: message}, callback);
            }
        };

        that.getMessage = function getMessage(callback) {
            that.states.getMessage('system.adapter.' + adapter + '.messagebox', callback);
        };

        that.lenMessage = function lenMessage(callback) {
            that.states.lenMessage('system.adapter.' + adapter + '.messagebox', callback);
        };
    ...
    function initRedis(cb) {
        ...
        that.states = new StatesRedis({
            ...
            change: function (id, state) {
                if (id == 'system.adapter.' + that.namespace + '.messagebox.changed') {
