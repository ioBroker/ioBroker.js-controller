...
function Adapter(options) {
    ...
    function initCouch(cb) {
        ...
        that._fixId = function _fixId(id) {
            if (typeof id == "object") {
                id = 'io.' + that.namespace + '.' + id.device + '.' + id.channel + '.' + id.state;
            } else {
        ...
        that.extendObject = function extendObject(id, obj, callback) {
            ...
            if (obj && (obj.children || obj.repositories || obj.certificates ||
        ...
        that.extendForeignObject = function extendForeignObject(id, obj, callback) {
            // delete arrays if they should be changed
            if (obj && (obj.children || (obj.common && (obj.common.repositories || obj.common.members || obj.common.certificates)))) {
                // Read whole object
                that.objects.getObject(id, function (err, oldObj) {
                    if (!oldObj) {
                        logger.error('Object ' + id + ' not exist!');
                    ...
                    if (obj.common && obj.common.repositories && oldObj.common && oldObj.common.repositories) {
                        oldObj.common.repositories = [];
                    }
                    if (obj.common && obj.common.members && oldObj.common && oldObj.common.members) {
                        oldObj.common.members = [];
                    }
                    if (obj.common && obj.common.certificates && oldObj.common && oldObj.common.certificates) {
                        oldObj.common.certificates = [];
                    }
                    obj = extend(true, oldObj, obj);
