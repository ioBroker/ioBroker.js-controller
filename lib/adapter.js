...
function Adapter(options) {
    ...
    if (fs.existsSync(__dirname + '/../adapter/' + options.name + '/io-package.json')) {
        // ioPack will be the indicator if adapter started from js-controller or from somewhere else
    ...
    function initCouch(cb) {
        ...
        that._fixId = function _fixId(id) {
            if (typeof id == "object") {
                id = 'io.' + that.namespace + '.' + (id.device ? id.device + '.' : '') + (id.channel ? id.channel + '.' : '') + id.state;
            } else {
                if (id.substring(0, 3) == 'io.') {
                    if (id.substring(0, that.namespace.length + 3) != 'io.' + that.namespace) {
                        id = 'io.' + that.namespace + '.' + id.substring(3);
                    }
                } else {
        ...
        that.extendForeignObject = function extendForeignObject(id, obj, callback) {
            // delete arrays if they should be changed
            if (obj && (obj.children || (obj.native && (obj.native.repositories || obj.native.certificates)) ||
                (obj.common && obj.common.members))) {
                // Read whole object
                that.objects.getObject(id, function (err, oldObj) {
                    if (!oldObj) {
        ...
        that.createChannel = function createChannel(parentDevice, channelName, states, roleOrCommon, native, callback) {
            ...
            if (parentDevice) obj.parent = that.namespace + '.' + parentDevice;
        ...
        that.createState = function createState(parentDevice, parentChannel, stateName, roleOrCommon, native, callback) {
            ...
            if (parentDevice) parentDevice  = parentDevice.replace(/[.\s]+/g, '_');
            parentChannel = (parentDevice ? (parentDevice  + '.') : '') + parentChannel.replace(/[.\s]+/g, '_');
            ...
            that.getObject(parentChannel, function (err, obj) {
                if (obj) {
                    if (obj.children) {
                        var pos = obj.children.indexOf('io.' + that.namespace + '.' + stateName);
                        if (pos == -1) {
                            obj.children.push('io.' + that.namespace + '.' + stateName);
            ...
            that.setObjectNotExists('io.' + stateName, {
                "type":     "state",
            ...
            if (common.def !== undefined) {
                that.setState('io.' + stateName, common.def);
            }
        };
        ...
        that.addStateToEnum = function addStateToEnum(enumName, addTo, parentDevice, parentChannel, stateName, callback) {
            ...
            var objId = 'io.' + that.namespace + '.' + stateName;
