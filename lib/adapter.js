...
function Adapter(options) {
    ...
    const extendObjects = (tasks, callback) => {
        ...
        try {
            tools.validateGeneralObjectProperties(task);
        } catch (e) {
        ...
        this.setState = (id, state, ack, options, callback) => {
            ...
            if (tools.isObject(state)) {
                ...
                try {
                    ...
                    this.log.warn(`{$this.namespaceLog} Please send this information to the developer: Warning on set State (will not be set in future!) {$id}: {$e.message}`);
                }
            } else {
    ...
    const initAdapter = adapterConfig => {
        initLogging(() => {
            ...
            this.pluginHandler.initPlugins(adapterConfig, () => {
                ...
                const promises = [];
                if (this.ioPack && Array.isArray(this.ioPack.encryptedNative)) {
                    for (const attr of this.ioPack.encryptedNative) {
                        // we can only decrypt strings
                        if (typeof this.config[attr] === 'string') {
                            promises.push(this.getEncryptedConfig(attr).then(decryptedValue => {
