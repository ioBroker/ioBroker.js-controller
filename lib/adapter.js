...
function Adapter(options) {
    ...
    function initObjects(cb) {
        ...
        that.findForeignObject = function findForeignObject(id, type, callback) {
        ...
        that.unsubscribeForeignObjects = function unsubscribeForeignObjects(pattern) {
            if (!pattern) pattern = '*';
            that.objects.unsubscribe(pattern);
        };

        that.unsubscribeObjects = function unsubscribeObjects(pattern) {
            if (pattern == '*') {
                that.objects.unsubscribe(that.namespace + '.*');
            } else {
                pattern = that._fixId(pattern);
                that.objects.unsubscribe(pattern);
            }
    ...
    function initStates(cb) {
        ...
        that.setState = function setState(id, state, ack, callback) {
        ...
        that.setForeignState = function setForeignState(id, state, ack, callback) {
        ...
        that.unsubscribeStates = function unsubscribeStates(pattern) {
            if (!pattern || pattern == '*') {
                that.states.unsubscribe(that.namespace + '.*');
            } else {
                pattern = that._fixId(pattern, 'state');
                that.states.unsubscribe(pattern);
            }
