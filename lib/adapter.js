...
function Adapter(options) {
            ...
            adapterObjects.getObjectView('system', 'state', {startkey: this.namespace + '.', endkey: this.namespace + '.\u9999', include_docs: true}, (err, _states) => {
                ...
                adapterObjects.getObjectView('system', 'channel', {startkey: this.namespace + '.', endkey: this.namespace + '.\u9999', include_docs: true}, (err, _channels) => {
                    ...
                    adapterObjects.getObjectView('system', 'device', {startkey: this.namespace + '.', endkey: this.namespace + '.\u9999', include_docs: true}, (err, _devices) => {
                        ...
                        adapterObjects.getObjectView('system', 'folder', {startkey: this.namespace + '.', endkey: this.namespace + '.\u9999', include_docs: true}, (err, _folders) => {
                            ...
                            if (_folders && _folders.rows) {
                                for (let f = _folders.rows.length - 1; f >= 0; f--) {
                                    objects[_folders.rows[f].id] = _folders.rows[f].value;
                                }
                            }
    ...
    const initStates = cb => {
        ...
        this._addAliasSubscribe = (aliasObj, pattern, callback) => {
            if (aliasObj && aliasObj.common && aliasObj.common.alias && aliasObj.common.alias.id) {
                ...
                return tools.maybeCallbackWithError(callback, new Error(`Alias ${aliasObj._id} is not state`));
            }
        };
    ...
    const checkLogging = () => {
        ...
        if (!adapterStates) return; // if adapterState was destroyed, we can not continue
    ...
    const initAdapter = adapterConfig => {
        initLogging(() => {
            ...
            this.pluginHandler.initPlugins(adapterConfig, () => {
                if (!adapterStates) return; // if adapterState was destroyed,we should not continue
