...
function Adapter(options) {
    ...
    const initStates = (cb) => {
                    ...
                    if (this.adapterReady) {
                        ...
                        if (this.aliases[id]) {
                            this.aliases[id].targets.forEach(target => {
                                const aState = tools.formatAliasValue(this.aliases[id].source, target, JSON.parse(JSON.stringify(state)), logger, this.namespaceLog);
                            ...
                            if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                                ...
                                checkStates(obj.common.alias.id, options, 'setState', (err, targetObj) => {
                                    if (err) {
                                        ...
                                        // write target state
                                        this.outputCount++;
                                        this.states.setState(obj.common.alias.id, tools.formatAliasValue(obj.common, targetObj.common, state, logger, this.namespaceLog), callback);
                                    }
                                });
                            } else {
                                ...
                                if (obj && obj.common && obj.common.alias && (obj.common.alias.id || obj.common.alias.val !== undefined)) {
                                    if (this.oStates && this.oStates[obj.common.alias.id]) {
                                        this.objects.getObject(obj.common.alias.id, (err, sourceObj) => {
                                            ...
                                            callback(err, tools.formatAliasValue(sourceObj.common, obj.common, state, logger, this.namespaceLog));
                                        });
                                    } else {
                                        this.objects.getObject(obj.common.alias.id, (err, sourceObj) => {
        ...
        this._addAliasSubscribe = (aliasObj, pattern, callback) => {
            if (aliasObj && aliasObj.common && aliasObj.common.alias && aliasObj.common.alias.id) {
                ...
                if (!this.aliases[sourceId].source) {
                    // this.rememberSystemSubscribes(sourceId, true);
                    if (pattern === '"*"' || pattern === '*') {
                    ...
                    if (aliasesIds.length) {
                        ...
                        this._getObjectsByArray(aliasesIds, null, options, (errors, aliasObjs) =>
                            aliasObjs.forEach(aliasObj =>
                                this._addAliasSubscribe(aliasObj, jsonPattern, () =>
                        ...
                        this.getForeignObjects(pattern, null, null, options, (err, objs) => {
                            ...
                            Object.keys(objs).forEach(id => {
                                ...
                                if (id.startsWith(ALIAS_STARTS_WITH)) {
                                    ...
                                    count++;
                                    this._addAliasSubscribe(aliasObj, pattern, () => {
                                        if (!--count) {
                                            if (pattern === '*') {
                                                this.rememberSystemSubscribes(pattern, true);
        ...
        this.unsubscribeForeignStates = (pattern, options, callback) => {
            ...
            if (Array.isArray(pattern)) {
                jsonPattern = JSON.stringify(pattern);

                pattern.forEach(_pattern => {
                    if (!_pattern.startsWith(ALIAS_STARTS_WITH + '*')) {
                        count++;
                        this.unsubscribeForeignObjects(_pattern, () =>
                            !--count && typeof callback === 'function' && callback());
                    }
                });
                ...
                if (pattern === '*' || pattern.startsWith(ALIAS_STARTS_WITH)) {
                    ...
                    if (pattern === '*') {
                        ...
                        count++;
                        this.states.unsubscribe(pattern, () =>
                            !--count && typeof callback === 'function' && callback());
                    }
                } else {
                    this.rememberSystemSubscribes(pattern, false);
            ...
            if (jsonPattern) {
                Object.keys(this.aliases).forEach(sourceId => {
                    for (let i = this.aliases[sourceId].targets.length - 1; i >= 0; i--) {
                        if (this.aliases[sourceId].targets[i].pattern === jsonPattern) {
                            count++;
