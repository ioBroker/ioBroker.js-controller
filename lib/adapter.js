...
const supportedFeatures = [
    ...
    'BASE_SETTINGS' // If base settings could be read and written
];
...
function Adapter(options) {
     ...
     */
    const validateId = (id, isForeignId) => {
        if (id === null || id === undefined || id === '') {
            throw new Error('The id is empty! Please provide a valid id.');
        }

        if (!isForeignId && typeof id === 'number') {
        ...
        } else if (typeof id !== 'string' && !tools.isObject(id)) {
            throw new Error(`The id has an invalid type! Expected "string" or "object", received "${typeof id}".`);
        }
            ...
            for (const reqProperty of reqProperties) {
                if (reqProperty !== undefined) {
                    if (typeof reqProperty !== 'string') {
                        throw new Error(`The id's property ${reqProperty} has an invalid type! Expected "string", received "${typeof reqProperty}".`);
                    }

                    if (reqProperty.includes('.')) {
                        throw new Error(`The id's property ${reqProperty} contains the invalid character "."!`);
                    }
                    found = true;
                }
            }
            if (found === false) {
                throw new Error(`The id is an invalid object! Expected at least one of the properties "device", "channel" or "state" to exist.`);
            }
        } else if (id.endsWith('.')) {
    ...
    const initObjects = cb => {
        ...
        const setObjectWithDefaultValue = (id, obj, options, callback) => {
            ...
            adapterObjects.setObject(id, obj, options, (err, result) => {
                ...
                this.setForeignState(id, obj.common.def, true, null, () =>
                    typeof callback === 'function' && callback(err, result));
            });
        };
        ...
        this.setForeignObject = (id, obj, options, callback) => {
            ...
            if (obj.common && obj.common.alias && obj.common.alias.id) {
                ...
                if (typeof obj.common.alias.id === 'object') {
                    try {
                        validateId(obj.common.alias.id.write, true);
                        validateId(obj.common.alias.id.read, true);
                    } catch (e) {
                    ...
                    try {
                        validateId(obj.common.alias.id, true);
                    } catch (e) {
                        return typeof callback === 'function' && setImmediate(() => callback(`Alias id is invalid: ${e.message}`));
                    }
    ...
    const initStates = cb => {
        ...
        this._addAliasSubscribe = (aliasObj, pattern, callback) => {
            if (aliasObj && aliasObj.common && aliasObj.common.alias && aliasObj.common.alias.id) {
                ...
                // validate here because we use objects/states db directly
                try {
                    validateId(sourceId, true);
                } catch (e) {
                    logger.warn(`${this.namespaceLog} Error validating alias id of ${aliasObj._id}: ${e.message}`);
                    return typeof callback === 'function' && setImmediate(() => callback(`Error validating alias id of ${aliasObj._id}: ${e.message}`));
                }
                ...
                typeof callback === 'function' && setImmediate(() => callback(`Alias ${aliasObj._id} has no target 12`));
            }
        };
