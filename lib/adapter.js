...
function Adapter(options) {
    ...
    function initRedis(cb) {
            ...
            that.states = new StatesRedis({
                ...
                change: function (id, state) {
                    if (id == 'system.adapter.' + that.namespace + '.common.messagebox') {
                        if (typeof options.onMessage === 'function') options.onMessage(state);
                    } else
    ...
    function initAdapter(adapterConfig) {
        ...
        if (adapterConfig.common.messagebox) {
            that.states.setState('system.adapter.' + that.namespace + '.common.messagebox', {command: null, message: null}, function () {
                // HQ: is it required? or by default adapter is subscribed to all children?
                that.subscribeStates('system.adapter.' + that.namespace + '.common.messagebox');
            });
        }
