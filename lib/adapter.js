...
function Adapter(options) {
    ...
    const initAdapter = (adapterConfig) => {
        initLogging(() => {
            ...
            if (options.instance === undefined) {
                ...
                reportStatus();
                tools.measureEventLoop(1000, (lag) => this.eventLoopLags.push(lag));
            }
    ...
    const reportStatus = () => {
        ...
        if (this.eventLoopLags.length) {
            this.states.setState(id + '.eventLoopLag', {val: Math.ceil(this.eventLoopLags.reduce((a, b) => (a + b)) / this.eventLoopLags.length), ack: true, from: id}); // average of measured values
            this.eventLoopLags = [];
        }
        this.inputCount  = 0;
        this.outputCount = 0;
