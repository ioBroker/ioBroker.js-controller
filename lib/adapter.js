...
function Adapter(options) {
    ...
    this._getObjectsByArray = (keys, options, cb, _index, _result, _errors) => {
        _index  = _index || 0;
        _result = _result || [];
        _errors = _errors || [];
        ...
        if (_index >= keys.length) {
            cb && cb(_errors.find(e => e) ? _errors : null, _result);
        }

        // if empty => skip immediately
        this.getForeignObject(keys[_index], options, (err, obj) => {
            _result[_index] = obj;
            setImmediate(() => this._getObjectsByArray(keys, options, cb, _index + 1, _result, _errors));
        });
    };
    ...
    this.getPort = (port, callback) => {
        ...
        try {
            ...
            server.on('error', (/* err */) => {
                setTimeout(() => this.getPort(port + 1, callback), 100);
            });
        } catch (e) {
            setImmediate(() => this.getPort(port + 1, callback));
        }
    };
    ...
    this.calculatePermissions = (user, commandsPermissions, options, callback) => {
        ...
        acl.groups = [];
        this.getForeignObjects('*', 'group', options, (err, groups) => {
    ...
    this.updateConfig = (newConfig) => {
        ...
        const config = Object.assign({}, this.config, newConfig);
    ...
    const createInstancesObjects = (callback, objs) => {
        if (!objs) {
            objs = this.ioPack.instanceObjects;
        }
        ...
        if (!objs || !objs.length) {
            ...
            if (obj && (obj._id || obj.type === 'meta')) {
                this.getObject(obj._id, (err, _obj) => {
                    if (!_obj) {
                        if (obj.common) {
                            if (obj.common.name) {
                        ...
                        this.setObject(obj._id, obj, err => {
                            err && this.log && logger.error('Cannot setObject: ' + err);
    ...
    const prepareInitAdapter = () => {
        this.getForeignState('system.adapter.' + this.namespace + '.alive', (err, res) => {
            ...
            if (!config.isInstall && res && res.val === true && !config.forceIfDisabled) {
                ...
                this.getForeignObject('system.adapter.' + this.namespace, (err, res) => {
    ...
    const initObjects = (cb) => {
        const objectsInst = new Objects({
            ...
            change: (id, obj) => {
                ...
                if ((options.noNamespace || config.noNamespace) && this._namespaceRegExp.test(id)) {
                    ...
                    setImmediate(() => {
                        if (typeof options.objectChange === 'function') options.objectChange(id.substring(this.namespace.length + 1), obj);
        ...
        this.setObject = (id, obj, options, callback) => {
            ...
            if (!obj) {
                logger.error(`setObject: try to set null object for ${id}`);
        ...
        this.extendObject = (id, obj, options, callback) => {
            ...
            if (!obj) {
                logger.error(`extendObject: try to set null object for ${id}`);
                return callback && callback(tools.ERRORS.ERROR_EMPTY_OBJECT);
            }
        ...
        this.extendForeignObject = (id, obj, options, callback) => {
            ...
            if (!obj) {
                logger.error(`extendForeignObject: try to set null object for ${id}`);
                return callback && callback(tools.ERRORS.ERROR_EMPTY_OBJECT);
            }
        ...
        this.getForeignObjects = (pattern, type, enums, options, callback) => {
            ...
            this.objects.getObjectView('system', type || 'state', params, options, (err, res) => {
                ...
                this.getEnums(enums, (err, _enums) => {
                    const list = {};
                    for (let i = 0; i < res.rows.length; i++) {
                        if (!res.rows[i].value) {
    ...
    const getGroups = (ids, callback, i) => {
        ...
        if (!ids || i >= ids.length) {
            ...
            this.getForeignObject(ids[i], (err, obj) => {
                this.groups[ids] = obj || {};
                setImmediate(getGroups, ids, callback, i + 1);
            });
        }
    ...
    const getUserGroups = (options, callback) => {
        ...
        options.groups = [];
        this.getForeignObject(options.user, (err, userAcl) => {
            if (!userAcl) {
                // User does not exists
                logger.error(this.namespace + ' unknown user "' + options.user + '"');
                callback(options);
            } else {
                this.getForeignObjects('*', 'group', (err, groups) => {
                    // aggregate all groups permissions, where this user is
                    if (groups) {
                        for (const g in groups) {
    ...
    const getDefaultHistory = (callback) => {
        if (!this.defaultHistory) {
            ...
            return this.getForeignObject('system.config', (err, data) => {
                if (data && data.common) {
                    this.defaultHistory = data.common.defaultHistory;
                }
                if (data && data.native) {
        ...
        if (id.startsWith('aliases.')) {
            this.objects.getObject(id, (err, obj) => {
                if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                    ...
                    logger.warn(err || `Alias ${id} has no target`);
                    callback(err || `Alias ${id} has no target`)
                }
            });
        } else {
            this.getForeignState(id, (err, oldState) => {
                if (err) {
                    typeof callback === 'function' && callback(err);
                } else {
                    let differ = false;
    ...
    const initStates = (cb) => {
            ...
            change: (id, state) => {
                ...
                if (id === 'messagebox.system.adapter.' + this.namespace && state) {
                    ...
                    if (this.aliases[id]) {
                        ...
                        if (this.aliases[id].read) {
                            try {
                                // process the value here
                                const func = new Function('val', 'return ' + this.aliases[id].read);
                                aState.val = func(aState.val);
                            } catch (e) {
                                logger.error(`Invalid read function for ${id}: ${this.aliases[id].read} => ${e}`);
                            }
                        }
                        // It was an error in the calculation
        ...
        this.sendTo = (instanceName, command, message, callback) => {
            ...
            if (typeof message !== 'object') {
                logger.debug('sendTo "' + command + '" to ' + instanceName + ' from system.adapter.' + this.namespace + ': ' + message);
            } else {
                logger.debug('sendTo "' + command + '" to ' + instanceName + ' from system.adapter.' + this.namespace);
            }
                    ...
                    if (err) {
                        ...
                        if (id.startsWith('aliases.')) {
                            ...
                            if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                                ...
                                if (obj.common.alias.write) {
                                    try {
                                        // process the value here
                                        const func = new Function('val', 'return ' + obj.common.alias.write);
                                        state.val = func(state.val);
                                    } catch (e) {
                                        logger.error(`Invalid write function for ${id}: ${obj.common.alias.write} => ${e}`);
                                    }
                                }

                                // todo: may be the rights must be checked
                                // this.setState(obj.common.alias.id, state, options, callback);

                                this.states.setState(obj.common.alias.id, state, callback);
                            } else {
                                logger.warn(err || `Alias ${id} has no target`);
                                callback(err || `Alias ${id} has no target`);
                            }
            ...
            if (options && options.user && options.user !== 'system.user.admin') {
                            ...
                            this.objects.getObject(id, options, (err, obj) => {
                                if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                                    if (this.oStates && this.oStates[obj.common.alias.id]) {
                                        ...
                                        if (obj.common.alias.read) {
                                            try {
                                                // process the value here
                                                const func = new Function('val', 'return ' + obj.common.alias.read);
                                                state.val = func(state.val);
                                            } catch (e) {
                                                logger.error(`Invalid read function for ${id}: ${obj.common.alias.read} => ${e}`);
                                            }
                                        }

                                        typeof callback === 'function' && callback(err, this.oStates[obj.common.alias.id]);
                                    } else if (obj.common.alias.read) {
                                        this.states.getState(obj.common.alias.id, (err, state) => {
                                            ...
                                            typeof callback === 'function' && callback(err, state);
                                        });
                                    } else {
                                    ...
                                    logger.warn(err || `Alias ${id} has no target`);
                                    typeof callback === 'function' && callback(err || `Alias ${id} has no target`);
                                }
                            });
                        } else {
                            if (this.oStates && this.oStates[id]) {
                                typeof callback === 'function' && callback(null, this.oStates[id]);
                            } else {
                                this.states.getState(id, callback);
                            }
                        }
                ...
                if (id.startsWith('aliases.')) {
                    this.objects.getObject(id, options, (err, obj) => {
                        if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                            if (this.oStates && this.oStates[obj.common.alias.id]) {
                                typeof callback === 'function' && callback(null, this.oStates[obj.common.alias.id]);
                            } else {
                                this.states.getState(obj.common.alias.id, callback);
                            }
                        } else {
                            logger.warn(err || `Alias ${id} has no target`);
                            typeof callback === 'function' && callback(err || `Alias ${id} has no target`);
                                ...
                                if (this.oStates && this.oStates[obj.common.alias.id]) {
                                    ...
                                    if (obj.common.alias.read) {
                                        try {
                                            ...
                                            logger.error(`Invalid write function for ${id}: ${obj.common.alias.read} => ${e}`);
                                        }
                                    }

                                    typeof callback === 'function' && callback(err, this.oStates[obj.common.alias.id]);
                                } else if (obj.common.common.alias.read) {
                                    this.states.getState(obj.common.common.alias.id, (err, state) => {
                                        try {
                                            // process the value here
                                            const func = new Function('val', 'return ' + obj.common.common.alias.read);
                                            state.val = func(state.val);
                                        } catch (e) {
                                            logger.error(`Invalid read function for ${id}: ${obj.common.common.alias.read} => ${e}`);
                                        }
                                        typeof callback === 'function' && callback(err, state);
                                    });
                                } else {
        ...
        this.getForeignStates = (pattern, options, callback) => {
            ...
            if (typeof callback !== 'function') {
                logger.error(this.namespace + ' getForeignStates invalid callback for ' + pattern);
                return;
            }
            ...
            this.objects.getObjectView('system', 'state', params, options, (err, res) => {
                ...
                if (options && options.user && options.user !== 'system.user.admin') {
                    checkStates(keys, options, 'getState', (err, keys, objs) => {
                        if (err) {
                            typeof callback === 'function' && callback(err);
                            return;
                        }
                        ...
                        keys.forEach((id, i) => {
                            if (id.startsWith('aliases.')) {
                                if (objs[i] && objs[i].common && objs[i].common.alias && objs[i].common.alias.id) {
                                    aliases[i] = objs[i].common.alias;
                                    keys[i] = objs[i].common.alias.id;
                                } else {
                                    logger.warn(err || `Alias ${keys[i]} has no target`);
                                }
                            }
                        });
                        ...
                        this.states.getStates(keys, (err, arr) => {
                            ...
                            for (let i = 0; i < res.rows.length; i++) {
                                ...
                                if (aliases[i] && aliases[i].read) {
                                    try {
                                        // process the value here
                                        const func = new Function('val', 'return ' + aliases[i].read);
                                        list[keys[i]].val = func(list[keys[i]].val);
                                    } catch (e) {
                                        logger.error(`Invalid read function for ${keys[i]}: ${aliases[i].read} => ${e}`);
                                    }
                                }
                            }
                    ...
                    const aIds = keys.map(id => id.startsWith('aliases.0') ? id : null);

                    // if any ID from aliases found
                    if (aIds.find(id => id)) {
                        // read aliases objects
                        this._getObjectsByArray(aIds, options, (errors, objs) => {
                            const aliases = [];

                            // replace aliases ID with targets
                            objs.forEach((obj, i) => {
                                if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                                    keys[i] = obj.common.alias.id;
                                    aliases[i] = obj.common.alias;
                                }
                            });
        ...
        this.subscribeForeignStates = (pattern, options, callback) => {
            ...
            autoSubscribeOn(() => {
                    ...
                    if (pattern === '*' || pattern.substring(0, this.autoSubscribe[s].length + 1) === this.autoSubscribe[s] + '.') {
                        ...
                        this.states.getState('system.adapter.' + this.autoSubscribe[s] + '.subscribes', (err, state) => {
                            ...
                            try {
                                ...
                                logger.error('Cannot parse subscribes for "' + this.autoSubscribe[s] + '.subscribes"');
                            }
                ...
                if (pattern.startsWith('aliases.')) {
                    if (pattern instanceof Array) {
                        let count = 0;
                        pattern.forEach(id => {
                            count++;
                            // TODO: this must be serialized
                            this.getForeignObjects(id, options, (err, obj) => {
                                if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                                    ...
                                    this.states.subscribe(obj.common.alias.id, callback);
                                } else {
                                    logger.warn(err || `Alias ${pattern} has no target`);
                                }
                                if (!--count) {
                        ...
                        // read all aliases
                        this.getForeignObjects(pattern, options, (err, objs) => {
                            Object.keys(objs).forEach(id => {
                                if (objs[id] && objs[id].common && objs[id].common.alias && objs[id].common.alias.id) {
                                    this.aliases[objs[id].common.alias.id] = JSON.parse(JSON.stringify(objs[id].common.alias));
                                    this.aliases[objs[id].common.alias.id].pattern = JSON.stringify(pattern);
                                    this.aliases[objs[id].common.alias.id].sourceId = id;
                                    this.states.subscribe(objs[id].common.alias.id, callback);
                                } else {
                                    logger.warn(err || `Alias ${pattern} has no target`);
                                }
                            })
            ...
            if (pattern.startsWith('aliases.')) {
                ...
                Object.keys(this.aliases).forEach(id => {
                    if (this.aliases[id].pattern === jsonPattern) {
                        this.states.unsubscribe(id, callback);
    ...
    const initAdapter = (adapterConfig) => {
        initLogging(() => {
            if (options.instance === undefined) {
                ...
                this.config = adapterConfig.native || {};
                this.common = adapterConfig.common || {};
                this.host = this.common.host || tools.getHostName() || require('os').hostname();
            }
            ...
            if (options.instance === undefined) {
                ...
                logger.info('starting. Version ' + this.version + ' in ' + this.adapterDir + ', node: ' + process.version);
    ...
    const reportStatus = () => {
        ...
        this.states.setState(id + '.memRss', {val: parseFloat((mem.rss / 1048576/* 1MB */).toFixed(2)), ack: true, from: id});
        this.states.setState(id + '.memHeapTotal', {val: parseFloat((mem.heapTotal / 1048576/* 1MB */).toFixed(2)), ack: true, from: id});
        this.states.setState(id + '.memHeapUsed', {val: parseFloat((mem.heapUsed / 1048576/* 1MB */).toFixed(2)), ack: true, from: id});
    ...
    const stop = (isPause, isScheduled) => {
        ...
        const finishUnload = () => {
            if (this.states) {
                ...
                this.states.setState(id + '.alive', {val: false, ack: true, from: id}, () => {
                    if (!isPause && this.log) logger.info('terminating');
        ...
        setTimeout(() => {
            if (this.states) {
                ...
                setTimeout(() => {
                    if (!isPause && this.log) logger.info('terminating with timeout');
                    this.terminate(isScheduled ? EXIT_CODES.START_IMMEDIATELY_AFTER_STOP : 0);
                }, 1000);
            } else {
                if (!isPause && this.log) logger.info('terminating');
    ...
    process.on('uncaughtException', err => {
        ...
        if (this.getPortRunning && err.message === 'listen EADDRINUSE') {
            logger.warn(this.namespace + ' Port ' + this.getPortRunning.port + ' is in use. Get next');

            setImmediate(() => this.getPort(this.getPortRunning.port + 1, this.getPortRunning.callback));
            return;
        }
    ...
    initObjects(() => {
        if (this.inited) {
            this.log && logger.warn('Reconnection to DB.');
            return;
        }
