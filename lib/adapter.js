        ...
        this.delForeignObject = (id, options, callback) => {
            ...
            if (options && options.recursive) {
                ...
                adapterObjects.getObject(id, options, (err, obj) => {
                    ...
                    adapterObjects.getObjectView('system', '*', selector, options, (err, res) => {
                        res && res.rows && res.rows.forEach(item => !tasks.find(task => task.id === item.id) && tasks.push({id: item.id, state: item.value && item.value.type === 'state'}));
