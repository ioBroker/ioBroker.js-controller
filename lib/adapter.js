...
function Adapter(options) {
    ...
    if (instance == '--force' || instance == '--install' || instance == '--logs') instance = 0;
    ...
    that.getCertificates = function (publicName, privateName, chainedName, callback) {
        ...
        that.getForeignObject('system.certificates', function (err, obj) {
            if (err || !obj ||
                ...
                (chainedName && !obj.that.certificates[chainedName])
            ) {
    ...
    function initObjects(cb) {
        ...
        that.formatValue = function (value, decimals, _format) {
            ...
            format = (_format === undefined) ? ((that.isFloatComma === undefined) ? '.,' : ((that.isFloatComma) ? '.,' : ',.')) : _format;
        ...
        that.formatDate = function formatDate(dateObj, isDuration, _format) {
            ...
            if (text !== 'object') {
                ...
                if (i == dateObj) {
                    ...
                    dateObj = (i < 946681200000) ? new Date(i * 1000) : new Date(i);
                } else {
            ...
            var validFormatChars = 'YJГMDTДhSчmмsс';
            var s      = '';
            var result = '';
    ...
    function initStates(cb) {
        ...
        that.getForeignStateHistory = function getForeignStateHistory(id, start, end, options, callback) {
            ...
            start = start || (new Date()).getTime() - 31536000000; // - 1 year
            end   = end   || (new Date()).getTime() + 5000000;
    ...
    function initLogging() {
        if (!options.logTransporter && !that.ioPack.common.logTransporter) {
            ...
            logger.on('logging', function (transport, level, msg, meta) {
                if (transport.name !== 'ioBroker') return;
    ...
    process.on('uncaughtException', function (err) {
		...
		try {
			...
			logger.error('exception by stop: ' + (err.message || err));
		}
    });
...
function ts2day(ts) {
    var dateObj;
    if (ts < 946681200000) { // if less 2000.01.01 00:00:00
