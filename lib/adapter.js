        ...
        const _states = new States({
            ...
            change: (id, state) => {
                ...
                if (id === 'system.adapter.' + this.namespace + '.sigKill') {
                    if (this.startedInCompactMode) {
                        logger.info(this.namespaceLog + ' Got terminate signal.');
                    } else {
                        logger.warn(this.namespaceLog + ' Got terminate signal. Checking desired PID: ' + (state && state.val) + ' vs own PID ' + process.pid);
                    }
