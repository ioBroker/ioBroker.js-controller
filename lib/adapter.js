...
function Adapter(options) {
    ...
    const extendObjects = (tasks, callback) => {
        ...
        try {
            tools.validateObjectCommonProperties(task);
        } catch (e) {
            // todo: in the future we will not create this object
            logger.warn(`Type check failed on ${task._id}: ${e.message}`);
            logger.warn('This object will not be created in future versions. Please report this to the developer.');
        }
    ...
    const initObjects = (cb) => {
        ...
        const setObjectWithDefaultValue = (id, obj, options, callback) => {
            ...
            try {
                ...
                // todo: in the future we will not create this object
                this.log.warn(`Type check failed on ${id}: ${e.message}`);
