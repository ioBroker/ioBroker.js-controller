        ...
        if (id.startsWith(ALIAS_STARTS_WITH)) {
            this.objects.getObject(id, (err, obj) => {
                if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                    ...
                    logger.warn(err || `Alias ${id} has no target 1`);
            ...
            change: (id, state) => {
                ...
                if (id === 'messagebox.system.adapter.' + this.namespace && state) {
                    ...
                    if (this.aliases[id]) {
                        ...
                        formatAliasValue(this.aliases[id].source, this.aliases[id].target, aState);
        ...
        function formatAliasValue(sourceObj, targetObj, state) {
            ...
            if (targetObj && targetObj.common && targetObj.common.alias && targetObj.common.alias.read) {
                try {
                    ...
                    logger.error(`Invalid read function for ${targetObj._id}: ${targetObj.common.alias.read} => ${e}`);
                }
            }
            ...
            if (sourceObj && sourceObj.common && sourceObj.common.alias && sourceObj.common.alias.write) {
                try {
                    ...
                    logger.error(`Invalid read function for ${sourceObj._id}: ${sourceObj.common.alias.write} => ${e}`);
                }
            }
            ...
            if ((targetObj && targetObj.common && targetObj.common.alias && !targetObj.common.alias.read) || (sourceObj && sourceObj.common && sourceObj.common.alias && !sourceObj.common.alias.write)) {
                if (targetObj && targetObj.common &&
                    ...
                    let val = (state.val - sourceObj.common.min) / (sourceObj.common.max - sourceObj.common.min);
                    ...
                    if (err) {
                        ...
                        if (id.startsWith(ALIAS_STARTS_WITH)) {
                            ...
                            if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                                ...
                                logger.warn(err || `Alias ${id} has no target 2`);
                                callback(err || `Alias ${id} has no target`);
                            }
                        } else {
                            this.outputCount++;
                ...
                if (id.startsWith(ALIAS_STARTS_WITH)) {
                    ...
                    this.objects.getObject(id, options, (err, obj) => {
                        if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                            ...
                            logger.warn(err || `Alias ${id} has no target 3`);
                            callback(err || `Alias ${id} has no target`);
                        }
                    });
                } else {
                    ...
                    if (err) {
                        ...
                        if (id.startsWith(ALIAS_STARTS_WITH)) {
                            ...
                            if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                                ...
                                logger.warn(err || `Alias ${id} has no target 4`);
                                callback(err || `Alias ${id} has no target`);
                            }
                        } else {
                            this.outputCount++;
                ...
                if (id.startsWith(ALIAS_STARTS_WITH)) {
                    ...
                    this.objects.getObject(id, options, (err, obj) => {
                        if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                            ...
                            logger.warn(err || `Alias ${id} has no target 5`);
                            callback(err || `Alias ${id} has no target`);
                        }
                    });
                } else {
                        ...
                        if (id.startsWith(ALIAS_STARTS_WITH)) {
                            this.objects.getObject(id, options, (err, obj) => {
                                if (obj && obj.common && obj.common.alias && (obj.common.alias.id || obj.common.alias.val !== undefined)) {
                                    ...
                                    logger.warn(err || `Alias ${id} has no target 6`);
                                    callback(err || `Alias ${id} has no target`);
                                }
                            });
                        } else {
                ...
                if (id.startsWith(ALIAS_STARTS_WITH)) {
                    this.objects.getObject(id, options, (err, obj) => {
                        if (obj && obj.common && obj.common.alias && (obj.common.alias.id || obj.common.alias.val !== undefined)) {
                            ...
                            logger.warn(err || `Alias ${id} has no target 7`);
                            callback(err || `Alias ${id} has no target`);
                        }
                    });
                } else {
                    ...
                    if (err) {
                        ...
                        if (id.startsWith(ALIAS_STARTS_WITH)) {
                            if (obj && obj.common && obj.common.alias && (obj.common.alias.id || obj.common.alias.val !== undefined)) {
                                ...
                                logger.warn(err || `Alias ${id} has no target 8`);
                                callback(err || `Alias ${id} has no target`);
                            }
                        } else {
                            if (this.oStates && this.oStates[id]) {
                ...
                if (id.startsWith(ALIAS_STARTS_WITH)) {
                    this.objects.getObject(id, (err, obj) => {
                        if (obj && obj.common && obj.common.alias && (obj.common.alias.id || obj.common.alias.val !== undefined)) {
                            ...
                            logger.warn(err || `Alias ${id} has no target 9`);
                            callback(err || `Alias ${id} has no target`);
                        }
                    });
                } else {
                ...
                if (pattern instanceof Array) {
                            ...
                            pattern.forEach((id, i) => {
                                if (ids[i]) {
                                    ...
                                    if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                                        ...
                                        logger.warn(`Alias ${obj._id} has no target 10`);
                                    }
                                } else {
                    ...
                    this.objects.getObject(pattern, options, (err, obj) => {
                        ...
                        if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                            ...
                            logger.warn(err || `Alias ${pattern} has no target 12`);
                            typeof callback === 'function' && callback();
                        }
                    });
                } else {
