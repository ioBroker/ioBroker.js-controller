...
function Adapter(options) {
    ...
    this.namespaceLog = options.name + (this.startedInCompactMode ? ' (COMPACT)' : ' (' + process.pid + ')');
    ...
    const initStates = (cb) => {
        ...
        const _states = new States({
            ...
            change: (id, state) => {
                ...
                if (id === 'system.adapter.' + this.namespace + '.sigKill' && state && state.val !== 0) {
                    if (this.startedInCompactMode) {
                        logger.info(this.namespaceLog + ' Got terminate signal ' + state.val);
                    } else {
                ...
                if (id === 'messagebox.system.adapter.' + this.namespace && state) {
                    ...
                    if (this.aliases[id]) {
                        ...
                        tools.formatAliasValue(this.aliases[id].source, this.aliases[id].target, aState, logger, this.namespaceLog);
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    if (obj && obj.common && obj.common.alias) {
                        if (obj.common.alias.val !== undefined) {
                            ...
                            result[obj._id] = tools.formatAliasValue(srcObjs[i], obj, arr[i] || null, logger, this.namespaceLog);
                        }
                    } else {
        ...
        this.getForeignStates = (pattern, options, callback) => {
            ...
            if (!this.objects) { // if states is no longer existing, we do not need to unsubscribe
                callback && callback(new Error('Objects database not connected'));
                return;
            }
