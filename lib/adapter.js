        ...
        function formatAliasValue(sourceObj, targetObj, state) {
            ...
            if (targetObj && targetObj.common && targetObj.common.alias && targetObj.common.alias.read) {
                try {
                    ...
                    logger.error(`Invalid read function for ${id}: ${targetObj.common.alias.read} => ${e}`);
                }
            }
            ...
            if (sourceObj && sourceObj.common && sourceObj.common.alias && sourceObj.common.alias.write) {
                try {
                    // process the value here
                    const func = new Function('val', 'return ' + sourceObj.common.alias.read);
                    state.val = func(state.val);
                } catch (e) {
                    logger.error(`Invalid read function for ${id}: ${sourceObj.common.alias.read} => ${e}`);
                }
            }
            ...
            if ((targetObj && targetObj.common && targetObj.common.alias && !targetObj.common.alias.read) || (sourceObj && sourceObj.common && sourceObj.common.alias && !sourceObj.common.alias.write)) {
                if (targetObj.common.type === 'number' &&
                    ...
                    let val = (state.val - source.common.min) / (sourceObj.common.max - sourceObj.common.min);
                            ...
                            if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                                ...
                                checkStates(obj.common.alias.id, options, 'setState', (err, targetObj) => {
                                    if (err) {
                                        ...
                                        // write target state
                                        this.outputCount++;
                                        this.states.setState(obj.common.alias.id, formatAliasValue(obj, targetObj, state), callback);
                                    }
                                });
                            } else {
                                logger.warn(err || `Alias ${id} has no target`);
                                ...
                                if (obj && obj.common && obj.common.alias && (obj.common.alias.id || obj.common.alias.val !== undefined)) {
                                    if (this.oStates && this.oStates[obj.common.alias.id]) {
                                        this.objects.getObject(obj.common.alias.id, (err, sourceObj) => {
                                            ...
                                            callback(err, formatAliasValue(sourceObj, obj, state));
                                        });
                                    } else {
                                        this.objects.getObject(obj.common.alias.id, (err, sourceObj) => {
                    ...
                    this.objects.getObject(id, (err, obj) => {
                        if (obj && obj.common && obj.common.alias && (obj.common.alias.id || obj.common.alias.val !== undefined)) {
                            this.objects.getObject(obj.common.alias.id, (err, sourceObj) => {
                                if (this.oStates && this.oStates[obj.common.alias.id]) {
                                    const state = JSON.parse(JSON.stringify(this.oStates[obj.common.alias.id]));
                                    callback(err, formatAliasValue(obj, targetObj, state));
                                } else {
                                    this.inputCount++;
                                    this.states.getState(obj.common.common.alias.id, (err, state) =>
                                        callback(err, formatAliasValue(obj, targetObj, state)));
                ...
                if (options && options.user && options.user !== 'system.user.admin') {
                    checkStates(keys, options, 'getState', (err, keys, objs) => {
                        ...
                        if (aliasFound) {
                            ...
                            this._getObjectsByArray(srcIds, options, (errors, srcObjs) => {
                                this.states.getStates(keys, (err, arr) => {
                                    if (err) {
                                        return callback(err);
                                    }
                                    for (let i = 0; i < res.rows.length; i++) {
                                        if (srcObjs[i] && !checkState(srcObjs[i], options, 'getState')) {
                                            arr[i] = null;
                                        } else {
                                            const obj = objs[i];
                                            ...
                                            if (obj && obj.common && obj.common.alias) {
                                                if (obj.common.alias.val !== undefined) {
                                                    list[obj._id] = obj.common.alias.val;
                                                } else {
                                                    list[obj._id] = formatAliasValue(srcObjs[i], obj, arr[i] || null);
                                                }
                                            } else {
