...
/**
 * Checks if a passed ID is valid. Returns `true` if it is, `false` otherwise.
 * If false the callback is called with an error, so do not call it again!
 *
 * @param {string} id  id to check
 * @param {function} callback  callback function
 * @return {boolean} id valid or not?
 */
function validateId(id, callback) {
    if (id === null || id === undefined || id === '') {
        callback && callback('The id is empty! Please provide a valid id.');
        return false;
    }
    if (typeof id !== 'string') {
        callback && callback(`The id has an invalid type! Expected "string", received "${typeof id}".`);
        return false;
    }
    return true;
}
...
function Adapter(options) {
    ...
    const initObjects = (cb) => {
        ...
        this.setObject = (id, obj, options, callback) => {
            ...
            if (obj.type !== 'meta' && !validateId(id, callback)) {
                logger.error(tools.appendStackTrace(this.namespaceLog + ' setObject id invalid (empty or no string)!!'));
                return;
            }
