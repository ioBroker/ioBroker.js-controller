...
function Adapter(options) {
    ...
    if (instance == '--force' || instance == '--silent') instance = 0;
            ...
            that.getForeignState('system.adapter.' + that.namespace + '.alive', function (err, res) {
                ...
                if (res && res.val === true) {
                    ...
                    that.getForeignObject('system.adapter.' + options.name + '.' + instance, function (err, res) {
                        if ((err || !res) && (!process.argv || process.argv.indexOf('--silent') == -1)) {
    ...
    function initAdapter(adapterConfig) {
        ...
        if (options.instance === undefined) {
            if (!adapterConfig || !adapterConfig.common || !adapterConfig.common.enabled) {
                var isSilence = (process.argv && process.argv.indexOf('--silent') != -1);

                if (adapterConfig && adapterConfig.common && adapterConfig.common.enabled !== undefined) {
                    if (!isSilence) logger.error('adapter disabled');
                } else {
                    if (!isSilence) logger.error('no config found for adapter');
                }

                if (isSilence) {
                    process.exit();
                } else if (!process.argv || process.argv.indexOf('--force') == -1) {
            ...
            if (!isSilence && !adapterConfig._id) {
