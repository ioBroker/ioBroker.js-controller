                ...
                if (options.user === obj.acl.owner) {
                    if (command === 'setState' || command === 'delState') {
                        if (command === 'delState' && !options.acl.state['delete']) {
                            logger.warn(`Permission error for user "${options.user} on "${obj._id}": ${command}`);
                            return false;
                        } else
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    if (obj && obj.common && obj.common.alias) {
                        if (obj.common.alias.val !== undefined) {
                            ...
                            result[obj._id] = formatAliasValue(srcObjs[i], obj, arr[i] || null);
                        }
                    } else {
        ...
        this.subscribeForeignStates = (pattern, options, callback) => {
            ...
            autoSubscribeOn(() => {
                ...
                if (pattern.startsWith('alias.')) {
                    if (pattern instanceof Array) {
                        this._getObjectsByArray(pattern, null, options, (errors, srcObjs) => {
                            srcObjs.forEach(obj => {
                                if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                                    ...
                                    // may be call here subscribeForeignStates because of autosubscribe
                                    this.states.subscribe(obj.common.alias.id, callback);
                                } else {
                                    logger.warn(err || `Alias ${obj._id} has no target`);
                                }
                            });
