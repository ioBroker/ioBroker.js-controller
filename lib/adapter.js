                    ...
                    if (this.adapterReady) {
                        ...
                        if (this.aliases[id]) {
                            this.aliases[id].targets.forEach(target => {
                                const aState = state !== null ? tools.formatAliasValue(this.aliases[id].source, target, JSON.parse(JSON.stringify(state)), logger, this.namespaceLog) : state;

                                if (aState || state === null) {
