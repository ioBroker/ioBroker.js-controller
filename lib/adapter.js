...
function Adapter(options) {
    ...
    const validateId = (id, isForeignId, options) => {
        ...
        if (tools.isObject(id)) {
            ...
            if (found === false) {
                throw new Error(`The "${JSON.stringify(id)}" is an invalid object! Expected at least one of the properties "device", "channel" or "state" to exist.`);
            }
        } else {
            if (type !== 'string') {
                throw new Error(`The "${JSON.stringify(id)}" has an invalid type! Expected "string", received "${type}".`);
            }
            if (id.endsWith('.')) {
                throw new Error(`The "${id}" is invalid. Ids are not allowed to end in "."`);
            }
        }
    };
    ...
    this.calculatePermissions = (user, commandsPermissions, options, callback) => {
        ...
        if (user === SYSTEM_ADMIN_USER) {
            ...
            for (const c in commandsPermissions) {
                if (!commandsPermissions.hasOwnProperty(c) || !commandsPermissions[c].type) {
                    continue;
                }
        ...
        this.getForeignObjects('*', 'group', null, options, (err, groups) => {
            ...
            typeof callback === 'function' && callback(acl);
        });
    };
    /**
        ...
        if (this.config.hasOwnProperty(attribute)) {
            if (typeof callback !== 'function') {
                ...
                if (systemSecret !== null) {
                    setImmediate(() => callback(null, tools.decrypt(systemSecret, this.config[attribute])));
                } else {
            ...
            if (typeof callback === 'function') {
                setImmediate(() => callback(`Attribute "${attribute}" not found`));
            } else {
                return Promise.reject(new Error(`Attribute "${attribute}" not found`));
            }
        }
    };
    ...
    const initObjects = cb => {
                    ...
                    if (isNewAlias) {
                        for (const aliasPattern of this.aliasPatterns) {
                            ...
                            if (typeof testPattern === 'string' && aliasPattern === id  || testPattern instanceof RegExp && testPattern.test(id)) {
                                this._addAliasSubscribe(obj, id);
                                break;
                            }
                        }
                    }
        ...
        const setObjectWithDefaultValue = async (id, obj, options, callback) => {
            ...
            if (!adapterObjects) { // if objects is no longer existing, we do not need to unsubscribe
                callback && setImmediate(() => callback('Objects database not connected'));
                return;
            }
            ...
            try {
                ...
                if (obj.common && obj.common.def !== undefined) {
                    ...
                    if (!state || state.val === undefined) {
                        await this.setForeignStateAsync(id, {val: obj.common.def}, true);
                    }
                }
        ...
        this.setObject = (id, obj, options, callback) => {
            ...
            if (!obj) {
                ...
                return callback && setImmediate(() => callback(tools.ERRORS.ERROR_EMPTY_OBJECT));
            }
            ...
            if (obj.hasOwnProperty('type')) {
                ...
                typeof callback === 'function' && setImmediate(() => callback('mandatory property type missing!'));
            }
        };
        /**
        ...
        this.getAdapterObjects = callback => {
            ...
            if (!adapterObjects) { // if objects is no longer existing, we do not need to unsubscribe
                callback && setImmediate(() => callback(objects));
                return;
            }
            ...
            adapterObjects.getObjectView('system', 'state', {startkey: this.namespace + '.', endkey: this.namespace + '.\u9999', include_docs: true}, (err, _states) => {
                ...
                if (!adapterObjects) { // if objects is no longer existing, we do not need to unsubscribe
                    return typeof callback === 'function' && callback(objects);
                }
        ...
        this.extendObject = async (id, obj, options, callback) => {
            ...
            if (!adapterObjects) { // if objects is no longer existing, we do not need to unsubscribe
                return callback && setImmediate(() => callback('Objects database not connected'));
            }
            ...
            if (obj && (
                ...
                try {
                    ...
                    if (defState !== undefined) {
                        let currentState;
                        try {
                            currentState = await this.getStateAsync(id);
                        } catch (e) {
                            currentState = null;
                        }
                        if (!currentState) {
                    ...
                    typeof callback === 'function' && callback(null, cbObj);
                } catch (e) {
        ...
        this.setForeignObject = (id, obj, options, callback) => {
            ...
            if (!obj) {
                ...
                return typeof callback === 'function' && setImmediate(() => callback(tools.ERRORS.ERROR_EMPTY_OBJECT));
            }
            ...
            if (obj.common && obj.common.alias && obj.common.alias.id) {
                ...
                if (typeof obj.common.alias.id === 'object') {
                    try {
                        ...
                        return typeof callback === 'function' && setImmediate(() => callback(`Alias id is invalid: ${e.message}`));
                    }

                    if (obj.common.alias.id.write.startsWith(ALIAS_STARTS_WITH) || obj.common.alias.id.read.startsWith(ALIAS_STARTS_WITH)) {
                        return typeof callback === 'function' && setImmediate(() => callback('Aliases cannot be used as target for aliases'));
                    }
                } else {
                    try {
        ...
        this.getEnums = (_enumList, options, callback) => {
            ...
            if (_enumList) {
                ...
                Promise.all(promises).then(() => typeof callback === 'function' && callback(null, _enums)).catch(e => {
                    typeof callback === 'function' && callback(e);
                });
            } else {
                // Read all enums
                        ...
                        for (let i = 0; i < res.rows.length; i++) {
                            ...
                            if (_enums && id) {
                                ...
                                for (const es in _enums) {
                                    if (!_enums.hasOwnProperty(es)) {
                                        continue;
                                    }
                                    for (const e in _enums[es]) {
                                        if (!_enums[es].hasOwnProperty(e)) {
                                            continue;
                                        }
        ...
        this.unsubscribeForeignObjects = (pattern, options, callback) => {
            ...
            if (!adapterObjects) { // if objects is no longer existing, we do not need to unsubscribe
                typeof callback === 'function' && setImmediate(() => callback('Objects database not connected'));
                return;
            }
        ...
        this.getDevices = (options, callback) => {
            ...
            if (!adapterObjects) { // if objects is no longer existing, we do not need to unsubscribe
                setImmediate(() => callback('Objects database not connected'));
                return;
            }
    ...
    const getUserGroups = (options, callback) => {
        if (this.users[options.user]) {
            ...
            return setImmediate(() => callback(options));
        }
        options.groups = [];
    ...
    const initStates = cb => {
        ...
        this.sendTo = (instanceName, command, message, callback) => {
            ...
            if (!adapterStates) { // if states is no longer existing, we do not need to unsubscribe
                callback && setImmediate(() => callback('States database not connected'));
                return;
            }
                        ...
                        if (id.startsWith(ALIAS_STARTS_WITH)) {
                            ...
                            if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                                ...
                                try {
                                    ...
                                    return typeof callback === 'function' && setImmediate(() => callback(`Error validating alias id of ${id}: ${e.message}`));
                                }
                                ...
                                setImmediate(() => callback(`Alias ${id} has no target`));
                            }
                        } else {
                            this.outputCount++;
                    ...
                    adapterObjects.getObject(id, options, (err, obj) => {
                        if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                            ...
                            try {
                                ...
                                logger.warn(`${this.namespaceLog} Error validating alias id of ${id}: ${e.message}`);
                                return typeof callback === 'function' && callback(`Error validating alias id of ${id}: ${e.message}`);
                            }
        ...
        this.setForeignState = (id, state, ack, options, callback) => {
            ...
            if (!id || typeof id !== 'string') {
                ...
                return (typeof callback === 'function') && setImmediate(() => callback(warn));
            }
                    ...
                    if (err) {
                        ...
                        if (id.startsWith(ALIAS_STARTS_WITH)) {
                            ...
                            if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                                ...
                                return typeof callback === 'function' && setImmediate(() => callback(`Alias ${id} has no target`));
                            }
                        } else {
                            this.outputCount++;
                            adapterStates.setState(id, state, callback);
        ...
        this.getState = (id, options, callback) => {
            ...
            if (!adapterStates) { // if states is no longer existing, we do not need to unsubscribe
                setImmediate(() => callback('States database not connected'));
                return;
            }
                    ...
                    if (err) {
                        ...
                        if (id.startsWith(ALIAS_STARTS_WITH)) {
                            adapterObjects.getObject(id, options, (err, obj) => {
                                if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                                    ...
                                    try {
                                        ...
                                        logger.warn(`${this.namespaceLog} Error validating alias id of ${id}: ${e.message}`);
                                        return callback(`Error validating alias id of ${id}: ${e.message}`);
                                    }
                            ...
                            if (this.oStates && this.oStates[id]) {
                                setImmediate(() => callback(null, this.oStates[id]));
                            } else {
                        ...
                        if (id.startsWith(ALIAS_STARTS_WITH)) {
                            if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
                                ...
                                try {
                                    ...
                                    logger.warn(`${this.namespaceLog} Error validating alias id of ${id}: ${e.message}`);
                                    return setImmediate(() => callback(`Error validating alias id of ${id}: ${e.message}`));
                                }
                                    ...
                                    if (this.oStates && this.oStates[aliasId]) {
                                        checkStates(aliasId, options, 'getState', (err, sourceObj) => {
                                            if (err) {
                                                ...
                                                const state = deepClone(this.oStates[aliasId]);
                                                setImmediate(() => callback(err, tools.formatAliasValue(sourceObj && sourceObj.common, obj.common, state, logger, this.namespaceLog)));
                                            }
                                        });
                                    } else {
                                        checkStates(aliasId, options, 'getState', (err, sourceObj) => {
        ...
        this._processStates = function (keys, targetObjs, callback) {
            ...
            const aIds = keys.map(id => {
                if (id && id.startsWith(ALIAS_STARTS_WITH)) {
                    aliasFound = true;
                    return id;
                } else {
                    return null;
        ...
        this.getForeignStates = (pattern, options, callback) => {
            ...
            if (!adapterStates) { // if states is no longer existing, we do not need to unsubscribe
                return setImmediate(() => callback('States database not connected'));
            }

            if (!adapterObjects) { // if states is no longer existing, we do not need to unsubscribe
                return setImmediate(() => callback('Objects database not connected'));
            }

            if (pattern instanceof RegExp) {
                logger.error(this.namespaceLog + ' Regexp is not supported for "getForeignStates"');
                return setImmediate(() => callback('Regexp is not supported for "getForeignStates"'));
            }
        ...
        this._addAliasSubscribe = (aliasObj, pattern, callback) => {
            if (aliasObj && aliasObj.common && aliasObj.common.alias && aliasObj.common.alias.id) {
                if (aliasObj.type !== 'state') {
                    ...
                    return typeof callback === 'function' && setImmediate(() => callback(new Error(`Expected alias ${aliasObj._id} to be of type "state", got "${aliasObj.type}"`)));
                }
                ...
                try {
                    ...
                    return typeof callback === 'function' && setImmediate(() => callback(new Error(`Error validating alias id of ${aliasObj._id}: ${e.message}`)));
                }
                ...
                typeof callback === 'function' && setImmediate(() => callback(new Error(`Alias ${aliasObj._id} has no target 12`)));
            }
        };
        ...
        this.subscribeForeignStates = (pattern, options, callback) => {
            ...
            autoSubscribeOn(async () => {
                if (!adapterStates) { // if states is no longer existing, we do not need to unsubscribe
                    typeof callback === 'function' && callback('States database not connected');
                    return;
                }
                if (!adapterObjects) { // if objects is no longer existing, we do not need to unsubscribe
                    typeof callback === 'function' && callback('Objects database not connected');
                    return;
                }
                ...
                if (pattern instanceof RegExp && pattern.toString().indexOf('alias') !== -1) {
                    ...
                    return typeof callback === 'function' && callback('Regexp is not supported for "subscribeForeignStates"');
                }
                        ...
                        try {
                            ...
                            Object.keys(objs).forEach(id => {
                                ...
                                if (id.startsWith(ALIAS_STARTS_WITH)) {
                                    ...
                                    promises.push(new Promise(resolve => this._addAliasSubscribe(aliasObj, aliasPattern, resolve)));
                                }
                            });
                            ...
                            logger.warn(`${this.namespaceLog} Cannot subscribe to ${pattern}: ${e}`);
                            typeof callback === 'function' && callback(e);
                        }
                    } else {
                        adapterStates.subscribeUser(pattern, callback);
        ...
        this.unsubscribeForeignStates = (pattern, options, callback) => {
            ...
            if (!adapterStates) { // if states is no longer existing, we do not need to unsubscribe
                return typeof callback === 'function' && setImmediate(() => callback('States database not connected'));
            }
        ...
        this.subscribeStates = (pattern, options, callback) => {
            ...
            if (!adapterStates) { // if states is no longer existing, we do not need to unsubscribe
                return callback && setImmediate(() => callback('States database not connected'));
            }
         ...
         */
        this.decrypt = value => {
            return tools.decrypt(systemSecret, value);
        };
         ...
         */
        this.encrypt = value => {
            return tools.encrypt(systemSecret, value);
        };
        ...
        this.setBinaryState = (id, binary, options, callback) => {
            ...
            checkStates(id, options, 'setState', (err, obj) => {
                if (!err && !obj) {
                    typeof callback === 'function' && setImmediate(() => callback('Object does not exist'));
                } else
                if (!err && !obj.binary) {
                    obj.binary = true;
        ...
        this.getBinaryState = (id, options, callback) => {
            ...
            if (typeof callback !== 'function') {
                return logger.error(this.namespaceLog + ' No callback for getBinaryState');
            }
                ...
                if (err) {
                    ...
                    adapterStates.getBinaryState(id, (err, data) => {
                        if (!err && data && obj && !obj.binary) {
                            ...
                            if (!err && !data && obj && !obj.binary) {
                                setImmediate(() => callback('State is not binary'));
                            } else {
                                callback(err, data);
                            }
                        }
        ...
        this.delBinaryState = (id, options, callback) => {
            ...
            if (!adapterStates) { // if states is no longer existing, we do not need to unsubscribe
                typeof callback === 'function' && setImmediate(() => callback('States database not connected'));
                return;
            }
        ...
        this.getMessage = callback => {
            logger.warn(this.namespaceLog + ' adapter.getMessage is deprecated and will be removed in js-controller >= 3.1. Please report this warning to the developer of this adapter!');
    ...
    const exceptionHandler = (err, isUnhandledRejection) => {
        ...
        if (isUnhandledRejection) {
            logger.error('Unhandled promise rejection. This error originated either by throwing inside of an async function without a catch block, or by rejecting a promise which was not handled with .catch().');
        }
