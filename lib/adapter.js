...
function Adapter(options) {
    ...
    const initObjects = cb => {
        ...
        const setObjectWithDefaultValue = async (id, obj, options, callback) => {
            ...
            try {
                ...
                if (obj.common && obj.common.def !== undefined && obj.common.def !== null) {
        ...
        this.getAdapterObjects = callback => {
            ...
            if (!adapterObjects) { // if objects is no longer existing, we do not need to unsubscribe
                typeof callback === 'function' && setImmediate(() => callback(objects));
                return;
            }
        ...
        this.extendObject = async (id, obj, options, callback) => {
            ...
            if (!adapterObjects) { // if objects is no longer existing, we do not need to unsubscribe
                return typeof callback === 'function' && setImmediate(() => callback('Objects database not connected'));
            }
                ...
                if (common.def !== undefined) {
                    ...
                    if (typeof def !== 'number') {
                        ...
                        if (isNaN(def)) {
                            ...
                            logger.error(this.namespaceLog + ' ' + err);
                            return typeof callback === 'function' && setImmediate(() => callback(err));
                        } else {
                            common.def = def;
                        }
                    }
    ...
    const initStates = cb => {
        ...
        this.subscribeForeignStates = (pattern, options, callback) => {
            ...
            autoSubscribeOn(async () => {
                ...
                for (let s = 0; s < this.autoSubscribe.length; s++) {
                    if (typeof pattern === 'string' && (pattern === '*' || pattern.startsWith(this.autoSubscribe[s] + '.'))) {
                        ...
                        adapterStates.getState('system.adapter.' + this.autoSubscribe[s] + '.subscribes', (err, state) => {
                            ...
                            this.outputCount++;
                            adapterStates.setState('system.adapter.' + this.autoSubscribe[s] + '.subscribes', JSON.stringify(subs));
                        });
                    }
                }
                ...
                if (Array.isArray(pattern)) {
                    ...
                    const aliasesIds = pattern
                        .map(id => id && id.startsWith(ALIAS_STARTS_WITH) ? id : null)
                        .filter(id => id);
        ...
        /**
         * DEPRCATED!!! Will be removed in js-controller 3.2!!
         * @param callback
         */
        this.getMessage = callback => {
            logger.warn(this.namespaceLog + ' adapter.getMessage is deprecated and will be removed in js-controller >= 3.2. Please report this warning to the developer of this adapter!');
