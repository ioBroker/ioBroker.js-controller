...
function Adapter(options) {
    ...
    that.checkPassword = function checkPassword(user, pw, options, callback) {
        ...
        that.getForeignObject('system.user.' + user, options, function (err, obj) {
            if (err || !obj || !obj.common || (!obj.common.enabled && user !== 'admin')) {
                callback(false);
                return;
            }
    ...
    that.checkGroup = function checkGroup(user, group, options, callback) {
        ...
        that.getForeignObject('system.user.' + user, options, function (err, obj) {
            ...
            that.getForeignObject('system.group.' + group, options, function (err, obj) {
                if (err || !obj) {
                    callback(false);
                    return;
                }
    ...
    that.getCertificates = function (publicName, privateName, chainedName, callback) {
        ...
        that.getForeignObject('system.certificates', function (err, obj) {
            if (err || !obj ||
                ...
                var ca = undefined;
                if (chainedName) {
                    var chained = obj.native.certificates[chainedName].split('-----END CERTIFICATE-----\r\n');
                    ca = [];
                ...
                if (callback) {
                    callback(null, {
                        key:  obj.native.certificates[privateName],
                        cert: obj.native.certificates[publicName],
                        ca:   ca
    ...
    function initObjects(cb) {
                    ...
                    that.objects.getObjectView('system', 'device', {startkey: that.namespace + '.', endkey: that.namespace + '.\u9999', include_docs: true}, function (err, _devices) {
                        ...
                        if (_states) {
                            ...
                            for (var s = _states.rows.length - 1; s >= 0; s--) {
                                ...
                                if (that.oStates) that.oStates[id] = null;
                            }
                        }
         ...
         * @memberof Adapter
         * @param {string} pattern object ID
         ...
         * @memberof Adapter
         * @param {string} id exactly object ID (without namespace)
         * @param {string} type optional type of state: 'number', 'string', 'boolean', 'file', ...
         * @param {object} options optional user context
         * @param {function} callback return result
         *        <pre><code>
    ...
    function initStates(cb) {
        ...
        that.states = new States({
            ...
            change: function (id, state) {
                ...
                if (that.logList && id.match(/\.logging$/)) {
                    ...
                    if (logger) logger.info(that.namespace + ' ' + instance + ': logging ' + (state ? state.val : false));
        ...
        that.sendTo = function sendTo(objName, command, message, callback) {
            ...
            if (typeof message !== 'object') {
                that.log.debug('sendTo "' + command + '" to ' + objName + ' from system.adapter.' + that.namespace + ': ' + message);
            } else {
                that.log.debug('sendTo "' + command + '" to ' + objName + ' from system.adapter.' + that.namespace);
            }
        ...
        that.sendToHost = function sendToHost(objName, command, message, callback) {
            ...
            if (objName && !objName.match(/^system\.host\./)) objName = 'system.host.' + objName;
                ...
                if (that.states) {
                    if (that.logRequired === undefined && that.logRequired !== isActive) {
                        ...
                        if (!isActive) {
                            ...
                            that.log.info('Change log subscriber state: true');
