    ...
    function initRedis(cb) {
        ...
        that.sendTo = function sendTo(adapter, command, message, callback) {
            ...
            if (adapter.indexOf('.') == -1) {
                ...
                that.states.pushMessage(adapter + '.messagebox', obj);
            }
        };

        // Send message to specific host or to all hosts
        that.sendToHost = function sendToHost(host, command, message, callback) {
            if (typeof message == 'undefined') {
                message = command;
                command = 'send';
            }
            ...
            if (!host) {
                ...
                if (host.substring(0, 'system.host.'.length) != 'system.host.') host = 'system.host.' + host;

                that.states.pushMessage(host + '.messagebox', obj);
            }
        };
        ...
        that.getForeignStateHistory = function getStateHistory(id, start, end, callback) {
            ...
            end =   end     || Math.round((new Date()).getTime() / 1000);
