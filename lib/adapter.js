...
function Adapter(options) {
    ...
    const initStates = (cb) => {
        ...
        const _states = new States({
            ...
            change: (id, state) => {
                ...
                if (id === 'system.adapter.' + this.namespace + '.sigKill' && state && state.ts > this.statesConnectedTime) {
                    let sigKillVal = parseInt(state.val);
                    if (!isNaN(sigKillVal)) {
        ...
        this.getMessage = (callback) => {
            logger.warn(this.namespaceLog + ' adapter.getMessage is deprecated and will be removed in js-controller 3.0. Please report this warning to the developer of this adapter!');
