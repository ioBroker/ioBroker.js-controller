    ...
    const createInstancesObjects = (callback, objs) => {
        ...
        if (!objs || !objs.length) {
            ...
            if (obj && (obj._id || obj.type === 'meta')) {
                this.getObject(obj._id, null, (err, _obj) => {
                    if (!_obj) {
                        if (obj.common) {
                            if (obj.common.name) {
                        ...
                        this.setObject(obj._id, obj, null, err => {
            ...
            change: (id, state) => {
                ...
                if (id === 'system.adapter.' + this.namespace + '.sigKill') {
                    if (this.startedInCompactMode) {
                        ...
                        logger.warn('Got terminate signal. Desired PID: ' + (state && state.val) + ' <> own PID ' + process.pid);
                    }
        ...
        this.getForeignStates = (pattern, options, callback) => {
            ...
            this.objects.getObjectView('system', 'state', params, options, (err, res) => {
                ...
                if (options && options.user && options.user !== 'system.user.admin') {
                    ...
                    const aIds = keys.map(id => id.startsWith('alias.') ? id : null);
                ...
                if (pattern.startsWith('alias.')) {
                    if (pattern instanceof Array) {
                        ...
                        pattern.forEach(id => {
                            ...
                            this.getForeignObjects(id, null, null, options, (err, obj) => {
                                if (obj && obj.common && obj.common.alias && obj.common.alias.id) {
