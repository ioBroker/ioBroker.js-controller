...
function Adapter(options) {
    ...
    // TODO: both adapterStates and adapterObjects should include `null | undefined` in their type
    // but setting this will create hundreds of unnecessary complaints in callbacks
    ...
    const readFileCertificate = cert => {
        if (typeof cert === 'string') {
            try {
                ...
                logger.error(`${this.namespaceLog} Could not read certificate from file ${cert}: ${e}`);
            }
        }
        return cert;
    };
    ...
    const initObjects = cb => {
         ...
         * @memberof Adapter
         * @param {string} id exactly object ID (without namespace)
         * @param {ioBroker.CommonType} type optional common.type of state: 'number', 'string', 'boolean', 'file', ...
         * @param {object} options optional user context
         ...
         *            }
         *        </code></pre>
         * @returns {ioBroker.GetObjectPromise}
         */
    ...
    const reportStatus = () => {
        ...
        if (!this.startedInCompactMode) {
            ...
            try {
                ...
                logger.warn(`${this.namespaceLog} Could not query used process memory: ${err}`);
            }
            this.outputCount += 3;
    ...
    /** @type {import('@iobroker/plugin-base').PluginHandlerSettings} */
