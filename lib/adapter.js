...
function Adapter(options) {
    ...
    that.terminate = function (reason) {
        logger.warn('Terminated: ' + (reason || 'Without reason'));
        process.exit(11);
    };
        ...
        that.setObject = function setObject(id, obj, options, callback) {
            ...
            if (obj.hasOwnProperty('type')) {
                ...
                if (!obj.hasOwnProperty('common')) {
                    ...
                    if (obj.common.hasOwnProperty('def') && obj.common.def !== undefined) {
                        initialState = true;
                    } // endIf
                }
                ...
                that.objects.setObject(id, obj, options, () => {
                    if (initialState) {
                        that.setState(id, obj.common.def, true, callback);
                    } else callback;
                });
        ...
        that.states = new States({
            ...
            change: (id, state) => {
                ...
                if (id === 'system.adapter.' + that.namespace + '.sigKill') {
                    that.log.warn('Got terminate signal. Desired PID: ' + state.val + ' <> own PID ' + process.pid);
    ...
    function initLogging() {
        ...
        if (options.logTransporter) {
            ...
            that.requireLog = function (isActive) {
                that.log.warn('requireLog is not supported by this adapter! Please set common.logTransporter to true');
            };
        }
    }
