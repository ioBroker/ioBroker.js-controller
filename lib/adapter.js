...
function Adapter(options) {
    ...
    const initObjects = cb => {
        ...
        this.extendObject = async (id, obj, options, callback) => {
            ...
            if (oldObj && options && tools.isObject(options.preserve)) {
                 ...
                 */
                const rmProperties = (preserve, oldObj, newObj) => {
                    for (const prop of Object.keys(preserve)) {
                        if (tools.isObject(preserve[prop]) && newObj[prop]) {
                            rmProperties(preserve[prop], oldObj[prop], newObj[prop]);
                        } else if (newObj && newObj[prop] !== undefined && (!oldObj || oldObj[prop] !== undefined)) {
                            // newObj has the prop and oldObj has it -> remove from newObj to preserve
                            delete newObj[prop];
                        }
                    }
                };

                rmProperties(options.preserve, oldObj, obj || {});
            }
         ...
         * @param {string} id object ID, that must be extended
         * @param {object} obj part that must be extended
         * @param {object} [options] optional user context, or use attribute preserve e.g. {preserve: {common: {name: true}}} to preserve common.name
         * @param {ioBroker.SetObjectCallback} [callback] return result
         *        <pre><code>
         *            function (err, obj) {
         *                // obj is {"id": id}
    ...
    const initStates = cb => {
        ...
        this.registerNotification = async (scope, category, message) => {
            await this.sendToHostAsync(this.host, 'addNotification', {scope, category, message, instance: this.namespace});
        };
