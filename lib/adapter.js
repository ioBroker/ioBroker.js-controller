...
function Adapter(options) {
    ...
    function initCouch(callback) {
        ...
        that.sendTo = function (adapter, command, message, callback) {
            ...
            if (adapter.indexOf('.') == -1) {
                ...
                that.getForeignObject('system.adapter.' + adapter, function (err, obj) {
                    if (!err && obj.common.children && obj.common.children.length) {
                        ...
                        for (var i = 0; i < obj.common.children.length; i++) {
                            that.getForeignState(obj.common.children + '.messagebox', function (err, obj) {
                                if (!err) {
                                    obj = obj || [];
                                    obj.push({command: command, message: message});
                                    // Send to specific instance of adapter
                                    that.setForeignState(obj.common.children + '.messagebox', obj); 
                                }
                            });
                        }
                ...
                that.getForeignState('system.adapter.' + adapter + '.messagebox', function (err, obj) {
                    if (!err) {
                        obj = obj || [];
                        obj.push({command: command, message: message});
                        // Send to specific instance of adapter
                        that.setForeignState('system.adapter.' + adapter + '.messagebox', obj, callback); 
                    } else {
                ...
                change: function (id, state) {
                    if (id == 'system.adapter.' + that.namespace + '.messagebox') {
                        if (state && state.length) {
                            that.setState("messagebox", [], function () {
    ...
    function initAdapter(adapterConfig) {
        ...
        if (typeof options.message === 'function' && adapterConfig.common.messagebox) {
            that.states.setState('system.adapter.' + that.namespace + '.messagebox', {command: null, message: null}, function () {
                // HQ: is it required? or by default adapter is subscribed to all children?
                that.subscribeStates('system.adapter.' + that.namespace + '.messagebox');
            });
        } else
        if (typeof options.message === 'function' && !adapterConfig.common.messagebox) {
