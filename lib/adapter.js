...
function Adapter(options) {
    ...
    function initCouch(callback) {
        ...
        that.createDevice = function createDevice(name, channels, common, native, callback) {
            if (!name) {
            ...
            if (channels) {
                for (var t = 0; t < channels.length; t++) {
                    _channles[t] = channels[t];
                    if (_channles[t].substring(0, this.namespace.length) != this.namespace) {
                        _channles[t] = this.namespace + '.' + _channles[t];
                    }
                }
            }

            this.setObjectNotExists((name ? name.replace(/ /g, '_') : '') , {
                "type":     "device",
                "children": _channles,
                "common":   common,
                "native":   native
        ...
        that.createChannel = function createChannel(name, parentDevice, states, role, common, native, callback) {
            if (!name) {
            ...
            parentDevice = parentDevice.replace(/ /g, '_');
            name = parentDevice + '.' + name.replace(/ /g, '_');
            var _states = [];
            if (states) {
                for (var t = 0; t < states.length; t++) {
                    _states[t] = states[t];
                    if (_states[t].substring(0, name.length) != name) {
                        _states[t] = name + '.' + _states[t];
                    }
        ...
        that.createState = function createState(name, parentDevice, parentChannel, role, common, native, callback) {
            if (!role) {
                log.error("Try to create state " + parentDevice + '.' + name + " without role");
                return;
            }
            if (typeof native == "function") {
                callback = native;
            ...
            parentDevice = parentDevice.replace(/ /g, '_');
            name = parentChannel + '.' + name.replace(/ /g, '_');
            parentChannel = this.namespace + '.' + parentChannel;
            parentDevice  = this.namespace + '.' + parentDevice;
