...
function Adapter(options) {
    ...
    function initCouch(cb) {
        ...
        that.getEnum = function getEnum(_enum, callback) {
            ...
            that.objects.getObjectView('system', 'enum', {startkey: _enum + '.', endkey: _enum + '.\u9999'}, function (err, res) {
                ...
                for (var t = 0; t < res.rows.length; t++) {
                    count++;
                    that.objects.getObject(res.rows[i].id, function (err, _obj) {
        ...
        that.deleteDevice = function deleteDevice(deviceName, callback) {
            ...
            that.getObjectView('system', 'device', {startkey: deviceName, endkey: deviceName}, function (err, res) {
                var cnt = 0;
                if (res.rows.length > 1) adapter.log.warn('Found more than one device ' + deviceName);

                for (var t = 0; t < res.rows.length; t++) {
                    cnt++;
                    that.delObject(res.rows[i].id, function () {
                        cnt--;
                        if (!cnt) {
                            that.getObjectView('system', 'channel', {startkey: deviceName + '.', endkey: deviceName + '.\u9999'}, function (err, res) {
                                for (var t = 0; t < res.rows.length; t++) {
                                    cnt++;
                                    that.deleteChannel(deviceName, res.rows[i].id, function () {
                                        cnt--;
        ...
        that.deleteChannel = function deleteChannel(parentDevice, channelName, callback) {
            ...
            that.getObjectView('system', 'channel', {startkey: channelName, endkey: channelName}, function (err, res) {
                var cnt = 0;
                ...
                for (var t = 0; t < res.rows.length; t++) {
                    ...
                    that.delObject(res.rows[i].id, function () {
                        cnt--;
                        if (!cnt) {
                            that.getObjectView('system', 'state', {startkey: channelName + '.', endkey: channelName + '.\u9999'}, function (err, res) {
                                for (var t = 0; t < res.rows.length; t++) {
                                    that.deleteState(_parentDevice, _channelName, res.rows[i].id, function () {
                                        cnt--;
                                        if(!cnt && callback) callback();
                                    })
                                }
        ...
        that.getChannelsOf = function getChannelsOf(parentDevice, callback) {
            ...
            that.getObjectView('system', 'channel', {startkey: parentDevice + '.', endkey: parentDevice + '.\u9999'}, function (err, obj) {
                if (callback) {
                    if (obj.rows.length) {
                        var res = [];
                        for (var i = 0; i < obj.rows.length; i++) {
    ...
    function initRedis(cb) {
        ...
        that.sendTo = function sendTo(objName, command, message, callback) {
            ...
            if (objName.indexOf('.') == -1) {
                ...
                that.getObjectView('system', 'instance', {startkey: 'system.adapter.' + objName + '.', endkey: 'system.adapter.' + objName + '.\u9999'}, function (err, obj) {
                    if (obj) {
                        for (var i = 0; i < obj.rows.length; i++) {
                ...
                that.states.pushMessage('system.adapter.' + objName, obj);
            }
        };
    ...
    function initLogging() {
        if (!options.logTransporter && !that.ioPack.common.logTransporter) {
                        ...
                        if (obj) {
                            for (var i = 0; i < keys.length; i++) {
                                ...
                                if (obj[i] && (obj[i].indexOf('"val":true') != -1 || obj[i].indexOf('"val":"true"') != -1)) {
                                    that.logRedirect(true, keys[i].substring(0, keys[i].length - '.logging'.length ).replace(/^io\./, ''));
                                }
                            }
                        }
                    });
            ...
            that.requireLog = function (isActive) {
                if (that.states) that.states.setState('system.adapter.' + that.namespace + '.logging', isActive, true);
            };
