...
function StatesInMemClient(settings) {
    ...
    var __construct = (function () {
        if (!settings.secure) {
            client = io.connect('http://' + (settings.bind != '0.0.0.0' ? settings.bind || '127.0.0.1' : '127.0.0.1') + ':' + (settings.port || 9000));
        } else {
            client = io.connect('https://' + (settings.bind != '0.0.0.0' ? settings.bind || '127.0.0.1' : '127.0.0.1') + ':' + (settings.port || 9000));
        }
        ...
        client.on('reconnect', function (error) {
            ...
            for (var t in subscribes) {
                for (var i = 0; i < subscribes[t].length; i++) client.emit(t, subscribes[t][i]);
            }
            if (typeof settings.connected === 'function') settings.connected();
        });
    })();
