...
function ObjectsInMemServer(settings) {
    ...
    function prepareRights(options) {
        ...
        if (!options.user) {
            ...
            fOptions.acl.ownerGroup = options.group || 'system.group.administrator';
            fOptions.acl.permissions = 0x744;
        }
    ...
    function checkFileRights(id, name, options, flag, callback) {
        ...
        if (!options.permissions) {
            that.getUserGroup(options.user, function (user, groups, permissions) {
                options.permissions = permissions ? permissions.groupPermissions : {};
        ...
        if (fileOptions[id][name].acl && fileOptions[id][name].acl.owner != options.user) {
            // If no one else may write this file
            if (!fileOptions[id][name].acl.permissions & flag) {
                return  callback('permission denied', options);
            }
        } // else everything is OK. This is owner
        return callback(null, options);
    }
    ...
    this.writeFile = function (id, name, data, options, callback) {
        ...
        var fOptions = prepareRights(options);
        ...
        try {
            ...
            if (!options.checked && fileOptions[id][name] && fileOptions[id][name].acl) {
                return checkFileRights(id, name, options, 0x2/*write*/, function (err, options) {
                    that.writeFile(id, name, data, options, callback);
                });
            }
            ...
            fOptions.mimeType       = options.mimeType || _mimeType;
            fOptions.binary         = isBinary;
            fOptions.acl.ownerGroup = options.groups ? options.groups[0] : '';

            fileOptions[id][name] = fOptions;
    ...
    this.getUserGroup = function (user, callback) {
        if (users[user]) {
            return callback.call(that, user, users[user].groups, users[user]);
        }
        ...
        this.getObjectList({startkey: 'system.group.', endkey: 'system.group.\u9999'}, function (err, arr) {
            ...
            that.getObjectList({startkey: 'system.user.', endkey: 'system.user.\u9999'}, function (err, arr) {
                ...
                for (var i = 0; i < arr.rows.length; i++) {
                    users[arr.rows[i].value._id] = {
                        groups:           [],
                        userPermissions:  arr.rows[i].value.common.permissions || {
                            files:   0x111,
                            states:  0x777,
                            objects: 0x777
                        },
                    ...
                    if (arr.rows[i].value._id == 'system.user.admin') {
                        users[arr.rows[i].value._id].userPermissions.files = users[arr.rows[i].value._id].userPermissions.files | 0x777;
                    }
                ...
                for (var g = 0; g < groups.length; g++) {
                    for (var m = 0; m < groups[g].common.members.length; m++) {
                        ...
                        if (groups[g].common.permissions) {
                            users[u].groupPermissions.files   = users[u].groupPermissions.files   | groups[g].common.permissions.files;
                            users[u].groupPermissions.states  = users[u].groupPermissions.states  | groups[g].common.permissions.states;
                            users[u].groupPermissions.objects = users[u].groupPermissions.objects | groups[g].common.permissions.objects;
                        }
                    }
                }

                callback.call(that, user, users[user] ? users[user].groups : {}, users[user]);
            });
        });
    }
    ...
    this.readFile = function (id, name, options, callback) {
        ...
        // Before files converted, lets think: if no options it is admin
        if (!options.permissions) {
            this.getUserGroup(options.user, function (user, groups, permissions) {
                options.permissions = permissions ? permissions.groupPermissions : {};
    ...
    this.readDir = function (id, name, options, callback) {
        ...
        for (var j = 0; j < files.length; j++) {
            if (fs.existsSync(objectsDir + id + '/' + name + files[j])) {
                ...
                res.push({file: files[j], stats: stats, isDir: stats.isDirectory()});
            }
        }
