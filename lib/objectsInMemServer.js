...
function ObjectsInMemServer(settings) {
    ...
    function checkObject(id, options) {
        ...
        if (options.user != 'system.user.admin' &&
            ...
            if (objects[id].acl.object.owner != options.user) {
                // Check if the user is in the group
                if (options.groups.indexOf(objects[id].acl.object.ownerGroup) != -1) {
                    // Check group rights
                    if (!(objects[id].acl.object.permissions & (flag << 4))) {
                        return false
                    }
                } else {
                    // everybody
                    if (!(objects[id].acl.object.permissions & flag)) {
                        return false
                    }
                }
            } else {
                // Check group rights
                if (!(objects[id].acl.object.permissions & (flag << 8))) {
                    return false
                }
            }
        }
    ...
    function checkObjectsRights(id, options, flag, callback) {
        options = options || {};
        if (!options.user) {
        ...
        if (!options.acl) {
            that.getUserGroup(options.user, function (user, groups, acl) {
                ...
                checkObjectsRights(id, options, flag, callback);
            });
            return;
        }
        // If user may write
    ...
    this.getObject = function (id, options, callback) {
        ...
        if (!options || !options.checked) {
            checkObjectsRights(id, options, 0x4/*read*/, function (err, options) {
                if (err) {
                    return callback(err);
                } else {
    ...
    this.getConfigKeys = function (pattern, options, callback, dontModify) {
        ...
        if (!options || !options.checked) {
            checkObjectsRights(null, options, 'list', function (err, options) {
                if (err) {
                    return callback(err);
                } else {
        ...
        for (var id in objects) {
            if (r.test(id) && checkObject(id, options)) result.push(id);
        }
        if (typeof callback === 'function') {
            setTimeout(function () {
    ...
    this.getObjects = function (keys, options, callback, dontModify) {
        ...
        if (!options || !options.checked) {
            checkObjectsRights(null, options, 0x4/*read*/, function (err, options) {
                if (err) {
                    return callback(err);
                } else {
        ...
        for (var i = 0; i < keys.length; i++) {
            if (checkObject(keys[i], options)) {
                result.push(clone(objects[keys[i]]));
            } else {
                result.push({error: 'permission denied'});
            }
        }
        if (typeof callback === 'function') {
            setTimeout(function () {
    ...
    this.setObject = function (id, options, obj, callback) {
        if (typeof options == 'function') {
            callback = options;
            options = null;
        }
        if (!options || !options.checked) {
            checkObjectsRights(id, options, 0x2/*write*/, function (err, options) {
                if (err) {
                    return callback(err);
                } else {
                    return this.setObject(id, options, obj, callback);
                }
            }.bind(this));
            return;
        }
    ...
    this.delObject = function (id, options, callback) {
        ...
        if (!options || !options.checked) {
            checkObjectsRights(id, options, 'delete', function (err, options) {
                if (err) {
                    return callback(err);
                } else {
        ...
        if (!options || !options.checked) {
            checkObjectsRights(null, options, 'list', function (err, options) {
                if (err) {
                    ...
                    return this.subscribeConfig(pattern, options);
                }
            }.bind(this));
            return;
        }
    ...
    this.writeFile = function (id, name, data, options, callback) {
        ...
        try {
            ...
            if ((!options || !options.checked) && fileOptions[id][name] && fileOptions[id][name].acl) {
        ...
        for (var j = 0; j < files.length; j++) {
            ...
            if (fs.existsSync(objectsDir + id + '/' + name + files[j])) {
                ...
                var acl = (fileOptions[id][name + files[j]] && fileOptions[id][name + files[j]].acl) ?
                    JSON.parse(JSON.stringify(fileOptions[id][name + files[j]].acl)) :
                   {read: true, write : true, permissions: 0x644, owner: 'system.user.admin', groupOwner: 'system.group.administrator'};
                ...
                res.push({file: files[j], stats: stats, isDir: stats.isDirectory(), acl: acl});
            }
        }
            ...
            for (var f in fileOptions[id]) {
                if (regEx.test(f)) {
                    ...
                    if (fs.existsSync(objectsDir + id + '/' + f)) {
                        ...
                        processed.push({path: parts.join('/'), file: fileName, stats: stats, isDir: stats.isDirectory(), acl: acl ? acl.acl : {}});
                    }
                }
            }
    ...
    this.getObjectList = function (params, options, callback) {
        ...
        for (var id in objects) {
            if (!checkObject(id, options)) continue;
            if (params) {
                if (params.startkey && id < params.startkey) continue;
                if (params.endkey   && id > params.endkey)   continue;
    ...
    this.extendObject = function (id, obj, options, callback) {
        ...
        if (!options || !options.checked) {
            checkObjectsRights(id, options, 2/*write*/, function (err, options) {
                if (err) {
                    return callback(err);
                } else {
