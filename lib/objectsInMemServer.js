...
function ObjectsInMemServer(settings) {
    ...
    function storeHistory(id, obj) {
        ...
        if (fs.existsSync(file)) {
            ...
            data.splice(data.length, 0, obj.common.data);
        } else {
            data = obj.common.data;
        }
    ...
    this.getConfig = function (id, callback) {
        if (typeof callback === 'function') {
            ...
            if (regHistory.test(id)) loadHistory(id, obj);
    ...
    this.writeFile = function (id, name, data, mimeType, callback) {
        ...
        types[id][name] = mimeType;
        files[id][name] = data;
        try {
            // Store file
            fs.writeFileSync(objectsDir + id + '/' + name, data, {'flags': 'w', 'encoding': 'binary'});
            // Store dir description
            fs.writeFileSync(objectsDir + id + '/_data.json', JSON.stringify(types[id]));
        } catch(e) {
            log.error('Cannot write files: ' + objectsDir + id + '/' + name);
        }
    ...
    this.readFile = function (id, name, params, callback) {
        ...
        if (!files[id][name]) {
            if (fs.existsSync(objectsDir + id + '/' + name)) {
                ...
                if (files[id][name]) files[id][name].toString();
            } else {
                files[id][name] = null;
            }
        }
    ...
    this.extendObject = function (id, obj, callback) {
        ...
        if (typeof callback === 'function') callback(null, objects[id], id);
        if (!configTimer) configTimer = setTimeout(saveConfig, 5000);
    };
    ...
    function socketEvents(socket, user) {
        ...
        socket.on('unlink', function (id, name, rev, callback) {
