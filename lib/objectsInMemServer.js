    ...
    this.readDir = function (id, name, options, callback) {
        ...
        for (var j = 0; j < files.length; j++) {
            if (fs.existsSync(objectsDir + id + '/' + name + files[j])) {
                ...
                var file = JSON.parse(JSON.stringify(fileOptions[id][name + files[j]]));

                // if filter for user
                if (options.filter && file && file.acl) {
                    // If user may not write
                    if (!options.acl.file.write) {// write
                        file.acl.permissions &= ~0x222;
                    }
                    // If user may not read
                    if (!options.acl.file.read) {// read
                        file.acl.permissions &= ~0x444;
                    }

                    if (options.groups.indexOf('system.group.administrator') == -1) {
                        if (file.acl.owner != options.user) {
                            // Check if the user is in the group
                            if (options.groups.indexOf(file.acl.ownerGroup) != -1) {
                                // Check group rights
                                ...
                                // everybody
                                if (!(file.acl.permissions & 0x6)) {
                                    continue;
                                }
                            }
                        } else {
                            // Check group rights
                            if (!(acl.acl.permissions & (0x6 << 8))) {
                                continue;
                            }
                        }
                    }
                ...
                res.push({file: files[j], stats: stats, isDir: stats.isDirectory(), acl: file ? file.acl : {}});
            }
        }
