...
class RedisHandler extends EventEmitter {
    ...
    _handleCommand(data) {
        ...
        if (this.handleBuffers) {
            ...
            if (data[0] && Buffer.isBuffer(data[0])) data[0] = data[0].toString('utf-8');
        ...
        this.log.silly(this.sockedId + ' Parser result: id=' + responseId + ', command=' + command + ', data=' + ((JSON.stringify(data).length > 1024) ? JSON.stringify(data).substring(0,100) + ' -- ' + JSON.stringify(data).length + ' bytes' : JSON.stringify(data))); // TODO remove data logging (performance)
    ...
    _sendQueued(responseId, data) {
        ...
        while (this.writeQueue.length && idx < this.writeQueue.length) {
            ...
            if (idx === 0 && this.writeQueue[idx].data !== false) {
                ...
                this.log.silly(this.sockedId + ' Redis response (' + response.id + '): ' + ((response.data.length > 1024) ? data.length + ' bytes' : response.data.toString().replace(/[\r\n]+/g, ''))); // TODO remove data logging (performance)
