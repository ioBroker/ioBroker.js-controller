...
class RedisHandler extends EventEmitter {
    ...
    constructor(socket, options) {
        ...
        if (this.logScope.length) this.logScope += ' ';
        this.socket = socket;

        this.sockedId = this.logScope + socket.remoteAddress + ':' + socket.remotePort;
        ...
        socket.on('data', (data) => {
            if (this.options.enhancedLogging) this.log.silly(this.sockedId + ' New Redis request: ' + ((data.length > 1024) ? data.toString().replace(/[\r\n]+/g, '').substring(0,100) + ' -- ' + data.length + ' bytes' : data.toString().replace(/[\r\n]+/g, '')));
    ...
    _handleCommand(data) {
        ...
        if (this.handleBuffers) {
            ...
            if (Buffer.isBuffer(data[0])) data[0] = data[0].toString('utf-8');
            // Binary data only relevant for GET and SET in our case
            if (command !== 'set' && data.length > 1) {
                for (let i = 1; i < data.length; i++) {
                    if (Buffer.isBuffer(data[i])) data[i] = data[i].toString('utf-8');
                }
            }
        }
        ...
        if (this.options.enhancedLogging) this.log.silly(this.sockedId + ' Parser result: id=' + responseId + ', command=' + command + ', data=' + ((JSON.stringify(data).length > 1024) ? JSON.stringify(data).substring(0,100) + ' -- ' + JSON.stringify(data).length + ' bytes' : JSON.stringify(data)));
    ...
    _sendQueued(responseId, data) {
        ...
        while (this.writeQueue.length && idx < this.writeQueue.length) {
            ...
            if (idx === 0 && this.writeQueue[idx].data !== false) {
                ...
                if (this.options.enhancedLogging) this.log.silly(this.sockedId + ' Redis response (' + response.id + '): ' + ((response.data.length > 1024) ? data.length + ' bytes' : response.data.toString().replace(/[\r\n]+/g, '')));
                ...
                if (this.writeQueue.length && this.writeQueue[idx].data === false) break;
            }
        ...
        if (idx > 0) {
            if (this.options.enhancedLogging) this.log.silly(this.sockedId + ' Redis response (' + responseId + '): Response queued');
        }
    }
    ...
    sendResponse(responseId, data) {
        ...
        if (responseId === null) {
            if (this.options.enhancedLogging) this.log.silly(this.sockedId + ' Redis response DIRECT: ' + data.toString().replace(/[\r\n]+/g, '')); // TODO remove data logging (performance)
        ...
        if (!data) {
            this.log.warn('Not able to write ' + JSON.stringify(data));
    ...
    sendError(responseId, error) {
        this.log.warn('Error from InMemDB: ' + error);
