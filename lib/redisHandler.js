...
class RedisHandler extends EventEmitter {
    ...
    constructor(socket, options) {
        ...
        socket.on('data', (data) => {
            if (this.options.enhancedLogging) this.log.silly(this.socketId + ' New Redis request: ' + ((data.length > 1024) ? data.toString().replace(/[\r\n]+/g, '').substring(0,100) + ' -- ' + data.length + ' bytes' : data.toString().replace(/[\r\n]+/g, '')));
    ...
    _sendQueued(responseId, data) {
        ...
        while (this.writeQueue.length && idx < this.writeQueue.length) {
            ...
            if (idx === 0 && this.writeQueue[idx].data !== false) {
                ...
                if (this.options.enhancedLogging) this.log.silly(this.socketId + ' Redis response (' + response.id + '): ' + ((response.data.length > 1024) ? data.length + ' bytes' : response.data.toString().replace(/[\r\n]+/g, '')));
        ...
        if (idx > 0) {
            if (this.options.enhancedLogging) this.log.silly(this.socketId + ' Redis response (' + responseId + '): Response queued');
        }
    }
    ...
    sendResponse(responseId, data) {
        ...
        if (responseId === null) {
            if (this.options.enhancedLogging) this.log.silly(this.socketId + ' Redis response DIRECT: ' + data.toString().replace(/[\r\n]+/g, '')); // TODO remove data logging (performance)
