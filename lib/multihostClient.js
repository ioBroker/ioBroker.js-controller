    ...
    this.browse = function (timeout, isDebug, callback) {
        ...
        startServer(true, timeout,
            ...
            function onMessage (srv, msg, rinfo) {
                // ignore own answers
                if (1 || rinfo.address !== '127.0.0.1' && ownIps.indexOf(rinfo.address) === -1) {
                    ...
                    if (msg.result === 'ok') {
                        if (callback) {
                            if (!msg.objects) {
                                ...
                                callback(null, msg.objects, msg.states);
                            }
                        }
                        ...
                        if (!password) {
                            ...
                            // this is not the solution. Just now I dont have the solution for that :( Just be sure that this is DMZ.
                            crypto = crypto || require('crypto');
                            var cipher = crypto.createCipher('aes192', msg.salt);
