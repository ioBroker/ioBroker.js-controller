...
function _addFile(objects, id, name, options, zip, files, callback) {
    objects.readFile(id, name, options, function (err, data, _mime) {
        if (!zip) {
            console.log(err);
...
function _checkDir(objects, id, root, parts, options, callback) {
    ...
    root += '/' + parts.shift();
    objects.readDir(id, root, options, function (err, _files) {
        if (err === tools.ERRORS.ERROR_NOT_EXISTS) {
            objects.mkdir(id, root, options, function (_err) {
