    ...
    try {
        zip.loadAsync(data).then(function () {
            ...
            for (const filename in zip.files) {
                if (!zip.files.hasOwnProperty(filename) || !filename || filename[filename.length - 1] === '/') continue;
                count++;
                try {
    ...
    try {
        zip.loadAsync(data)
            .then(() => {
                ...
                const error = [];
                for (const filename in zip.files) {
                    if (!zip.files.hasOwnProperty(filename) || filename[filename.length - 1] === '/') continue;
                    count++;
