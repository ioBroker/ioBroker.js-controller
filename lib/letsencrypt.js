...
async function createServerAsync(app, settings, certificates, leSettings, log, adapter) {
    ...
    for (const domain of leSettings.domains) {
        ...
        try {
            await gl.add({subject: domain, altnames: [domain]});
            addedDomains++;
        } catch (err) {
            log.warn(`Invalid domain "${domain}" provided for Let's Encrypt was not added to configuration: ${err}`);
        }
    }

    if (!addedDomains) {
        log.warn(`No domains available for Let's Encrypt, reuse old certificates`);
        return require('https').createServer(certificates, app);
    }
