        ...
        return await new Promise(resolve => {
            require('greenlock-express').init({
                ...
                if (settings.leUpdate) {
                    // Start the challenge server. Wait for it before resolving with https.
                    ...
                    httpServer.listen(settings.lePort, bind, err => {
                        if (err) {
                            throw new Error(`Failed to start challenge server listen on port $(settings.lePort}: ${err}`);
                        }
