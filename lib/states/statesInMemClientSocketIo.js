...
/** @class */
function StatesInMemClientSocketIo(settings) {
    let client;
    const subscribes = {};
    ...
    (function __construct() {
        ...
        client.on('connect', function (_error) {
            if (connectionTimeout) {
                clearTimeout(connectionTimeout);
                connectionTimeout = null;
            }
        ...
        client.on('reconnect', function (_error) {
            // Re-init subscribes
            for (const t in subscribes) {
                for (let i = 0; i < subscribes[t].length; i++) {
    ...
    this.unsubscribe = function (pattern, callback) {
        if (subscribes.subscribe) {
            const pos = subscribes.subscribe.indexOf(pattern);
    ...
    this.unsubscribeMessage = function (pattern, callback) {
        if (subscribes.subscribeMessage) {
            const pos = subscribes.subscribeMessage.indexOf(pattern);
    ...
    this.unsubscribeLog = function (pattern, callback) {
        if (subscribes.subscribeLog) {
            const pos = subscribes.subscribeLog.indexOf(pattern);
    ...
    this.getConfigKeys = function (pattern, callback, _dontModify) {
        if (!client) return;
        client.emit('getConfigKeys', pattern, callback);
    };

    this.getConfigs = function (keys, callback, _dontModify) {
        if (!client) return;
    ...
    this.unsubscribeConfig = function (pattern, callback) {
        if (subscribes.subscribeConfig) {
            const pos = subscribes.subscribeConfig.indexOf(pattern);
