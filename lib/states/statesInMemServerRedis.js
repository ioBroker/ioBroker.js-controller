...
class StatesInMemoryServer extends StatesInMemoryFileDB {
    ...
    constructor(settings) {
        ...
        if (this.settings.connected) {
            setImmediate(() => this.settings.connected('Redis InMemoryDB states'));
        }
    }
    ...
    publishToClients(client, type, id, obj) {
        ...
        if (found) {
            ...
            this.log.silly('Redis Publish ' + id + '=' + objString);
    ...
    _socketEvents(handler) {
        ...
        handler.on('setex', (data, responseId) => {
            ...
            if (namespace === this.namespaceStates) {
                try {
                    const state = JSON.parse(data[2]);
                    state.expire = parseInt(data[1], 10);
                    this.setState(id, state, (err, id) => {
                        if (err || !id) {
                            handler.sendError(responseId, new Error('ERROR id=' + id + ' - ' + err)); // TODO
                        } else {
                ...
                try {
                    const state = JSON.parse(data[2]);
                    this.setSession(id, parseInt(data[1], 10), state, () => {
                        handler.sendString(responseId, 'OK');
                    });
                } catch (err) {
                    handler.sendError(responseId, new Error('ERROR ' + err)); // TODO
     ...
     */
    _initSocket(socket) {
        this.log.silly('Handling new Redis connection');
        const options = {log: this.log};
    ...
    _initRedisServer(settings, server) {
        ...
        this.log.info(this.namespace + ' ' + (settings.secure ? 'Secure ' : '') + ' Redis inMem-states listening on port ' + (settings.port || 9000));
    }
}
