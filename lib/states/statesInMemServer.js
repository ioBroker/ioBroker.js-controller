...
function StatesInMemory(settings) {
    ...
    function subscribe(socket, type, pattern, cb) {
        ...
        //
        s.push({pattern: pattern, regex: new RegExp(tools.pattern2RegEx(pattern))});
        typeof cb === 'function' && cb();
    }
    ...
    function saveState() {
        if (fs.existsSync(statesName)) {
            const old = fs.readFileSync(statesName);
        ...
        try {
            ...
            if (!settings.backup.disabled) {
                ...
                if (settings.backup.period && (!lastSave || now - lastSave > settings.backup.period)) {
                    lastSave = now;
                    const backFileName = backupDir + getTimeStr(now) + '_states.json.gz';
    ...
    function socketEvents(socket, _user) {
        /*
         *      states
         */
        socket.on('getStates', function (_keys, _callback, _dontModify) {
            that.getStates.apply(that, arguments);
        });
        socket.on('getState', function (_id, _callback) {
            that.getState.apply(that, arguments);
        });
        socket.on('setState', function (_id, _state, _callback) {
            that.setState.apply(that, arguments);
        });
        socket.on('setRawState', function (_id, _state, _callback) {
            that.setRawState.apply(that, arguments);
        });
        socket.on('delState', function (_id, _callback) {
            that.delState.apply(that, arguments);
        });
        socket.on('getKeys', function (_pattern, _callback, _dontModify) {
            that.getKeys.apply(that, arguments);
        });
        socket.on('subscribe', function (_pattern, _callback) {
            that.subscribe.apply(this, arguments);
        });
        socket.on('unsubscribe', function (_pattern, _callback) {
        ...
        // });
        socket.on('pushMessage', function (_id, _state, _callback) {
            that.pushMessage.apply(that, arguments);
        });
        socket.on('lenMessage', function (_id, _callback) {
            that.lenMessage.apply(that, arguments);
        });
        socket.on('getMessage', function (_id, _callback) {
            that.getMessage.apply(that, arguments);
        });
        socket.on('delMessage', function (_id, _messageId, _callback) {
            that.delMessage.apply(that, arguments);
        });
        socket.on('subscribeMessage', function (_id, _callback) {
            that.subscribeMessage.apply(this, arguments);
        });
        socket.on('unsubscribeMessage', function (_id, _callback) {
            that.unsubscribeMessage.apply(this, arguments);
        });
        socket.on('pushLog', function (_id, _state, _callback) {
            that.pushLog.apply(that, arguments);
        });
        socket.on('lenLog', function (_id, _callback) {
            that.lenLog.apply(that, arguments);
        });
        socket.on('getLog', function (_id, _callback) {
            that.getLog.apply(that, arguments);
        });
        socket.on('delLog', function (_id, _logId, _callback) {
            that.delLog.apply(that, arguments);
        });
        socket.on('subscribeLog', function (_id, _callback) {
            that.subscribeLog.apply(this, arguments);
        });
        socket.on('unsubscribeLog', function (_id, _callback) {
            that.unsubscribeLog.apply(this, arguments);
        });
        socket.on('getSession', function (_id, _callback) {
            that.getSession.apply(that, arguments);
        });
        socket.on('setSession', function (_id, _expire, _obj, _callback) {
            that.setSession.apply(that, arguments);
        });
        socket.on('destroySession', function (_id, _callback) {
            that.destroySession.apply(that, arguments);
        });
        socket.on('getConfig', function (_id, _callback) {
            that.getConfig.apply(that, arguments);
        });
        socket.on('getConfigKeys', function (_pattern, _callback, _dontModify) {
            that.getConfigKeys.apply(that, arguments);
        });
        socket.on('getConfigs', function (_keys, _callback, _dontModify) {
            that.getConfigs.apply(that, arguments);
        });
        socket.on('setConfig', function (_id, _obj, _callback) {
            that.setConfig.apply(that, arguments);
        });
        socket.on('delConfig', function (_id, _callback) {
            that.delConfig.apply(that, arguments);
        });
        socket.on('subscribeConfig', function (_pattern, _callback) {
            that.subscribeConfig.apply(this, arguments);
        });
        socket.on('unsubscribeConfig', function (_pattern, _callback) {
            that.unsubscribeConfig.apply(this, arguments);
        });
        socket.on('setBinaryState', function (_id, _data, _callback) {
            that.setBinaryState.apply(that, arguments);
        });
        socket.on('getBinaryState', function (_id, _callback) {
            that.getBinaryState.apply(that, arguments);
        });
        socket.on('delBinaryState', function (_id, _callback) {
    ...
    this.getStates = function (keys, callback, _dontModify) {
        if (!keys) {
    ...
    this.setState = function (id, state, callback) {
        ...
        if (state.expire) {
            ...
            if (!expiresInterval) {
                ...
                if (lastExpire) state.expire -= (Date.now() - lastExpire);
            }
    ...
    this.getKeys = function (pattern, callback, _dontModify) {
    ...
    this.unregisterAdapterSubs = function (instance, cb) {
        const pos = adapterSubs.indexOf(instance);
        if (pos !== -1) {
    ...
    this.lenMessage = function (id, callback) {
        if (messagebox[id]) {
            ...
            if (typeof callback === 'function') callback(tools.ERRORS.ERROR_NOT_EXISTS, null);
        }
    };
    ...
    this.delMessage = function (id, messageId, callback) {
        if (messagebox[id]) {
            ...
            if (!found) {
                ...
                if (typeof callback === 'function') callback(tools.ERRORS.ERROR_NOT_EXISTS);
            } else {
    ...
    this.lenLog = function (id, callback) {
        if (logs[id]) {
            ...
            if (typeof callback === 'function') callback(tools.ERRORS.ERROR_NOT_EXISTS, null, id);
        }
    };
    ...
    this.getLog = function (id, callback) {
        if (logs[id]) {
            ...
            if (typeof callback === 'function') callback(tools.ERRORS.ERROR_NOT_EXISTS, null, 0);
        }
    };
    ...
    this.delLog = function (id, logId, callback) {
        if (logs[id]) {
            ...
            callback(tools.ERRORS.ERROR_NOT_EXISTS);
        }
    };
    ...
    this.setSession = function (id, expire, obj, callback) {
        ...
        if (!expiresInterval) {
            ...
            if (lastExpire) session[id]._expire -= Date.now() - lastExpire;
        }
