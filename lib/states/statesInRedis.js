...
class StateRedis {
        ...
        this.client.on('ready', () => {
            ...
            if (!this.subSystem && typeof onChange === 'function') {
                ...
                this.client.config('set', ['notify-keyspace-events', 'Exe'], (err) => { // enable Expiry/Evicted events in server
                    ...
                    if (typeof onChange === 'function' || typeof onChangeUser === 'function') {
                        // subscribe on key deleted message
                        this.subSystem.on('message', (channel, message) =>
                            setImmediate(() => {
                                this.log.silly(this.namespace + ' redis message about deletion/evicted ' + channel + ':' + message);
                                try {
                    ...
                    this.subSystem.on('error', error => {
                        if (this.stop) return;
                        if (this.settings.connection.enhancedLogging) this.log.silly(this.namespace + ' Sub-Client States System No redis connection: ' + JSON.stringify(error));
                    });
                    ...
                    this.subSystem.on('ready', _error => {
                        this.subSystem.subscribe('__keyevent@' + this.settings.connection.options.db + '__:expired', err => {
                            err && this.log.warn('Unable to subscribe to expiry Keyspace events from Redis Server: ' + err)

                            this.subSystem.subscribe('__keyevent@' + this.settings.connection.options.db + '__:evicted', err => {
            ...
            if (!this.sub && typeof onChangeUser === 'function') {
                ...
                this.sub.on('error', error => {
                    if (this.stop) return;
                    if (this.settings.connection.enhancedLogging) this.log.silly(this.namespace + ' Sub-Client States User No redis connection: ' + JSON.stringify(error));
                });
    ...
    subscribe(pattern, subClient, callback) {
        ...
        if (!subClient) {
            return typeof callback === 'function' && callback(tools.ERRORS.ERROR_DB_CLOSED);
        }
