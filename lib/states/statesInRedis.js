...
class StateRedis {
    ...
    constructor(settings) {
        ...
        if (this.settings.connection.port === 0) {
            ...
            this.client = new Redis(this.settings.connection.host, this.settings.connection.options);
        } else {
            this.client = new Redis(this.settings.connection.port, this.settings.connection.host, this.settings.connection.options);
        }
        ...
        this.client.on('ready', () => {
            ...
            if (!ready) {
                if (this.settings.connection.port === 0) {
                    // initiate a unix socket connection using the parameter 'host'
                    this.sub = new Redis(this.settings.connection.host, this.settings.connection.options);
                } else {
                    this.sub = new Redis(this.settings.connection.port, this.settings.connection.host, this.settings.connection.options);
                }
    ...
    delBinaryState(id, callback) {
        this.client.del(this.namespaceRedis + id, () => typeof callback === 'function' && callback());
    }
}
