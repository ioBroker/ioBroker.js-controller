...
//    var namespaceFifo    = (settings.namespaceFifo    || 'history') + '.';
    ...
    //     log.silly(settings.namespace + ' trimFifo history.' + id + ' minLength=' + minLength + ' maxLength=' + maxLength);
    //     this.lenFifo(id, function (err, res) {
    //         log.silly(settings.namespace + ' lenFifo ' + id + ' ' + res);
    //         if (!err && res) {
    //             var length = parseInt(res, 10);
    //             if (length <= maxLength) {
    //                 if (typeof callback === 'function') callback(null, []);
    //             } else {
    //                 log.silly(settings.namespace + ' lrange history.' + id + ' ' + (0 - (length - minLength)) + ' -1');
    ...
    //                         obj[i] = obj[i] ? JSON.parse(obj[i]) : null;
    //                     }
    //                     log.silly(settings.namespace + ' ltrim history.' + id + ' 0 ' + (minLength - 1));
    ...
    this.pushMessage = function (id, state, callback) {
        ...
        //client.lpush(namespaceMsg + id, JSON.stringify(state), function (err, obj) {
            //client.ltrim(0, settings.connection.maxQueue - 1, function (err) {
                // publish event in redis
                //log.silly('redis publish ' + namespaceMsg + id + ' ' + JSON.stringify(state));
    ...
    this.pushLog = function (id, log, callback) {
        ...
        // client.lpush(namespaceLog + id, JSON.stringify(state), function (err, obj) {
            // client.ltrim(0, settings.connection.maxQueue - 1, function (err) {
                // publish event in redis
                //log.silly('redis publish ' + redisNamespace + id + ' ' + JSON.stringify(log));
