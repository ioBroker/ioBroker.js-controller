...
//    var namespaceFifo    = (settings.namespaceFifo    || 'history') + '.';
    ...
    var __construct = (function () {
        ...
        if (typeof settings.change === 'function') {
            sub.on('pmessage', function (pattern, channel, message) {
                log.debug(settings.namepsace + ' redis pmessage ', pattern, channel, message);
                try {
                    ...
                    log.error(settings.namepsace + ' pmessage ' + channel + ' ' + message + ' ' + e.message);
                    log.error(settings.namepsace + ' ' + e.stack);
                }
            });
        }
        ...
        client.on('error', function (error) {
            if (typeof settings.disconnected === 'function') {
                ...
                log.error(settings.namepsace + ' ' + error.message);
                log.error(settings.namepsace + ' ' + error.stack);
            }
        });

        sub.on('error', function (error) {
            log.error(settings.namepsace + ' No redis connection!');
        });

        sub.on('connect', function (error) {
            log.info(settings.namepsace + ' States connected to redis ' + settings.connection.host + ':' + settings.connection.port);
        });
    ...
    this.setState = function (id, state, callback) {
        ...
        client.get(namespace + id, function (err, oldObj) {
            // TODO Error Handling
            if (err) log.warn(settings.namepsace + ' ' + err);
            ...
            log.debug(settings.namepsace + ' redis publish ' + namespace + id + ' ' + JSON.stringify(obj));
    ...
    this.getState = function (id, callback) {
        client.get(namespace + id, function (err, obj) {
            if (err) {
                log.warn(settings.namepsace + ' redis get ' + id + ', error - ' + err);
            } else {
                log.debug(settings.namepsace + ' redis get ' + id + ' ok: ' + obj);
            }
            if (typeof callback === 'function') {
    ...
    this.getStates = function (keys, callback, dontModify) {
        ...
        client.mget(_keys, function (err, obj) {
            if (err) {
                log.warn(settings.namepsace + ' redis mget ' + ((!obj) ? 0 :  obj.length) + ' ' + _keys.length + ', err: ' + err);
            } else {
                log.debug(settings.namepsace + ' redis mget ' + ((!obj) ? 0 :  obj.length) + ' ' + _keys.length);
            }
    ...
    this.delState = function (id, callback) {
        client.del(namespace + id, function (err) {
            if (err) {
                log.warn(settings.namepsace + ' redis del ' + id + ', error - ' + err);
            } else {
                client.publish(namespace + id, 'null');
                log.debug(settings.namepsace + ' redis del ' + id + ', ok');
            }
    ...
    this.getKeys = function (pattern, callback, dontModify) {
        client.keys(namespace + pattern, function (err, obj) {
            log.debug(settings.namepsace + ' redis keys ' + obj.length + ' ' + pattern);
            if (typeof callback === 'function') {
                if (obj && !dontModify) {
    ...
    this.subscribe = function (pattern, callback) {
        log.debug(settings.namepsace + ' redis psubscribe ' + namespace + pattern);
    ...
    this.unsubscribe = function (pattern, callback) {
        log.debug(settings.namepsace + ' redis punsubscribe ' + namespace + pattern);
    ...
    this.trimFifo = function (id, minLength, maxLength, callback) {
        log.debug(settings.namepsace + ' trimFifo history.' + id + ' minLength=' + minLength + ' maxLength=' + maxLength);
        this.lenFifo(id, function (err, res) {
            log.debug(settings.namepsace + ' lenFifo ' + id + ' ' + res);
            if (!err && res) {
                var length = parseInt(res, 10);
                if (length <= maxLength) {
                    if (typeof callback === 'function') callback(null, []);
                } else {
                    log.debug(settings.namepsace + ' lrange history.' + id + ' ' + (0 - (length - minLength)) + ' -1');
                    client.lrange(namespaceFifo + id, 0 - (length - minLength), -1, function (err, obj) {
                        ...
                        log.debug(settings.namepsace + ' ltrim history.' + id + ' 0 ' + (minLength - 1));
        ...
        client.lpop(namespaceMsg + id, function (err, obj) {
            if (typeof callback === 'function') {
                try {
                    ...
                    obj = null;
                    log.error(settings.namepsace + ' Cannot parse messagebox object');
                }
                if (obj) {
                    callback(err, obj);
                } else {
        ...
        client.lpop(namespaceLog + id, function (err, obj) {
            if (typeof callback === 'function') {
                try {
                    ...
                    obj = null;
                    log.error(settings.namepsace + ' Cannot parse log object');
                }
                if (obj) {
                    callback(err, obj);
                } else {
        ...
        client.lrange(namespaceLog + id, 0, -1, function (err, obj) {
            if (obj) {
                ...
                if (!found) {
                    ...
                    log.error(settings.namepsace + ' WARNING: cannot find log with id = ' + logId);
                }
            }
        });
    };
    ...
    this.clearAllLogs = function (callback) {
        client.keys(namespaceLog + '*', function (err, obj) {
            if (obj) {
                for (var i = 0; i < obj.length; i++) {
                    log.debug(settings.namepsace + ' redis clear log for ' + obj[i]);
                    client.del(obj[i]);
                }
            }
    ...
    this.subscribeLog = function (id, callback) {
        log.debug(settings.namepsace + ' redis subscribeMessage ' + namespaceLog + id);
    ...
    this.unsubscribeLog = function (id, callback) {
        log.debug(settings.namepsace + ' redis unsubscribeMessage ' + namespaceLog + id);
    ...
    this.getSession = function (id, callback) {
        client.get(namespaceSession + id, function (err, obj) {
            log.debug(settings.namepsace + ' redis get ' + id + ' ' + obj);
    ...
    this.setSession = function (id, expire, obj, callback) {
        client.setex(namespaceSession + id, expire, JSON.stringify(obj), function () {
            log.debug(settings.namepsace + ' redis setex', id, expire, obj);
    ...
    this.destroySession = function (id, callback) {
        ...
        log.debug(settings.namepsace + ' redis del ' + id);
    ...
    this.getConfigKeys = function (pattern, callback, dontModify) {
        client.keys(namespaceConfig + pattern, function (err, obj) {
            log.debug(settings.namepsace + ' redis config keys ' + obj.length + ' ' + pattern);
            if (typeof callback === 'function') {
                if (obj && !dontModify) {
                    var len = namespace.length;
                    for (var i = 0; i < obj.length; i++) {
    ...
    this.setConfig = function (id, obj, callback) {
        ...
        client.set(id, JSON.stringify(obj), function (err) {
            log.debug(settings.namepsace + ' redis set', id, obj);
    ...
    this.subscribeConfig = function (id, callback) {
        log.debug(settings.namepsace + ' redis subscribeConfig ' + namespaceConfig + id);
    ...
    this.unsubscribeConfig = function (id, callback) {
        log.debug(settings.namepsace + ' redis unsubscribeConfig ' + namespaceConfig + id);
