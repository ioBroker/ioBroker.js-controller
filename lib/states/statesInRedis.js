...
class StateRedis {
    ...
    constructor(settings) {
        ...
        if (this.settings.connection.port === 0) { // Port = 0 means unix socket
            ...
            this.log.debug('Redis States: Use File Socket for connection: ' + this.settings.connection.options.path);
            ...
            this.settings.connection.options.sentinels = [];
            this.settings.connection.host.forEach((redisNode, idx) => {
                this.settings.connection.options.sentinels.push({
                    host: redisNode,
                    port: defaultPort ? defaultPort : this.settings.connection.port[idx]
                });
            });
            ...
            this.log.debug('Redis States: Use Sentinel for connection: ' + this.settings.connection.options.name + ', ' + JSON.stringify(this.settings.connection.options.sentinels));
        } else {
            ...
            this.log.debug('Redis States: Use Redis connection: ' + this.settings.connection.options.host + ':' + this.settings.connection.options.port);
        }
        ...
        const fallbackToSocketIo = () => {
            ...
            this.log.silly('Initiate Fallback to SocketIo States');
            const StatesSocketIo = require('./statesInMemClientSocketIo');
            const _newStates = new StatesSocketIo(originalSettings);
        };

        this.client.on('error', error => {
            if (this.settings.enhancedLogging) this.log.silly('Redis ERROR States: (' + ignoreErrors + '/' + this.stop + ') ' + error.message + ' / ' + error.stack);
            if (this.stop) return;
            if (!ready) {
                initError = true;
        ...
        this.client.on('end', () => {
            if (this.settings.enhancedLogging) this.log.silly('States-Redis Event end (stop=' + this.stop + ')');
            if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
        });

        this.client.on('connect', () => {
            if (this.settings.enhancedLogging) this.log.silly('States-Redis Event connect (stop=' + this.stop + ')');
            connected = true;
        });

        this.client.on('close', () => {
            if (this.settings.enhancedLogging) this.log.silly('States-Redis Event close (stop=' + this.stop + ')');
            if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
        });

        this.client.on('reconnecting', () => {
            if (connected && !ready && !initError && !ignoreErrors) reconnectCounter++;
            if (this.settings.enhancedLogging) this.log.silly('States-Redis Event reconnect (reconnectCounter=' + reconnectCounter + ', stop=' + this.stop + ')');
        ...
        this.client.on('ready', () => {
            if (this.settings.enhancedLogging) this.log.silly('States-Redis Event ready (stop=' + this.stop + ')');
            if (this.stop) return;
            initError = false;
            ignoreErrors = false;
            ...
            if (!ready && !this.sub) {
                ...
                if (typeof onChange === 'function') {
                    this.sub.on('pmessage', (pattern, channel, message) => {
                        this.log.silly(this.settings.namespace + ' redis pmessage ', pattern, channel, message);
                        try {
                            ...
                            this.log.warn(this.settings.namespace + ' pmessage ' + channel + ' ' + message + ' ' + e.message);
                            this.log.warn(this.settings.namespace + ' ' + e.stack);
                        }
                    });
                }
                ...
                this.sub.on('error', error => {
                    if (this.stop) return;
                    this.log.error(this.settings.namespace + ' Sub-Client States No redis connection: ' + error);
                });

                this.sub.on('ready', _error => {
                    if (this.settings.connection.port === 0) {
                        this.log.debug(this.settings.namespace + ' States connected to redis: ' + this.settings.connection.host);
                    } else {
                        this.log.debug(this.settings.namespace + ' States connected to redis: ' + this.settings.connection.host + ':' + this.settings.connection.port);
                    }
                });
    ...
    setState(id, state, callback) {
        ...
        this.client.get(this.namespaceRedis + id, (err, oldObj) => {
            ...
            // TODO Error Handling
            if (err) {
                this.log.warn(this.settings.namespace + ' get state ' + err);
            ...
            if (this.settings.enhancedLogging) this.log.silly(this.settings.namespace + ' redis publish ' + this.namespaceRedis + id + ' ' + JSON.stringify(obj));
    ...
    getState(id, callback) {
        this.client.get(this.namespaceRedis + id, (err, obj) => {
            if (err) {
                ...
                if (this.settings.enhancedLogging) this.log.silly(this.settings.namespace + ' redis get ' + id + ' ok: ' + obj);
            }
    ...
    getStates(keys, callback, dontModify) {
        ...
        this.client.mget(_keys, (err, obj) => {
            if (err) {
                ...
                if (this.settings.enhancedLogging) this.log.silly(this.settings.namespace + ' redis mget ' + ((!obj) ? 0 :  obj.length) + ' ' + _keys.length);
            }
    ...
    delState(id, callback) {
        this.client.del(this.namespaceRedis + id, err => {
            if (err) {
                ...
                if (this.settings.enhancedLogging) this.log.silly(this.settings.namespace + ' redis del ' + id + ', ok');
            }
    ...
    getKeys(pattern, callback, dontModify) {
        this.client.keys(this.namespaceRedis + pattern, (err, obj) => {
            if (this.settings.enhancedLogging) this.log.silly(this.settings.namespace + ' redis keys ' + obj.length + ' ' + pattern);
            if (typeof callback === 'function') {
                if (obj && !dontModify) {
    ...
    subscribe(pattern, callback) {
        if (this.settings.enhancedLogging) this.log.silly(this.settings.namespace + ' redis psubscribe ' + this.namespaceRedis + pattern);
        this.sub.psubscribe(this.namespaceRedis + pattern, err => typeof callback === 'function' && callback(err));
    }

    unsubscribe(pattern, callback) {
        if (this.settings.enhancedLogging) this.log.silly(this.settings.namespace + ' redis punsubscribe ' + this.namespaceRedis + pattern);
    ...
    clearAllMessages(callback) {
        this.client.keys(this.namespaceLog + '*', (err, obj) => {
            if (obj) {
                for (const o of obj) {
                    if (this.settings.enhancedLogging) this.log.silly('redis clear message for ' + o);
                    this.client.del(o);
                }
            }
    ...
    subscribeMessage(id, callback) {
        ...
        if (this.settings.enhancedLogging) this.log.silly('redis subscribeMessage ' + this.namespaceMsg + id);
        this.sub.psubscribe(this.namespaceMsg + id, err => typeof callback === 'function' && callback(err));
    }

    unsubscribeMessage(id, callback) {
        if (id && id[0] === '.') id = id.substring(1);
        if (this.settings.enhancedLogging) this.log.silly('redis unsubscribeMessage ' + this.namespaceMsg + id);
    ...
    clearAllLogs(callback) {
        this.client.keys(this.namespaceLog + '*', (err, obj) => {
            if (obj) {
                for (const o of obj) {
                    if (this.settings.enhancedLogging) this.log.silly(this.settings.namespace + ' redis clear log for ' + o);
                    this.client.del(o);
                }
            }
    ...
    subscribeLog(id, callback) {
        if (this.settings.enhancedLogging) this.log.silly(this.settings.namespace + ' redis subscribeMessage ' + this.namespaceLog + id);
    ...
    unsubscribeLog(id, callback) {
        if (this.settings.enhancedLogging) this.log.silly(this.settings.namespace + ' redis unsubscribeMessage ' + this.namespaceLog + id);
    ...
    getSession(id, callback) {
        this.client.get(this.namespaceSession + id, (_err, obj) => {
            if (this.settings.enhancedLogging) this.log.silly(this.settings.namespace + ' redis get ' + id + ' ' + obj);
    ...
    setSession(id, expire, obj, callback) {
        this.client.setex(this.namespaceSession + id, expire, JSON.stringify(obj), err => {
            if (this.settings.enhancedLogging) this.log.silly(this.settings.namespace + ' redis setex', id, expire, obj);
    ...
    destroySession(id, callback) {
        ...
        if (this.settings.enhancedLogging) this.log.silly(this.settings.namespace + ' redis del ' + id);
