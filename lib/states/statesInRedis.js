...
function StateRedis(settings) {
    ...
    if (settings.connection.options) {
        if (settings.connection.options.retry_max_delay) {
            ...
            settings.connection.options.retry_strategy = (_options) => {
    ...
    this.setState = function (id, state, callback) {
        ...
        client.get(redisNamespace + id, (err, oldObj) => {
            ...
            if (expire) {
                ...
                client.setex(redisNamespace + id, expire, JSON.stringify(obj), () =>
                    typeof callback === 'function' && callback());
            } else {
                //console.log('set',redisNamespace + id, JSON.stringify(obj));
                client.set(redisNamespace + id, JSON.stringify(obj), () =>
                    typeof callback === 'function' && callback());
            }
        });
    };
    ...
    this.setRawState = function (id, state, callback) {
        client.set(redisNamespace + id, JSON.stringify(state), () =>
            typeof callback === 'function' && callback());
    };
    ...
    this.delState = function (id, callback) {
        client.del(redisNamespace + id, err => {
            if (err) {
    ...
    this.getKeys = function (pattern, callback, dontModify) {
        client.keys(redisNamespace + pattern, (err, obj) => {
    ...
    this.subscribe = function (pattern, callback) {
        ...
        sub.psubscribe(redisNamespace + pattern, err => typeof callback === 'function' && callback(err));
    };

    this.unsubscribe = function (pattern, callback) {
        log.silly(settings.namespace + ' redis punsubscribe ' + redisNamespace + pattern);
        sub.punsubscribe(redisNamespace + pattern, err => typeof callback === 'function' && callback(err));
    };
    ...
    // todo: delete it
    this.delMessage = function (id, _messageId, callback) {
        if (typeof callback === 'function') callback(null, id);
    };

    // todo: delete it
    this.clearAllMessages = function (callback) {
        client.keys(namespaceLog + '*', (err, obj) => {
            if (obj) {
                for (let i = 0; i < obj.length; i++) {
    ...
    this.subscribeMessage = function (id, callback) {
        ...
        sub.psubscribe(namespaceMsg + id, err => typeof callback === 'function' && callback(err));
    };

    this.unsubscribeMessage = function (id, callback) {
        if (id && id[0] === '.') id = id.substring(1);
        log.silly('redis unsubscribeMessage ' + namespaceMsg + id);
        sub.punsubscribe(namespaceMsg + id, err => typeof callback === 'function' && callback(err));
    };
    ...
    this.lenLog = function (id, callback) {
        if (typeof callback === 'function') callback(tools.ERRORS.ERROR_NOT_EXISTS, 0, id);
        // client.llen(namespaceLog + id, (err, obj) => {
        //    if (typeof callback === 'function') callback(err, obj, id);
        // });
    };

    // todo: delete it
    this.getLog = function (_id, callback) {
        if (typeof callback === 'function') {
            callback(tools.ERRORS.ERROR_NOT_EXISTS, null, 0);
        }
    };

    // todo: delete it
    this.delLog = function (_id, _logId, callback) {
        if (typeof callback === 'function') {
    ...
    this.getSession = function (id, callback) {
        client.get(namespaceSession + id, (_err, obj) => {
    ...
    this.setSession = function (id, expire, obj, callback) {
        client.setex(namespaceSession + id, expire, JSON.stringify(obj), err => {
    ...
    this.destroySession = function (id, callback) {
        ...
        client.del(id, () => typeof callback === 'function' && callback());
    };

    function _createBinaryClient() {
        if (!clientBin) {
            settings.connection.options = settings.connection.options || {};
            const opt = JSON.parse(JSON.stringify(settings.connection.options));
    ...
    (function __construct() {
        ...
        sub.on('error', error => {
            log.error(settings.namespace + ' No redis connection: ' + error);
        });

        sub.on('connect', _error => {
            if (settings.connection.port === 0) {
