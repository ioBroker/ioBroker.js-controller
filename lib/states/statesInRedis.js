...
class StateRedis {
    ...
    constructor(settings) {
        ...
        this.client.on('error', error => {
            this.settings.enhancedLogging && this.log.silly('Redis ERROR States: (' + ignoreErrors + '/' + this.stop + ') ' + error.message + ' / ' + error.stack);
            if (this.stop) return;
            if (!ready) {
                initError = true;
        ...
        this.client.on('end', () => {
            this.settings.enhancedLogging && this.log.silly('States-Redis Event end (stop=' + this.stop + ')');
            if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
        });

        this.client.on('connect', () => {
            this.settings.enhancedLogging && this.log.silly('States-Redis Event connect (stop=' + this.stop + ')');
            connected = true;
        });

        this.client.on('close', () => {
            this.settings.enhancedLogging && this.log.silly('States-Redis Event close (stop=' + this.stop + ')');
            if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
        });

        this.client.on('reconnecting', () => {
            if (connected && !ready && !initError && !ignoreErrors) reconnectCounter++;
            this.settings.enhancedLogging && this.log.silly('States-Redis Event reconnect (reconnectCounter=' + reconnectCounter + ', stop=' + this.stop + ')');
        ...
        this.client.on('ready', () => {
            this.settings.enhancedLogging && this.log.silly('States-Redis Event ready (stop=' + this.stop + ')');
            if (this.stop) return;
            initError = false;
            ignoreErrors = false;
    ...
    setState(id, state, callback) {
        ...
        this.client.get(this.namespaceRedis + id, (err, oldObj) => {
            ...
            this.settings.enhancedLogging && this.log.silly(this.settings.namespace + ' redis publish ' + this.namespaceRedis + id + ' ' + JSON.stringify(obj));
    ...
    getState(id, callback) {
        this.client.get(this.namespaceRedis + id, (err, obj) => {
            if (err) {
                ...
                this.settings.enhancedLogging && this.log.silly(this.settings.namespace + ' redis get ' + id + ' ok: ' + obj);
            }
    ...
    delState(id, callback) {
        this.client.del(this.namespaceRedis + id, err => {
            if (err) {
                ...
                this.settings.enhancedLogging && this.log.silly(this.settings.namespace + ' redis del ' + id + ', ok');
            }
            typeof callback === 'function' && callback(err, id);
        });
    }
    ...
    getKeys(pattern, callback, dontModify) {
        this.client.keys(this.namespaceRedis + pattern, (err, obj) => {
            this.settings.enhancedLogging && this.log.silly(this.settings.namespace + ' redis keys ' + obj.length + ' ' + pattern);
            if (typeof callback === 'function') {
                if (obj && !dontModify) {
    ...
    subscribe(pattern, callback) {
        this.settings.enhancedLogging && this.log.silly(this.settings.namespace + ' redis psubscribe ' + this.namespaceRedis + pattern);
        this.sub.psubscribe(this.namespaceRedis + pattern, err => typeof callback === 'function' && callback(err));
    }

    unsubscribe(pattern, callback) {
        this.settings.enhancedLogging && this.log.silly(this.settings.namespace + ' redis punsubscribe ' + this.namespaceRedis + pattern);
    ...
    clearAllMessages(callback) {
        this.client.keys(this.namespaceLog + '*', (err, obj) => {
            if (obj) {
                for (const o of obj) {
                    this.settings.enhancedLogging && this.log.silly('redis clear message for ' + o);
                    this.client.del(o);
                }
            }
    ...
    subscribeMessage(id, callback) {
        ...
        this.settings.enhancedLogging && this.log.silly('redis subscribeMessage ' + this.namespaceMsg + id);
        this.sub.psubscribe(this.namespaceMsg + id, err => typeof callback === 'function' && callback(err));
    }

    unsubscribeMessage(id, callback) {
        if (id && id[0] === '.') id = id.substring(1);
        this.settings.enhancedLogging && this.log.silly('redis unsubscribeMessage ' + this.namespaceMsg + id);
    ...
    clearAllLogs(callback) {
        this.client.keys(this.namespaceLog + '*', (err, obj) => {
            if (obj) {
                for (const o of obj) {
                    this.settings.enhancedLogging && this.log.silly(this.settings.namespace + ' redis clear log for ' + o);
                    this.client.del(o);
                }
            }
    ...
    subscribeLog(id, callback) {
        this.settings.enhancedLogging && this.log.silly(this.settings.namespace + ' redis subscribeMessage ' + this.namespaceLog + id);
    ...
    unsubscribeLog(id, callback) {
        this.settings.enhancedLogging && this.log.silly(this.settings.namespace + ' redis unsubscribeMessage ' + this.namespaceLog + id);
    ...
    getSession(id, callback) {
        this.client.get(this.namespaceSession + id, (_err, obj) => {
            this.settings.enhancedLogging && this.log.silly(this.settings.namespace + ' redis get ' + id + ' ' + obj);
    ...
    setSession(id, expire, obj, callback) {
        this.client.setex(this.namespaceSession + id, expire, JSON.stringify(obj), err => {
            this.settings.enhancedLogging && this.log.silly(this.settings.namespace + ' redis setex', id, expire, obj);
            typeof callback === 'function' && callback(err);
        });
    }
    ...
    destroySession(id, callback) {
        ...
        this.settings.enhancedLogging && this.log.silly(this.settings.namespace + ' redis del ' + id);
