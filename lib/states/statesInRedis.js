...
class StateRedis {
    ...
    constructor(settings) {
        ...
        this.client.on('ready', () => {
            ...
            if (!ready && !this.sub) {
                this.client.config('set', 4, 'notify-keyspace-events', 'Exe', (err) => { // enable Expiry/Evicted events in server
                    if (err) {
                        ...
                        this.sub.on('message', (channel, message) => {
                            ...
                            try {
                                if (channel === '__keyevent@' + this.settings.connection.options.db + '__:expired') {
                                    onChange(message.slice(this.namespaceRedis.length), null);
                                } else
                    ...
                    this.sub.on('ready', _error => {
                        this.sub.subscribe('__keyevent@' + this.settings.connection.options.db + '__:expired', err => {
                            ...
                            this.sub.subscribe('__keyevent@' + this.settings.connection.options.db + '__:evicted', err => err && this.log.warn('Unable to subscribe to evicted Keyspace events from Redis Server: ' + err));
