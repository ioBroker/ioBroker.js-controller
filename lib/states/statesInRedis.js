...
class StateRedis {
    ...
    constructor(settings) {
        ...
        this.settings.connection.options.retryStrategy = (_options) => {
            if (!ready && initError && ignoreErrors) return new Error('ERR unknown command \'info\'');
        ...
        this.client.on('error', error => {
            console.log('Redis ERROR States: (' + ignoreErrors + '/' + this.stop + ') ' + error.message + ' / ' + error.stack);
            if (this.stop) return;
            if (!ready) {
                // Seems we have a socket.io server
                if (!initError && !ignoreErrors && error.message.startsWith('Protocol error, got "H" as reply type byte.')) {
                    initError = true;
                    this.stop = true;
                    ...
                    this._newStates = new StatesSocketIo(originalSettings);
                }
                return;
            }
        ...
        this.client.on('end', () => {
            console.log('Redis end (' + this.stop + ')');
            if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
        });

        this.client.on('close', () => {
            console.log('Redis close (' + this.stop + ')');
            if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
        });

        this.client.on('reconnecting', () => {
            console.log('Redis client will try to reconnect (' + this.stop + ')');
        });
    ...
    destroy() {
        console.log('DESTROY States Client');
        this.stop = true;
        if (this.client) {
            try {
                this.client.disconnect();
    ...
    clearAllMessages(callback) {
        this.client.keys(this.namespaceLog + '*', (err, obj) => {
            if (obj) {
                for (const o of obj) {
                    this.log.silly('redis clear message for ' + o);
                    this.client.del(o);
                }
            }
    ...
    clearAllLogs(callback) {
        this.client.keys(this.namespaceLog + '*', (err, obj) => {
            if (obj) {
                for (const o of obj) {
                    this.log.silly(this.settings.namespace + ' redis clear log for ' + o);
                    this.client.del(o);
                }
            }
    ...
    setBinaryState(id, data, callback) {
        ...
        //this.clientBin.set(this.namespaceRedis + id, data, callback);
        this.client.set(this.namespaceRedis + id, data, callback);
    }

    getBinaryState(id, callback) {
        //if (!this.clientBin) this._createBinaryClient ();
        //this.clientBin.get(this.namespaceRedis + id, (err, data) => {
    ...
    delBinaryState(id, callback) {
        //if (!this.clientBin) this._createBinaryClient ();
        //this.clientBin.del(this.namespaceRedis + id, () => typeof callback === 'function' && callback());
